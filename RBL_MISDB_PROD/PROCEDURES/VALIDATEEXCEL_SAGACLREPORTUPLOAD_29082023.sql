--------------------------------------------------------
--  DDL for Procedure VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" 
(
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            END;
            END IF;
            IF utils.object_id('TEMPDB..tt_UploadSAGACLReport_6') IS NOT NULL THEN

            BEGIN
               EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UploadSAGACLReport_6 ';

            END;
            END IF;
            --drop table IF exists tt_UploadSAGACLReport_6
            BEGIN
               SELECT 1 INTO v_temp
                 FROM DUAL
            ...
            BEGIN
               ...
               DELETE FROM tt_UploadSAGACLReport_6;
               UTILS.IDENTITY_RESET('tt_UploadSAGACLReport_6');

               INSERT INTO tt_UploadSAGACLReport_6 SELECT * ,
            ...
            END;
            END IF;
            UPDATE tt_UploadSAGACLReport_6 
            ...
            UPDATE tt_UploadSAGACLReport_6 V
            ...
            --SELECT DISTINCT UserLoginId INTO #DistinctUserId FROM tt_UploadSAGACLReport_6
            --IF NOT EXISTS(SELECT 1 FROM #DistinctUserId WHERE UserLoginId=@UserLoginId)
            --BEGIN
            --	UPDATE tt_UploadSAGACLReport_6 SET
            ...
            --	,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'User Id' ELSE   ErrorinColumn +','+SPACE(1)+'User Id' END     
            --	,Srnooferroneousrows=Srnooferroneousrows
            --FROM tt_UploadSAGACLReport_6 V
            UPDATE tt_UploadSAGACLReport_6 V
               SET ErrorMessage = CASE 
            ...
            ;
            IF utils.object_id('TEMPDB..tt_DUB_UCIF_13') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_UCIF_13 ';
            END IF;
            --SELECT * FROM tt_UploadSAGACLReport_6
            DELETE FROM tt_DUB_UCIF_13;
            UTILS.IDENTITY_RESET('tt_DUB_UCIF_13');

            INSERT INTO tt_DUB_UCIF_13 SELECT * 
                 FROM ( SELECT * ,
                               ROW_NUMBER() OVER ( PARTITION BY UCIC_ID ORDER BY UCIC_ID  ) rw  
                        FROM tt_UploadSAGACLReport_6  ) X
                WHERE  rw > 1;
            MERGE INTO V 
            ...
            FROM V ,tt_UploadSAGACLReport_6 V
                   JOIN tt_DUB_UCIF_13 D   ON D.UCIC_Id = V.UCIC_Id ) src
              ...
              AND V.CIF_Id NOT IN ( SELECT CustomerId 
                                    FROM CustomerBasicDetail 
                                     WHERE  ( EffectiveFromTimeKey <= v_Timekey
                                              AND EffectiveToTimeKey >= v_Timekey ) )
            ;
            IF utils.object_id('TEMPDB..tt_DUB_CIF_13') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_CIF_13 ';
            END IF;
            DELETE FROM tt_DUB_CIF_13;
            UTILS.IDENTITY_RESET('tt_DUB_CIF_13');

            INSERT INTO tt_DUB_CIF_13 SELECT * 
                 FROM ( SELECT * ,
            ...
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'CUSTOMER ID'
               END AS pos_3, V.Srnooferroneousrows
            FROM V ,tt_UploadSAGACLReport_6 V
                   JOIN tt_DUB_CIF_13 D   ON D.CIF_ID = V.CIF_ID ) src
            ON ( V.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET ErrorMessage = pos_2,
                                         ErrorinColumn = pos_3,
                                         Srnooferroneousrows = src.Srnooferroneousrows;
              ...
              AND V.ACCOUNT_Id NOT IN ( SELECT CustomerACID 
                                        FROM AdvAcBasicDetail 
                                         WHERE  ( EffectiveFromTimeKey <= v_Timekey
                                                  AND EffectiveToTimeKey >= v_Timekey ) )
            ;
            IF utils.object_id('TEMPDB..tt_DUB_ACCOUNTID_13') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_ACCOUNTID_13 ';
            END IF;
            DELETE FROM tt_DUB_ACCOUNTID_13;
            UTILS.IDENTITY_RESET('tt_DUB_ACCOUNTID_13');

            INSERT INTO tt_DUB_ACCOUNTID_13 SELECT * 
                 FROM ( SELECT * ,
            ...
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'ACCOUNT ID'
               END AS pos_3, V.Srnooferroneousrows
            FROM V ,tt_UploadSAGACLReport_6 V
                   JOIN tt_DUB_ACCOUNTID_13 D   ON D.ACCOUNT_Id = V.ACCOUNT_Id ) src
            ON ( V.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET ErrorMessage = pos_2,
                                         ErrorinColumn = pos_3,
                                         Srnooferroneousrows = src.Srnooferroneousrows;
         ...
         BEGIN
            ...
            BEGIN
               DELETE tt_UploadSAGACLReport_6
               ;
               DBMS_OUTPUT.PUT_LINE(1);
               DBMS_OUTPUT.PUT_LINE('ROWS DELETED FROM tt_UploadSAGACLReport_6' || UTILS.CONVERT_TO_VARCHAR2(SQL%ROWCOUNT,100));
            ...
            OPEN  v_cursor FOR
               SELECT 1 
                 FROM DUAL  ;
               DBMS_SQL.RETURN_RESULT(v_cursor);
            IF utils.object_id('TEMPDB..tt_EMPTY_6') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_EMPTY_6 ';
            END IF;
            DELETE FROM tt_EMPTY_6;
            OPEN  v_cursor FOR
               SELECT * 
                 FROM tt_EMPTY_6  ;
               DBMS_SQL.RETURN_RESULT(v_cursor);
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_29082023" TO "ADF_CDR_RBL_STGDB";
