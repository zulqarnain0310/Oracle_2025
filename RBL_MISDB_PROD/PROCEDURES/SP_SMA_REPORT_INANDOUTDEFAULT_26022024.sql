--------------------------------------------------------
--  DDL for Procedure SP_SMA_REPORT_INANDOUTDEFAULT_26022024
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" /*==============================                      
...
BEGIN
   ...
   -----------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_TempACCOUNTCAL_3') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TempACCOUNTCAL_3 ';
   END IF;
   DELETE FROM tt_TempACCOUNTCAL_3;
   UTILS.IDENTITY_RESET('tt_TempACCOUNTCAL_3');

   INSERT INTO tt_TempACCOUNTCAL_3 ( 
   	SELECT * 
   	  FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL 
   	 WHERE  FinalAssetClassAlt_Key = 1 );
   --and           DPD_MAX>0
   -------------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_ACCOUNT_MOVEMENT_HISTORY_10') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ACCOUNT_MOVEMENT_HISTORY_10 ';
   END IF;
   DELETE FROM tt_ACCOUNT_MOVEMENT_HISTORY_10;
   UTILS.IDENTITY_RESET('tt_ACCOUNT_MOVEMENT_HISTORY_10');

   INSERT INTO tt_ACCOUNT_MOVEMENT_HISTORY_10 ( 
   	SELECT * 
   ...
   -------------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_Temp_ACCOUNT_MOVEMENT_HI_7') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Temp_ACCOUNT_MOVEMENT_HI_7 ';
   END IF;
   DELETE FROM tt_Temp_ACCOUNT_MOVEMENT_HI_7;
   UTILS.IDENTITY_RESET('tt_Temp_ACCOUNT_MOVEMENT_HI_7');

   INSERT INTO tt_Temp_ACCOUNT_MOVEMENT_HI_7 ( 
   	SELECT A.CustomerAcID ,
           MAX(MovementFromDate)  MovementFromDate1  
   	  FROM PRO_RBL_MISDB_PROD.ACCOUNT_MOVEMENT_HISTORY a
             JOIN tt_TempACCOUNTCAL_3 b   ON a.CustomerAcID = b.CustomerACID
   ...
   -------------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_TempMaxEntityKey_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TempMaxEntityKey_2 ';
   END IF;
   DELETE FROM tt_TempMaxEntityKey_2;
   UTILS.IDENTITY_RESET('tt_TempMaxEntityKey_2');

   INSERT INTO tt_TempMaxEntityKey_2 ( 
   	SELECT A.CustomerAcID ,
   	 ...
   	 WHERE  A.FinalAssetClassAlt_Key = 1
   	  ...
   	  GROUP BY a.CustomerAcID );
   IF utils.object_id('TEMPDB..tt_Temp_ACCOUNT_MOVEMENT_HI_8') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Temp_ACCOUNT_MOVEMENT_HI_8 ';
   END IF;
   DELETE FROM tt_Temp_ACCOUNT_MOVEMENT_HI_8;
   UTILS.IDENTITY_RESET('tt_Temp_ACCOUNT_MOVEMENT_HI_8');

   INSERT INTO tt_Temp_ACCOUNT_MOVEMENT_HI_8 ( 
   	SELECT A.CustomerAcID ,
           MovementFromStatus ,
           MovementToStatus ,
           MovementFromDate ,
           MovementToDate 
   	  FROM PRO_RBL_MISDB_PROD.ACCOUNT_MOVEMENT_HISTORY a
             JOIN tt_TempMaxEntityKey_2 b   ON a.EntityKey = b.EntityKey );
   ------------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_Accountcal_hist_dpd_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Accountcal_hist_dpd_2 ';
   END IF;
   DELETE FROM tt_Accountcal_hist_dpd_2;
   UTILS.IDENTITY_RESET('tt_Accountcal_hist_dpd_2');

   INSERT INTO tt_Accountcal_hist_dpd_2 ( 
   	SELECT * 
   	  FROM PRO_RBL_MISDB_PROD.Accountcal_hist_DPD 
   	 WHERE  TimeKey = v_TIMEKEY );
   -----------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_ALL_CUSTOMER_EXPOSURE_DE') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ALL_CUSTOMER_EXPOSURE_DE ';
   END IF;
   DELETE FROM tt_ALL_CUSTOMER_EXPOSURE_DE;
   UTILS.IDENTITY_RESET('tt_ALL_CUSTOMER_EXPOSURE_DE');

   INSERT INTO tt_ALL_CUSTOMER_EXPOSURE_DE ( 
   	SELECT * 
   	  FROM ALL_CUSTOMER_EXPOSURE_DETAIL 
   	 WHERE  EffectiveFromTimeKey <= v_Timekey
              AND EffectiveToTimeKey >= v_Timekey );
   ------------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_TempDPD_4') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TempDPD_4 ';
   END IF;
   DELETE FROM tt_TempDPD_4;
   UTILS.IDENTITY_RESET('tt_TempDPD_4');

   INSERT INTO tt_TempDPD_4 ( 
   	SELECT b.UcifEntityID ,
           ...
           b.Asset_Norm ,
           A.* 
   	  FROM tt_TempACCOUNTCAL_3 b
             LEFT JOIN tt_Accountcal_hist_dpd_2 a   ON a.AccountEntityid = b.AccountEntityID
             AND a.TimeKey = v_Timekey );
   --where         b.FinalAssetClassAlt_Key=1

   EXECUTE IMMEDIATE ' ALTER TABLE tt_TempDPD_4 
   ...
   --UPDATE A SET DPD_Overdrawn=0 FROM tt_TempDPD_4 A where ISNULL(DPD_Overdrawn,0)<=30
   UPDATE tt_TempDPD_4
      SET DPD_Overdrawn = 0
    WHERE  NVL(DPD_Overdrawn, 0) <= 30;
   UPDATE tt_TempDPD_4
   ...
   --FROM tt_TempDPD_4 A where Asset_Norm='ALWYS_STD'
   ----		/*----------------FIND MAX DPD---------------------------------------*/
   ...
   IF tt_DPD_UCIF_ID_21  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_DPD_UCIF_ID_21;
   UTILS.IDENTITY_RESET('tt_DPD_UCIF_ID_21');

   INSERT INTO tt_DPD_UCIF_ID_21 ( 
   	SELECT UCIF_ID ,
   ...
   FROM A ,tt_TempDPD_4 A
          JOIN tt_DPD_UCIF_ID_21 B   ON A.UCIF_ID = B.UCIF_ID ) src
   ON ( A.ROWID = src.row_id )
   ...
   INSERT INTO tt_TEMP_36 ( 
   	SELECT UTILS.CONVERT_TO_NVARCHAR2(v_Date,30,p_style=>105) Report_date  ,
   	  ...
   	  FROM PRO_RBL_MISDB_PROD.CUSTOMERCAL A
             JOIN tt_TempACCOUNTCAL_3 B   ON A.CustomerEntityID = B.CustomerEntityID
   ...
   IF utils.object_id('TEMPDB..tt_TEMP_363') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_temp3_6 ';
   END IF;
   DELETE FROM tt_temp3_6;
   UTILS.IDENTITY_RESET('tt_temp3_6');

   INSERT INTO tt_temp3_6 SELECT RefSystemAcid ,
                                 OverDueSinceDt ,
                                 BillRefNo ,
                                 BillDueDt ,
                                 BillExtendedDueDt ,
      ...
      WHERE  EffectiveFromTimeKey <= v_Timekey
             ...
             ELSE BillExtendedDueDt
                END) <= v_Date
        ORDER BY BillDueDt;
   IF utils.object_id('TEMPDB..tt_PC_OVERDUE_41') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_41 ';
   END IF;
   DELETE FROM tt_PC_OVERDUE_41;
   UTILS.IDENTITY_RESET('tt_PC_OVERDUE_41');

   INSERT INTO tt_PC_OVERDUE_41 ( 
   	SELECT RefSystemAcid ,
   	 ...
   	 WHERE  EFFECTIVEFROMTIMEKEY <= v_Timekey
            ...
            ELSE PCExtendedDueDt
               END) <= v_Date );
   ------------------------------------------------------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_Final_Output_4') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Final_Output_4 ';
   END IF;
   DELETE FROM tt_Final_Output_4;
   UTILS.IDENTITY_RESET('tt_Final_Output_4');

   INSERT INTO tt_Final_Output_4 ( 
   	SELECT * 
   	  FROM ( SELECT A."Report date" ,
                    ...
                    NPA_Norms 
             FROM tt_TEMP_36 a
                    LEFT JOIN tt_temp3_6 b   ON a.Account_No_ = b.RefSystemAcid
             ...
             SELECT a.Report_date ,
                    ...
                    NPA_Norms 
             FROM tt_TEMP_36 a
                    LEFT JOIN tt_PC_OVERDUE_41 b   ON a.Account_No_ = b.RefSystemAcid
   ...
   IF utils.object_id('TEMPDB..#TEMPSCF') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPSCF_4 ';
   END IF;
   DELETE FROM tt_TEMPSCF_4;
   UTILS.IDENTITY_RESET('tt_TEMPSCF_4');

   INSERT INTO tt_TEMPSCF_4 ( 
   	SELECT UTILS.CONVERT_TO_NVARCHAR2(v_Date,30,p_style=>105) Report_date  ,
           A.UCIF_ID UCIC  ,
           A.RefCustomerID CIF_ID  ,
           REPLACE(CustomerName, ',', ' ') Borrower_Name  ,
   ...
   DELETE tt_TEMPSCF_4
    ...
    WHERE  Scheme_Type = 'ODA'
             ...
             AND NVL(DPD_Limit_Expiry, 0) - 90 <= 0;
   ---------------------------------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_TEMP_363SCF') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_temp3SCF_4 ';
   END IF;
   DELETE FROM tt_temp3SCF_4;
   UTILS.IDENTITY_RESET('tt_temp3SCF_4');

   INSERT INTO tt_temp3SCF_4 SELECT RefSystemAcid ,
                                    OverDueSinceDt ,
                                    BillRefNo ,
                                    BillDueDt ,
                                    BillExtendedDueDt ,
      ...
      WHERE  EffectiveFromTimeKey <= v_Timekey
        ...
        ORDER BY BillDueDt;
   -----------------------------------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_Final_Output_4SCF') IS NOT NULL THEN
    -----------------------------------------------------------------------------------------------------------------------------
   EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Final_OutputSCF_4 ';
   END IF;
   DELETE FROM tt_Final_OutputSCF_4;
   UTILS.IDENTITY_RESET('tt_Final_OutputSCF_4');

   INSERT INTO tt_Final_OutputSCF_4 ( 
   	SELECT DISTINCT A."Report date" ,
                    ...
                    NPA_Norms 
   	  FROM tt_TEMPSCF_4 a
             LEFT JOIN tt_temp3SCF_4 b   ON a.Account_No_ = b.RefSystemAcid );
   ...
   INSERT INTO OverdueReportPhase2
     ( SELECT Report_date ,
              ...
              Asset_Classification ,
              NPA_Norms 
       FROM tt_Final_Output_4 a
              JOIN tt_TempDPD_4 b   ON a.Account_No_ = b.CustomerAcID
       ...
       SELECT Report_date ,
              ...
              Asset_Classification ,
              NPA_Norms 
       FROM tt_Final_OutputSCF_4 a
              LEFT JOIN tt_TempDPD_4 b   ON a.Account_No_ = b.CustomerAcID );
   ---------------------------------------Overdue Bill & PC END--------------------------------------------------------
   ---------------------------------------Investment Start--------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_TempInvestment_4') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TempInvestment_4 ';
   END IF;
   DELETE FROM tt_TempInvestment_4;
   UTILS.IDENTITY_RESET('tt_TempInvestment_4');

   INSERT INTO tt_TempInvestment_4 ( 
   ...
   IF utils.object_id('TEMPDB..tt_TempDefault_3') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TempDefault_3 ';
   END IF;
   DELETE FROM tt_TempDefault_3;
   UTILS.IDENTITY_RESET('tt_TempDefault_3');

   INSERT INTO tt_TempDefault_3 ( 
   ...
   UPDATE tt_TempACCOUNTCAL_3
      SET SMA_Class = 'Agri'
    WHERE  FinalAssetClassAlt_Key = 1
     AND RefPeriodOverdue > 91;
   -------------------------------------------------------------------------------

   EXECUTE IMMEDIATE ' ALTER TABLE tt_TempACCOUNTCAL_3 
   ...
   FROM A ,tt_TempACCOUNTCAL_3 a
          JOIN tt_TempDefault_3 b   ON a.AccountEntityID = b.AccountEntityID 
   ...
   */
   ...
   --------------------IF ANY DPD IS NEGATIVE THEN ZERO-----------------------------------
    ...
    UPDATE tt_TempDPD_4 SET DPD_IntService=0 WHERE isnull(DPD_IntService,0)<0
    UPDATE tt_TempDPD_4 SET DPD_NoCredit=0 WHERE isnull(DPD_NoCredit,0)<0
    UPDATE tt_TempDPD_4 SET DPD_Overdrawn=0 WHERE isnull(DPD_Overdrawn,0)<0
    UPDATE tt_TempDPD_4 SET DPD_Overdue=0 WHERE isnull(DPD_Overdue,0)<0
    UPDATE tt_TempDPD_4 SET DPD_Renewal=0 WHERE isnull(DPD_Renewal,0)<0
    UPDATE tt_TempDPD_4 SET DPD_StockStmt=0 WHERE isnull(DPD_StockStmt,0)<0
    UPDATE A SET DPD_Overdrawn=0 FROM tt_TempDPD_4 A where ISNULL(DPD_Overdrawn,0)<=30
    UPDATE tt_TempDPD_4 SET DPD_IntOverdueSince=0 WHERE isnull(DPD_StockStmt,0)<0
    UPDATE tt_TempDPD_4 SET DPD_PrincOverdue=0 WHERE isnull(DPD_StockStmt,0)<0
    UPDATE tt_TempDPD_4 SET DPD_OtherOverdueSince=0 WHERE isnull(DPD_StockStmt,0)<0
   ...
   ----		------------------FIND MAX DPD-----------------------------------------
   	...
   	DROP TABLE IF EXISTS tt_DPD_UCIF_ID_21
   	SELECT UCIF_ID,MAX(DPD_MAX) DPD_UCIF_ID 
   		INTO tt_DPD_UCIF_ID_21
   	FROM tt_TempDPD_4 
   	GROUP BY UCIF_ID
   	...
   	FROM tt_TempDPD_4 A
   		INNER JOIN tt_DPD_UCIF_ID_21 B
   			ON A.UCIF_ID =B.UCIF_ID
   */
   ------------------------------------------------------------------------------------------------------
   --Truncate table SMA_OUTPUT
   --Insert into SMA_OUTPUT
   IF utils.object_id('TEMPDB..tt_Temp_SMA_Main_Table_3') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Temp_SMA_Main_Table_3 ';
   END IF;
   DELETE FROM tt_Temp_SMA_Main_Table_3;
   UTILS.IDENTITY_RESET('tt_Temp_SMA_Main_Table_3');

   INSERT INTO tt_Temp_SMA_Main_Table_3 ( 
   	SELECT * 
   	  FROM ( SELECT v_PROCESSDATE Report_Date  ,
                    ...
                    NVL(DPD.DPD_Overdue, 0) DPD_Overdue  ,
                    ...
                    CASE 
                         WHEN SMA_Class IN ( 'FDOD','Agri' )
                          THEN 0
                         WHEN A.SourceAlt_Key <> 6
                           AND A.FacilityType IN ( 'CC','OD' )

                           AND DPD.DPD_Overdrawn <= 30 THEN 0
                         WHEN A.SourceAlt_Key <> 6
                           AND A.FacilityType IN ( 'CC','OD' )

                           AND DPD.DPD_Overdrawn >= 31 THEN DPD.DPD_Overdrawn
                    ELSE DPD.DPD_Overdue
                       END SMA_DPD  ,
                    ...
                    NVL(DPD.DPD_UCIF_ID, 0) DPD_UCIF_ID  ,
                    F.FlgSMA UCICFlgSMA  ,
                    F.SMA_Dt UCICSMA_Dt  ,
                    F.CustMoveDescription UCICSMA_AssetStatus  ,
                    F.SMA_Dt SMA_Classification_Date  ,
                    In_Default in_default_Y_N_  ,
                    --,A.SMA_Dt [in_default date]
                    MD.MovementFromDate1 in_default_date  ,
                    Out_Default out_of_default_Y_N_  ,
                    CASE 
                    ...
                    CE.CREDIT_FB_EXPOSURE Credit_FB_Exposure  ,
                    CE.CREDIT_NFB_EXPOSURE Credit_NFB_Exposure  ,
                    CE.NON_SLR Non_SLR  ,
                    CE.LER_O_s LER  ,
                    CE.GROSS_TOTAL_EXPOSURE Total_Exposure  ,
                    CASE 
                         WHEN CE.GROSS_TOTAL_EXPOSURE >= 50000000 THEN 'Y'
                    ELSE 'N'
                       END Exposure_5crore_above_Flag  ,
                    CASE 
                    ...
                    NVL(DPD.DPD_StockStmt, 0) DPD_StockStmt  ,
                    ...
                    NVL(DPD.DPD_NoCredit, 0) DPD_NoCredit  ,
             ...
             FROM tt_TempACCOUNTCAL_3 A
                  ...
                  --INNER JOIN Pro.CustomerCal_hist F On F.CustomerEntityId=A.CustomerEntityId
                    ...
                    LEFT JOIN tt_TempDPD_4 DPD   ON DPD.AccountEntityID = A.AccountEntityID
                    LEFT JOIN tt_ACCOUNT_MOVEMENT_HISTORY_10 I   ON I.CustomerAcID = A.CustomerAcID
                    ...
                    LEFT JOIN OverdueReportPhase2 OP2   ON OP2.Account_No = A.CustomerAcID
                    LEFT JOIN tt_ALL_CUSTOMER_EXPOSURE_DE CE   ON CE.CustomerEntityId = F.CustomerEntityID
                    AND CE.EffectiveFromTimeKey <= v_TIMEKEY
                    AND CE.EffectiveToTimeKey >= v_TIMEKEY
                    LEFT JOIN tt_Temp_ACCOUNT_MOVEMENT_HI_7 MD   ON MD.CustomerAcID = A.CustomerAcID
                    LEFT JOIN tt_Temp_ACCOUNT_MOVEMENT_HI_8 MS   ON MS.CustomerAcID = A.CustomerAcID
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SP_SMA_REPORT_INANDOUTDEFAULT_26022024" TO "ADF_CDR_RBL_STGDB";
