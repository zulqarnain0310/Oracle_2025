--------------------------------------------------------
--  DDL for Procedure VALIDATEEXCEL_UPLOAD_PROD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" 
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
BEGIN
   ...
   IF utils.object_id('tempdb..tt_OAOLMasterUploadData_6') IS NOT NULL THEN

   BEGIN
      EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_OAOLMasterUploadData_6 ';

   END;
   END IF;
   DELETE FROM tt_OAOLMasterUploadData_6;
   ...
   BEGIN
      IF utils.object_id('TEMPDB..tt_UserMasterUploadData_16') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UserMasterUploadData_16 ';
      END IF;
      --Select * from DimUserInfo
      DELETE FROM tt_UserMasterUploadData_16;
      UTILS.IDENTITY_RESET('tt_UserMasterUploadData_16');

      INSERT INTO tt_UserMasterUploadData_16 ( 
      ...
      --SELECT * FROM tt_UserMasterUploadData_16
      --uPDATE tt_UserMasterUploadData_16 SET --ApplicableSolID =REPLACE(REPLACE(ApplicableSolID, CHAR(13), ''), CHAR(10), '')
      --ApplicableBACID =REPLACE(REPLACE(ApplicableBACID, CHAR(13), ''), CHAR(10), ''),UserRole=LTRIM(RTRIM(UserRole))
      --SELECT * FROM tt_UserMasterUploadData_16
      INSERT INTO tt_OAOLMasterUploadData_6
        ( SELECT SrNo ,
                 -- ,RowNo  
                 'Sr No' ColumnName  ,
                 NULL ErrorData  ,
                 ...
                 T.UserID 
          FROM tt_UserMasterUploadData_16 T
      ...
      BEGIN
         SELECT 1 INTO v_temp
          ...
          WHERE EXISTS ( SELECT 1 
                         FROM tt_OAOLMasterUploadData_6  );
      EXCEPTION
         WHEN OTHERS THEN
            NULL;
      END;
          ...
          WHERE EXISTS ( SELECT 1 
                         FROM tt_UserMasterUploadData_16 T
                          WHERE  T.UserID IN ( SELECT UserID 
                                               FROM tt_UserMasterUploadData_16 
                                                 GROUP BY UserID
      ...
      --(
      --   Select UserEmailId from tt_UserMasterUploadData_16
      --   group by UserEmailId
      --   having COUNT(UserEmailId) > 1
      ...
      --		(
      --		   Select UserExtensionNumber from tt_UserMasterUploadData_16
      --		   group by UserExtensionNumber
      --		   having COUNT(UserExtensionNumber) > 1
      --		) AND ISNULL(T.UserExtensionNumber,'') <> ''
      --GOTO RETURNDATA
      ---- 
      IF utils.object_id('TEMPDB..tt_TempAlreadyExists_6') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TempAlreadyExists_6 ';
      END IF;
      DELETE FROM tt_TempAlreadyExists_6;
      UTILS.IDENTITY_RESET('tt_TempAlreadyExists_6');

      INSERT INTO tt_TempAlreadyExists_6 ( 
      	SELECT * 
      ...
      BEGIN
         SELECT 1 INTO v_temp
          ...
          WHERE EXISTS ( SELECT 1 
                         ...
                         SELECT 1 
                         FROM tt_UserMasterUploadData_16 T
                                JOIN tt_TempAlreadyExists_6 E   ON E.Email_ID = T.UserEmailId
                                AND T.Perform = 'A'
                                AND NVL(T.UserEmailId, ' ') <> ' '
                         UNION 
                         ...
                         SELECT 1 
                         FROM tt_UserMasterUploadData_16 T
                                JOIN tt_TempAlreadyExists_6 E   ON E.MobileNo = T.UserMobileNumber
                                AND T.Perform = 'A'
                                AND NVL(T.UserMobileNumber, ' ') <> ' '
                         UNION 
                         ...
                         SELECT 1 
                         FROM tt_UserMasterUploadData_16 T
                                JOIN tt_TempAlreadyExists_6 E   ON E.ExtensionNo = T.UserExtensionNumber
                                AND T.Perform = 'A'
                                AND NVL(T.UserExtensionNumber, ' ') <> ' '
                         UNION 
      ...
      --  ,T.UserID
      --FROM tt_UserMasterUploadData_16 T  
      --inner join tt_TempAlreadyExists_6 E ON E.ExtensionNo=T.UserExtensionNumber
      --AND T.Perform='A'
      --AND ISNULL(T.UserExtensionNumber,'') <> ''
      --UNION
      ----INSERT INTO tt_OAOLMasterUploadData_6 
      ...
      --FROM tt_UserMasterUploadData_16 T  
      --inner join tt_TempAlreadyExists_6 E ON E.ExtensionNo=T.UserExtensionNumber
      --AND T.UserID NOT IN (Select UserLoginID from tt_TempAlreadyExists_6)
      --AND T.Perform='U'
      --AND ISNULL(T.UserExtensionNumber,'') <> ''
      -- GOTO RETURNDATA 
      -----
      ...
      ----	inner join tt_UserMasterUploadData_16 U ON U.UserDepartment=D.DepartmentCode
      ...
      ----inner join tt_UserMasterUploadData_16 U ON U.UserID=T.UserID
      ...
      ------SELECT * FROM #TempSelectedBACIDsUserWise
      ------SELECT * FROM tt_OAOLMasterUploadData_6
      --INSERT INTO tt_OAOLMasterUploadData_6    
      --SELECT Distinct  t.SrNo  
      --    -- ,RowNo  
      --     ,'Applicable BAC ID' ColumnName  
      ...
      --inner join tt_UserMasterUploadData_16 U ON 
      ----U.UserID=T.UserID
      ----AND 
      ...
      -- )
      -- A 
      -- INNER JOIN tt_UserMasterUploadData_16 B 
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD_PROD" TO "ADF_CDR_RBL_STGDB";
