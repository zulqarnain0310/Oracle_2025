--------------------------------------------------------
--  DDL for Procedure VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" 
(
  iv_MenuID IN NUMBER DEFAULT 24762 ,
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            END;
            END IF;
            IF utils.object_id('TEMPDB..tt_UploadProvCompReport_3') IS NOT NULL THEN

            BEGIN
               EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UploadProvCompReport_3 ';
            ...
            BEGIN
               ...
               DELETE FROM tt_UploadProvCompReport_3;
               UTILS.IDENTITY_RESET('tt_UploadProvCompReport_3');

               INSERT INTO tt_UploadProvCompReport_3 SELECT * ,
            ...
            END;
            END IF;
            UPDATE tt_UploadProvCompReport_3 
            ...
            BEGIN
               SELECT 1 INTO v_temp
                 FROM DUAL
                WHERE EXISTS ( SELECT 1 
                               FROM tt_UploadProvCompReport_3 
            ...
            UPDATE tt_UploadProvCompReport_3 V
            ...
            --SELECT DISTINCT UserLoginId INTO #DistinctUserId FROM tt_UploadProvCompReport_3
            --IF NOT EXISTS(SELECT 1 FROM #DistinctUserId WHERE UserLoginId=@UserLoginId)
            --BEGIN
            --	UPDATE tt_UploadProvCompReport_3 SET
            ...
            --	,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'User ID' ELSE   ErrorinColumn +','+SPACE(1)+'User ID' END     
            --	,Srnooferroneousrows=Srnooferroneousrows
            --FROM tt_UploadProvCompReport_3 V
            UPDATE tt_UploadProvCompReport_3 V
               SET ErrorMessage = CASE 
            ...
            ;
            IF utils.object_id('TEMPDB..tt_DUB_UCIF_7') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_UCIF_7 ';
            END IF;
            --SELECT * FROM tt_UploadProvCompReport_3
            DELETE FROM tt_DUB_UCIF_7;
            UTILS.IDENTITY_RESET('tt_DUB_UCIF_7');

            INSERT INTO tt_DUB_UCIF_7 SELECT * 
                 FROM ( SELECT * ,
                               ROW_NUMBER() OVER ( PARTITION BY UCIC_ID ORDER BY UCIC_ID  ) rw  
                        FROM tt_UploadProvCompReport_3  ) X
                WHERE  rw > 1;
            MERGE INTO V 
            ...
            FROM V ,tt_UploadProvCompReport_3 V
                   JOIN tt_DUB_UCIF_7 D   ON D.UCIC_Id = V.UCIC_Id ) src
              ...
              AND V.CIF_Id NOT IN ( SELECT CustomerId 
                                    FROM CustomerBasicDetail 
                                     WHERE  ( EffectiveFromTimeKey <= v_Timekey
                                              AND EffectiveToTimeKey >= v_Timekey ) )
            ;
            IF utils.object_id('TEMPDB..tt_DUB_CIF_7') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_CIF_7 ';
            END IF;
            DELETE FROM tt_DUB_CIF_7;
            UTILS.IDENTITY_RESET('tt_DUB_CIF_7');

            INSERT INTO tt_DUB_CIF_7 SELECT * 
                 FROM ( SELECT * ,
            ...
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'CUSTOMER ID'
               END AS pos_3, V.Srnooferroneousrows
            FROM V ,tt_UploadProvCompReport_3 V
                   JOIN tt_DUB_CIF_7 D   ON D.CIF_ID = V.CIF_ID ) src
            ON ( V.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET ErrorMessage = pos_2,
                                         ErrorinColumn = pos_3,
                                         Srnooferroneousrows = src.Srnooferroneousrows;
              ...
              AND V.ACCOUNT_Id NOT IN ( SELECT CustomerACID 
                                        FROM AdvAcBasicDetail 
                                         WHERE  ( EffectiveFromTimeKey <= v_Timekey
                                                  AND EffectiveToTimeKey >= v_Timekey ) )
            ;
            IF utils.object_id('TEMPDB..tt_DUB_ACCOUNTID_7') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_ACCOUNTID_7 ';
            END IF;
            DELETE FROM tt_DUB_ACCOUNTID_7;
            UTILS.IDENTITY_RESET('tt_DUB_ACCOUNTID_7');

            INSERT INTO tt_DUB_ACCOUNTID_7 SELECT * 
                 FROM ( SELECT * ,
            ...
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'ACCOUNT ID'
               END AS pos_3, V.Srnooferroneousrows
            FROM V ,tt_UploadProvCompReport_3 V
                   JOIN tt_DUB_ACCOUNTID_7 D   ON D.ACCOUNT_Id = V.ACCOUNT_Id ) src
            ON ( V.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET ErrorMessage = pos_2,
                                         ErrorinColumn = pos_3,
                                         Srnooferroneousrows = src.Srnooferroneousrows;
            ...
            UPDATE tt_UploadProvCompReport_3 V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'CustomerId & Account_Id is Availble and Valid but UCIC ID is not available for the Uploaded date' || ' ' || UTILS.CONVERT_TO_VARCHAR2(v_rptDate,10,p_style=>103) || '. Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'CustomerId & Account_Id is Availble and Valid but UCIC ID is not available for the Uploaded date' || ' ' || UTILS.CONVERT_TO_VARCHAR2(v_rptDate,10,p_style=>103) || '. Please check the values and upload again'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'UCIC ID'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'UCIC ID'
            ...
            UPDATE tt_UploadProvCompReport_3 V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Account_Id, CustomerId & UCIC ID is not available for the Uploaded date' || ' ' || UTILS.CONVERT_TO_VARCHAR2(v_rptDate,10,p_style=>103) || '. Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Account_Id, CustomerId & UCIC ID is not available for the Uploaded date' || ' ' || UTILS.CONVERT_TO_VARCHAR2(v_rptDate,10,p_style=>103) || '. Please check the values and upload again'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'CUSTOMER ID'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'CUSTOMER ID'
            ...
            UPDATE tt_UploadProvCompReport_3 V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Account_Id is Availble and Valid but CustomerId & UCIC ID is not available for the Uploaded date' || ' ' || UTILS.CONVERT_TO_VARCHAR2(v_rptDate,10,p_style=>103) || '. Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Account_Id is Availble and Valid but CustomerId & UCIC ID is not available for the Uploaded date' || ' ' || UTILS.CONVERT_TO_VARCHAR2(v_rptDate,10,p_style=>103) || '. Please check the values and upload again'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'CUSTOMER ID'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'CUSTOMER ID'
         ...
         BEGIN
            ...
            BEGIN
               DELETE tt_UploadProvCompReport_3
               ;
               DBMS_OUTPUT.PUT_LINE(1);
               DBMS_OUTPUT.PUT_LINE('ROWS DELETED FROM tt_UploadProvCompReport_3' || UTILS.CONVERT_TO_VARCHAR2(SQL%ROWCOUNT,100));
            ...
            OPEN  v_cursor FOR
               SELECT 1 
                 FROM DUAL  ;
               DBMS_SQL.RETURN_RESULT(v_cursor);
            IF utils.object_id('TEMPDB..tt_EMPTY_3') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_EMPTY_3 ';
            END IF;
            DELETE FROM tt_EMPTY_3;
            OPEN  v_cursor FOR
               SELECT * 
                 FROM tt_EMPTY_3  ;
               DBMS_SQL.RETURN_RESULT(v_cursor);
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_07082023" TO "ADF_CDR_RBL_STGDB";
