--------------------------------------------------------
--  DDL for Procedure VALIDATEEXCEL_UPLOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" 
(
...
BEGIN
   ...
   BEGIN
      IF utils.object_id('tempdb..tt_OAOLMasterUploadData') IS NOT NULL THEN

      BEGIN
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_OAOLMasterUploadData ';

      END;
      END IF;
      DELETE FROM tt_OAOLMasterUploadData;
      ...
      BEGIN
         IF utils.object_id('TEMPDB..tt_UserMasterUploadData_11') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UserMasterUploadData_11 ';
         END IF;
         --Select * from DimUserInfo
         DELETE FROM tt_UserMasterUploadData_11;
         UTILS.IDENTITY_RESET('tt_UserMasterUploadData_11');

         INSERT INTO tt_UserMasterUploadData_11 ( 
         ...
         --SELECT * FROM tt_UserMasterUploadData_11
         --uPDATE tt_UserMasterUploadData_11 SET --ApplicableSolID =REPLACE(REPLACE(ApplicableSolID, CHAR(13), ''), CHAR(10), '')
         --ApplicableBACID =REPLACE(REPLACE(ApplicableBACID, CHAR(13), ''), CHAR(10), ''),UserRole=LTRIM(RTRIM(UserRole))
         --SELECT 'tt_UserMasterUploadData_11', * FROM tt_UserMasterUploadData_11
         INSERT INTO tt_OAOLMasterUploadData
           ( SELECT SrNo ,
                    -- ,RowNo  
                    'Sr No' ColumnName  ,
                    NULL ErrorData  ,
                    ...
                    T.UserID 
             FROM tt_UserMasterUploadData_11 T
             ...
             WHERE EXISTS ( SELECT 1 
                            FROM tt_UserMasterUploadData_11 T
                             WHERE  T.UserID IN ( SELECT UserID 
                                                  FROM tt_UserMasterUploadData_11 
                                                    GROUP BY UserID
         ...
         --		(
         --		   Select UserExtensionNumber from tt_UserMasterUploadData_11
         --		   group by UserExtensionNumber
         --		   having COUNT(UserExtensionNumber) > 1
         --		) AND ISNULL(T.UserExtensionNumber,'') <> ''
         --GOTO RETURNDATA
         ---- 
         IF utils.object_id('TEMPDB..tt_TempAlreadyExists') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TempAlreadyExists ';
         END IF;
         DELETE FROM tt_TempAlreadyExists;
         UTILS.IDENTITY_RESET('tt_TempAlreadyExists');

         INSERT INTO tt_TempAlreadyExists ( 
         	SELECT * 
         ...
         BEGIN
            SELECT 1 INTO v_temp
             ...
             WHERE EXISTS ( SELECT 1 
                            ...
                            SELECT 1 
                            FROM tt_UserMasterUploadData_11 T
                                   JOIN tt_TempAlreadyExists E   ON E.Email_ID = T.UserEmailId
                                   AND T.Perform = 'A'
                                   AND NVL(T.UserEmailId, ' ') <> ' '
                            UNION 
                            ...
                            SELECT 1 
                            FROM tt_UserMasterUploadData_11 T
                                   JOIN tt_TempAlreadyExists E   ON E.MobileNo = T.UserMobileNumber
                                   AND T.Perform = 'A'
                                   AND NVL(T.UserMobileNumber, ' ') <> ' '
                            UNION 
                            ...
                            SELECT 1 
                            FROM tt_UserMasterUploadData_11 T
                                   JOIN tt_TempAlreadyExists E   ON E.ExtensionNo = T.UserExtensionNumber
                                   AND T.Perform = 'A'
                                   AND NVL(T.UserExtensionNumber, ' ') <> ' '
                            UNION 
         ...
         --  ,T.UserID
         --FROM tt_UserMasterUploadData_11 T  
         --inner join tt_TempAlreadyExists E ON E.ExtensionNo=T.UserExtensionNumber
         --AND T.Perform='A'
         --AND ISNULL(T.UserExtensionNumber,'') <> ''
         --UNION
         ----INSERT INTO tt_OAOLMasterUploadData 
         ...
         --FROM tt_UserMasterUploadData_11 T  
         --inner join tt_TempAlreadyExists E ON E.ExtensionNo=T.UserExtensionNumber
         --AND T.UserID NOT IN (Select UserLoginID from tt_TempAlreadyExists)
         --AND T.Perform='U'
         --AND ISNULL(T.UserExtensionNumber,'') <> ''
         -- GOTO RETURNDATA 
         -----
         ...
         ----	inner join tt_UserMasterUploadData_11 U ON U.UserDepartment=D.DepartmentCode
         ...
         ----inner join tt_UserMasterUploadData_11 U ON U.UserID=T.UserID
         ...
         ------SELECT * FROM #TempSelectedBACIDsUserWise
         ------SELECT * FROM tt_OAOLMasterUploadData
         --INSERT INTO tt_OAOLMasterUploadData    
         --SELECT Distinct  t.SrNo  
         --    -- ,RowNo  
         --     ,'Applicable BAC ID' ColumnName  
         ...
         --inner join tt_UserMasterUploadData_11 U ON 
         ----U.UserID=T.UserID
         ----AND 
         ...
         -- )
         -- A 
         -- INNER JOIN tt_UserMasterUploadData_11 B 
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_UPLOAD" TO "ADF_CDR_RBL_STGDB";
