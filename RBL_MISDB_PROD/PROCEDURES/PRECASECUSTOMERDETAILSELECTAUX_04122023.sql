--------------------------------------------------------
--  DDL for Procedure PRECASECUSTOMERDETAILSELECTAUX_04122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" 
(
  --DECLARE
...
AS
   ...
   --IF (SELECT COUNT(CustomerEntityId) FROM tt_CustomerEntityId_2)>100
   --	BEGIN
   --			SET @Result=-3  /* If customer DATA IS MORE THAN 100*/
   --			RETURN @Result
   --	END
   v_CntCust NUMBER(10,0) := ( SELECT COUNT(*)  
     FROM tt_CustomerEntityId_2  );
   v_temp NUMBER(1, 0) := 0;
   v_cursor SYS_REFCURSOR;

BEGIN

   IF utils.object_id('TEMPDB..tt_CustomerEntityId_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CustomerEntityId_2 ';
   END IF;
   DELETE FROM tt_CustomerEntityId_2;
   ...
   BEGIN
      --SELECT 'TRI'
      INSERT INTO tt_CustomerEntityId_2
        ( CustomerEntityId )
   ...
   END;
   END IF;
   IF utils.object_id('Tempdb..tt_CustAcData_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CustAcData_2 ';
   END IF;
   DELETE FROM tt_CustAcData_2;
   IF NVL(v_CustomerAcID, ' ') <> ' ' THEN
    DECLARE
      v_temp NUMBER(1, 0) := 0;

   BEGIN
      INSERT INTO tt_CustAcData_2
        ( SELECT CustomerEntityID ,
                 AuthorisationStatus 
   ...
   END IF;
   --RETURN @Result
   IF utils.object_id('Tempdb..tt_CustDefData_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CustDefData_2 ';
   END IF;
   DELETE FROM tt_CustDefData_2;
   IF NVL(v_DefendentName, ' ') <> ' ' THEN
    DECLARE
      v_temp NUMBER(1, 0) := 0;

   BEGIN
      INSERT INTO tt_CustDefData_2
   ...
   END IF;
   --RETURN @Result
   --IF (SELECT COUNT(CustomerEntityId) FROM tt_CustDefData_2)>1000
   --BEGIN
   --		SET @Result=-4  /* If customer defendant DATA IS MORE THAN 100*/
   --		RETURN @Result
   --END
   IF utils.object_id('Tempdb..tt_CustBrData_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CustBrData_2 ';
   END IF;
   DELETE FROM tt_CustBrData_2;
   BEGIN
      SELECT 1 INTO v_temp
        FROM DUAL
   ...
   BEGIN
      ...
      INSERT INTO tt_CustBrData_2
        ( SELECT ABD.CustomerEntityID ,
                 BranchCode 
          FROM CurDat_RBL_MISDB_PROD.AdvAcBasicDetail ABD
                 JOIN tt_CustomerEntityId_2 C   ON ( EffectiveFromTimeKey <= v_TimeKey
                 AND EffectiveToTimeKey >= v_TimeKey )
          ...
          -- AND ABD.CustomerEntityId IN (SELECT CustomerEntityId FROM tt_CustomerEntityId_2 GROUP BY CustomerEntityId)
   ...
   ELSE
      ...
      END IF;
   END IF;
   IF utils.object_id('Tempdb..tt_PreCaseCustData_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PreCaseCustData_2 ';
   END IF;
   DELETE FROM tt_PreCaseCustData_2;
   DBMS_OUTPUT.PUT_LINE('1 PRE');
   DBMS_OUTPUT.PUT_LINE(v_CustomerID);
   --SELECT * FROM tt_CustBrData_2
   --SELECT * FROM tt_CustAcData_2
   INSERT INTO tt_PreCaseCustData_2
     ( CustomerEntityId --1
   ...
   , CurrStageMenuId, NxtStageMenuId, CurrStageAuthMode, NxtStageAuthMode, CurrStageNonAllowOp, NxtStageNonAllowOp, AuthorisationStatus )
     ( SELECT CBD.CustomerEntityId ,--1
       ...
       FROM ( SELECT
              ...
              LEFT JOIN tt_CustBrData_2 CBR   ON CBR.CustomerEntityId = CBD.CustomerEntityId
            ...
            --INNER JOIN CustPreCaseDataStage STG
             --		ON CBD.CustomerEntityId=STG.CustomerEntityId
              ...
              LEFT JOIN tt_CustAcData_2 T   ON T.CustomerEntityID = CBD.CustomerEntityID
              LEFT JOIN tt_CustDefData_2 D   ON D.CustomerEntityID = CBD.CustomerEntityID
   ...
   BEGIN
      ...
      FROM T ,tt_PreCaseCustData_2 T
             JOIN ( SELECT A.CustomerEntityId ,
                           A.BranchCode 
                    FROM CurDat_RBL_MISDB_PROD.AdvAcBasicDetail a
                           JOIN tt_PreCaseCustData_2 T   ON A.CustomerEntityId = T.CustomerEntityID
   ...
   IF utils.object_id('Tempdb..tt_TempBrData_7') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TempBrData_7 ';
   END IF;
   DELETE FROM tt_TempBrData_7;
   INSERT INTO tt_TempBrData_7
     ( SELECT BranchCode 
       FROM DimBranch A
        WHERE  ( EffectiveFromTimeKey <= v_TimeKey
                 AND EffectiveToTimeKey >= v_TimeKey )
      ...
      SELECT A.* ,
        ...
        FROM tt_PreCaseCustData_2 A
               LEFT JOIN ( SELECT CustomerEntityId ,
                           ...
                           FROM SysDataUpdationStatus 
                             GROUP BY CustomerEntityId ) t   ON A.CustomerEntityID = t.CustomerEntityId
               LEFT JOIN tt_TempBrData_7 BR   ON BR.BranchCode = A.BranchCode
               LEFT JOIN CurDat_RBL_MISDB_PROD.AdvAcBasicDetail BSC   ON A.CustomerEntityID = BSC.CustomerEntityId
               AND ( BSC.EffectiveFromTimeKey <= v_TimeKey
               AND BSC.EffectiveToTimeKey >= v_TimeKey )
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PRECASECUSTOMERDETAILSELECTAUX_04122023" TO "ADF_CDR_RBL_STGDB";
