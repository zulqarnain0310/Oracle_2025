--------------------------------------------------------
--  DDL for Procedure VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" 
(
      ...
      BEGIN
         ...
         BEGIN
            ...
            UPDATE CalypsoUploadAccMOCPool V
             ...
             WHERE  NVL(V.InvestmentIDDerivativeRefNo, ' ') <> ' '
              AND V.InvestmentIDDerivativeRefNo NOT IN ( SELECT invid 
                                                         ...
                                                         UNION 
                                                         SELECT Refinvid 
                                                         FROM InvestmentFinancialDetail 
                                                          WHERE  EffectiveFromTimeKey <= v_Timekey
                                                                   AND EffectiveToTimeKey >= v_Timekey
                                                         UNION 
                                                         ...
                                                         FROM CurDat_RBL_MISDB_PROD.DerivativeDetail 
                                                          WHERE  EffectiveFromTimeKey <= v_Timekey
                                                                   AND EffectiveToTimeKey >= v_Timekey )
            ;
            IF utils.object_id('TEMPDB..tt_DUB2_19') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB2_19 ';
            END IF;
            DELETE FROM tt_DUB2_19;
            UTILS.IDENTITY_RESET('tt_DUB2_19');

            INSERT INTO tt_DUB2_19 SELECT * 
                 FROM ( SELECT * ,
            ...
            FROM V ,CalypsoUploadAccMOCPool V
                   JOIN tt_DUB2_19 D   ON D.InvestmentIDDerivativeRefNo = V.InvestmentIDDerivativeRefNo ) src
            ...
            UPDATE CalypsoUploadAccMOCPool V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Decimal values not allowed more then 2 digit in column AdditionalProvisionAbsoluteinRs. Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Decimal values not allowed more then 2 digit in column AdditionalProvisionAbsoluteinRs. Please check the values and upload again'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'AdditionalProvisionAbsoluteinRs'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'AdditionalProvisionAbsoluteinRs'
            ...
            UPDATE CalypsoUploadAccMOCPool V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Numeric value not allowed more then 16 digit in column AdditionalProvisionAbsoluteinRs. Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Numeric value not allowed more then 16 digit in column AdditionalProvisionAbsoluteinRs. Please check the values and upload again'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'AdditionalProvisionAbsoluteinRs'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'AdditionalProvisionAbsoluteinRs'
                   ...
                   Srnooferroneousrows = V.SlNo
             WHERE  NVL(AdditionalProvisionAbsolute, ' ') <> ' '
              AND (LENGTH(SUBSTR(NVL(AdditionalProvisionAbsolute, ' '), 0, INSTR(NVL(AdditionalProvisionAbsolute, ' '), '.'))) - 1) > 16;
            ...
            SELECT COUNT(*)  
             ...
             WHERE  B.MOCTypeName IS NULL
                      ...
                      OR ( B.EffectiveFromTimeKey IS NULL
                      AND B.EffectiveToTimeKey IS NULL ) );
            ...
            UPDATE CalypsoUploadAccMOCPool V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Digit between 0 to 9 must be present in column BookValueINRMTMValue. Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Digit between 0 to 9 must be present in column BookValueINRMTMValue. Please check the values and upload again'
                      END,
                   ErrorinColumn = CASE 
            ...
            UPDATE CalypsoUploadAccMOCPool V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Following character ,!@#$%^&*()_-+=/ must not be present in column BookValueINRMTMValue.Not allow special character in BookValueINRMTMValue column'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Following character ,!@#$%^&*()_-+=/ must not be present in column BookValueINRMTMValue. Please check the values and upload again'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'BookValueINRMTMValue'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'BookValueINRMTMValue'
            ...
            UPDATE CalypsoUploadAccMOCPool V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Decimal values not allowed more then 2 digit in column BookValueINRMTMValue’. Kindly check and upload value'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Decimal values not allowed more then 2 digit in column ‘BookValueINRMTMValue’. Kindly check and upload value'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'BookValueINRMTMValue'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'BookValueINRMTMValue'
            ...
            UPDATE CalypsoUploadAccMOCPool V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Digit between 0 to 9 must be present in column UnservicedInterest. Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Digit between 0 to 9 must be present in column UnservicedInterest. Please check the values and upload again'
                      END,
                   ErrorinColumn = CASE 
            ...
            UPDATE CalypsoUploadAccMOCPool V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Invalid UnservicedInterest. Not allow special character in UnservicedInterest column'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Invalid UnservicedInterest. Not allow special character in UnservicedInterest column'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'UnservicedInterest'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'UnservicedInterest'
            ...
            UPDATE CalypsoUploadAccMOCPool V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Decimal values not allowed more then 2 digit in column ‘UnservicedInterest’. Kindly check and upload value'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Decimal values not allowed more then 2 digit in column ‘UnservicedInterest’. Kindly check and upload value'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'UnservicedInterest'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'UnservicedInterest'
            ...
            UPDATE CalypsoUploadAccMOCPool V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'One values is mandatory out of UnservicedInterest , BookValueINRMTMValue ,AdditionalProvisionAbsolute'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'One values is mandatory out of UnservicedInterest , BookValueINRMTMValue ,AdditionalProvisionAbsolute'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'UnservicedInterest , BookValueINRMTMValue ,AdditionalProvisionAbsolute'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'UnservicedInterest , BookValueINRMTMValue ,AdditionalProvisionAbsolute'
                      END,
                   Srnooferroneousrows = V.SlNo
                   --        ----STUFF((SELECT ','+SlNo       
                    --        ----FROM CalypsoUploadCustMocUpload A      
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_CALYPSOACCNPAMOCUPLOAD_04122023" TO "ADF_CDR_RBL_STGDB";
