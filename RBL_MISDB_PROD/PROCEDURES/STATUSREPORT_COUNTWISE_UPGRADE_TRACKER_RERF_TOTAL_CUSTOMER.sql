--------------------------------------------------------
--  DDL for Procedure STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" 
AS
...
BEGIN
   ...
   IF utils.object_id('tempdb..tt_temp1_36') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_temp1_36 ';
   END IF;
   DELETE FROM tt_temp1_36;
   --------------Finacle      

   --INSERT INTO tt_temp1_36      
   ...
   -- group by a.SourceAlt_Key,a.SourceSystemName      
    /*  
       IF OBJECT_ID('TempDB..tt_Finacle_69') Is Not Null      
      Drop Table tt_Finacle_69      
      ...
      Y.EffectiveFromTimeKey<=@TimeKey and Y.EffectiveToTimeKey>=@TimeKey)      
       ...
       Delete        a  
      from          tt_Finacle_69 a  
      inner join    ReverseFeedData b  
      on            a.CustomerAcID=b.AccountID  
      where         DateofData=@Date  
      ---------------------      
      */
   IF utils.object_id('TempDB..tt_Finacle_69') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Finacle_69 ';
   END IF;
   DELETE FROM tt_Finacle_69;
   UTILS.IDENTITY_RESET('tt_Finacle_69');

   INSERT INTO tt_Finacle_69 ( 
   ...
   -- group by a.SourceAlt_Key,a.SourceSystemName      
   /*     
    IF OBJECT_ID('TempDB..tt_Ganaseva_59') Is Not Null      
      Drop Table tt_Ganaseva_59      
      ...
      */
   ----------------      
   IF utils.object_id('TempDB..tt_Ganaseva_59') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Ganaseva_59 ';
   END IF;
   DELETE FROM tt_Ganaseva_59;
   UTILS.IDENTITY_RESET('tt_Ganaseva_59');

   INSERT INTO tt_Ganaseva_59 ( 
   	SELECT A.RefCustomerID CustomerID  ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
           A.SysNPA_Dt FinalNpaDt  ,
           Z.UpgDate ,
   ...
   -- group by a.SourceAlt_Key,a.SourceSystemName      
   /*   
    IF OBJECT_ID('TempDB..tt_VisionPlus_30') Is Not Null        
   Drop Table tt_VisionPlus_30        

   Select A.CustomerAcID,A.FinalAssetClassAlt_Key,A.FinalNpaDt,B.SourceAssetClass,B.SourceNpaDate,        
   DA.AssetClassAlt_Key BankAssetClass ,ds.SourceAlt_Key,ds.SourceName      
     INto tt_VisionPlus_30        
    from Pro.ACCOUNTCAL A        
   Inner Join dbo.AdvAcBalanceDetail B ON A.AccountEntityID=B.AccountEntityID        
   ANd B.EffectiveFromTimeKey<=@Timekey ANd B.EffectiveToTimeKey>=@Timekey        
   Inner Join DIMSOURCEDB DS ON DS.SourceAlt_Key=A.SourceAlt_Key        
   ...
   ANd Not exists (Select 1 from ReverseFeedDataInsertSync R where A.CustomerAcID=R.CustomerAcID      
      ...
      Y.EffectiveFromTimeKey<=@TimeKey and Y.EffectiveToTimeKey>=@TimeKey)    
         ...
         Delete        a  
      from          tt_VisionPlus_30 a  
      inner join    ReverseFeedData b  
      on            a.CustomerAcID=b.AccountID  
      where         DateofData=@Date  
      */
   IF utils.object_id('TempDB..tt_VisionPlus_30') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_VisionPlus_30 ';
   END IF;
   DELETE FROM tt_VisionPlus_30;
   UTILS.IDENTITY_RESET('tt_VisionPlus_30');

   INSERT INTO tt_VisionPlus_30 ( 
   	SELECT A.RefCustomerID CustomerID  ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
           A.SysNPA_Dt FinalNpaDt  ,
           (CASE 
   ...
   -- group by a.SourceAlt_Key,a.SourceSystemName      
   /*      
    IF OBJECT_ID('TempDB..tt_MiFin_69') Is Not Null      
      Drop Table tt_MiFin_69      
      ...
      */
   ----------------     
   IF utils.object_id('TempDB..tt_MiFin_69') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MiFin_69 ';
   END IF;
   DELETE FROM tt_MiFin_69;
   UTILS.IDENTITY_RESET('tt_MiFin_69');

   INSERT INTO tt_MiFin_69 ( 
   	SELECT A.RefCustomerID CustomerID  ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
           A.SysNPA_Dt FinalNpaDt  ,
           Z.UpgDate ,
   ...
   --       group by a.SourceAlt_Key,a.SourceSystemName      
   /*   
     IF OBJECT_ID('TempDB..tt_Indus_69') Is Not Null      
      Drop Table tt_Indus_69      
    ...
    */
   ----------------   
   IF utils.object_id('TempDB..tt_Indus_69') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Indus_69 ';
   END IF;
   DELETE FROM tt_Indus_69;
   UTILS.IDENTITY_RESET('tt_Indus_69');

   INSERT INTO tt_Indus_69 ( 
   	SELECT A.RefCustomerID CustomerID  ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
           A.SysNPA_Dt FinalNpaDt  ,
           Z.UpgDate ,
   ...
   --)T      
   /*      
   IF OBJECT_ID('TempDB..tt_ECBF_71') Is Not Null      
     Drop Table tt_ECBF_71      
      ...
      where         DateofData=@Date  
     */
   IF utils.object_id('TempDB..tt_ACCOUNTCAL_DPD_27') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ACCOUNTCAL_DPD_27 ';
   END IF;
   DELETE FROM tt_ACCOUNTCAL_DPD_27;
   UTILS.IDENTITY_RESET('tt_ACCOUNTCAL_DPD_27');

   INSERT INTO tt_ACCOUNTCAL_DPD_27 ( 
   	SELECT CustomerEntityID ,
           MAX(DPD_Max)  DPD_Max  
   	  FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL 
   	  GROUP BY CustomerEntityID );
   IF utils.object_id('TempDB..tt_ECBF_71') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ECBF_71 ';
   END IF;
   DELETE FROM tt_ECBF_71;
   UTILS.IDENTITY_RESET('tt_ECBF_71');

   INSERT INTO tt_ECBF_71 ( 
   	SELECT CustomerID ,
   	  ...
   	  FROM ( SELECT A.RefCustomerID CustomerID  ,
             ...
             FROM PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                    ...
                    AND DA.EffectiveFromTimeKey <= v_Timekey
                    AND DA.EffectiveToTimeKey >= v_Timekey
                    LEFT JOIN tt_ACCOUNTCAL_DPD_27 AD   ON AD.CustomerEntityID = A.CustomerEntityID
              WHERE  DS.SourceName = 'ECBF'
                       AND NOT EXISTS ( SELECT 1 
                                        FROM ReverseFeedDataInsertSync_Customer R
                                         WHERE  A.RefCustomerID = R.CustomerID
   ...
   --  group by a.SourceAlt_Key,a.SourceSystemName      
   /*    
     IF OBJECT_ID('TempDB..tt_MetaGrid_71') Is Not Null      
      Drop Table tt_MetaGrid_71      
    ...
    */
   ----------------   
   IF utils.object_id('TempDB..tt_MetaGrid_71') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MetaGrid_71 ';
   END IF;
   DELETE FROM tt_MetaGrid_71;
   UTILS.IDENTITY_RESET('tt_MetaGrid_71');

   INSERT INTO tt_MetaGrid_71 ( 
   ...
   --select * from tt_temp1_36      
   --select *       
   MERGE INTO A 
   USING (SELECT A.ROWID row_id, b.CNT
   FROM A ,StatusReport_RERF_Customer a
          JOIN tt_temp1_36 b   ON a.SourceAlt_Key = b.SourceAlt_Key ) src
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER" TO "ADF_CDR_RBL_STGDB";
