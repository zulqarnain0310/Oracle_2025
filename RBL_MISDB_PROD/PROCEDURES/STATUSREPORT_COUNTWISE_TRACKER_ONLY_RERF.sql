--------------------------------------------------------
--  DDL for Procedure STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" 
AS
...
BEGIN
   ...
   --------------Finacle            
   IF utils.object_id('tempdb..tt_temp1_26') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_temp1_26 ';
   END IF;
   DELETE FROM tt_temp1_26;
   DBMS_OUTPUT.PUT_LINE('Finacle ');
   IF utils.object_id('TempDB..tt_Finacle_66') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Finacle_66 ';
   END IF;
   DELETE FROM tt_Finacle_66;
   UTILS.IDENTITY_RESET('tt_Finacle_66');

   INSERT INTO tt_Finacle_66 ( 
   ...
   --------------Ganaseva---------------------------------------------------------------------------------------------------------------        
   IF utils.object_id('TempDB..tt_Ganaseva_56') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Ganaseva_56 ';
   END IF;
   DELETE FROM tt_Ganaseva_56;
   UTILS.IDENTITY_RESET('tt_Ganaseva_56');

   INSERT INTO tt_Ganaseva_56 ( 
   ...
   INSERT INTO tt_temp1_26
     ( SELECT SourceAlt_Key ,
       ...
       FROM ( SELECT * 
         ...
         GROUP BY SourceAlt_Key,SourceName );
   --------------ECBF  ----------------------------------------------------------------------------------------------------
   IF utils.object_id('TempDB..tt_ACCOUNTCAL_DPD_24') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ACCOUNTCAL_DPD_24 ';
   END IF;
   DELETE FROM tt_ACCOUNTCAL_DPD_24;
   UTILS.IDENTITY_RESET('tt_ACCOUNTCAL_DPD_24');

   INSERT INTO tt_ACCOUNTCAL_DPD_24 ( 
   	SELECT CustomerEntityID ,
           MAX(DPD_Max)  DPD_Max  
   	  FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL 
   	  GROUP BY CustomerEntityID );
   IF utils.object_id('TempDB..tt_ECBF_68') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ECBF_68 ';
   END IF;
   DELETE FROM tt_ECBF_68;
   UTILS.IDENTITY_RESET('tt_ECBF_68');

   INSERT INTO tt_ECBF_68 ( 
   	SELECT RefCustomerID ,
   	  ...
   	  FROM ( SELECT A.RefCustomerID ,
             ...
             FROM PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                    ...
                    AND Z.EffectiveFromTimeKey <= v_Timekey
                    AND Z.EffectiveToTimeKey >= v_Timekey
                    LEFT JOIN tt_ACCOUNTCAL_DPD_24 AD   ON AD.CustomerEntityID = Z.CustomerEntityID
   ...
   INSERT INTO tt_temp1_26
     ( SELECT SourceAlt_Key ,
       ...
       FROM ( SELECT * 
         ...
         GROUP BY SourceAlt_Key,SourceName );
   --------------Indus  -----------------------------------------------------------------------------------------------------------------
   IF utils.object_id('TempDB..tt_Indus_66') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Indus_66 ';
   END IF;
   DELETE FROM tt_Indus_66;
   UTILS.IDENTITY_RESET('tt_Indus_66');

   INSERT INTO tt_Indus_66 ( 
   	SELECT A.RefCustomerID ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
           A.SysNPA_Dt FinalNpaDt  ,
           B.SourceAssetClass ,
   ...
   INSERT INTO tt_temp1_26
     ( SELECT SourceAlt_Key ,
       ...
       FROM ( SELECT * 
         ...
         GROUP BY SourceAlt_Key,SourceName );
   --------------MiFin -------------------------------------------------------------------------------------------------
   IF utils.object_id('TempDB..tt_MiFin_66') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MiFin_66 ';
   END IF;
   DELETE FROM tt_MiFin_66;
   UTILS.IDENTITY_RESET('tt_MiFin_66');

   INSERT INTO tt_MiFin_66 ( 
   	SELECT A.RefCustomerID ,
           A.SysAssetClassAlt_Key ,
           A.SysNPA_Dt ,
           B.SourceAssetClass ,
   ...
   INSERT INTO tt_temp1_26
     ( SELECT SourceAlt_Key ,
       ...
       FROM ( SELECT * 
         ...
         GROUP BY SourceAlt_Key,SourceName );
   --------------VisionPlus ---------------------------------------------------------------------------------------------------------
   IF utils.object_id('TempDB..tt_CorporateCustID_7') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CorporateCustID_7 ';
   END IF;
   DELETE FROM tt_CorporateCustID_7;
   UTILS.IDENTITY_RESET('tt_CorporateCustID_7');

   INSERT INTO tt_CorporateCustID_7 ( 
   ...
   IF utils.object_id('TempDB..#VisionPlus') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_VisionPlus_27 ';
   END IF;
   DELETE FROM tt_VisionPlus1_7;
   UTILS.IDENTITY_RESET('tt_VisionPlus1_7');

   INSERT INTO tt_VisionPlus1_7 ( 
   	SELECT A.RefCustomerID CustomerID  ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
           A.SysNPA_Dt FinalNpaDt  ,
           (CASE 
   	  ...
   	  FROM PRO_RBL_MISDB_PROD.CUSTOMERCAL A
             ...
             LEFT JOIN tt_CorporateCustID_7 CC   ON a.RefCustomerID = CC.RefCustomerID
   ...
   DELETE A
    WHERE ROWID IN 
   ( SELECT A.ROWID
     FROM tt_VisionPlus1_7 a
            JOIN ReverseFeedData b   ON A.CustomerID = b.CustomerID,
          A
    WHERE  b.DateofData = v_Date );
   IF utils.object_id('TempDB..tt_AssetClass_7') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AssetClass_7 ';
   END IF;
   DELETE FROM tt_AssetClass_7;
   UTILS.IDENTITY_RESET('tt_AssetClass_7');

   INSERT INTO tt_AssetClass_7 ( 
     ...
     ( SELECT SourceAlt_Key ,
       ...
       FROM ( SELECT * 
              FROM ( SELECT CorporateCustomerID CustomerID  ,
                            ...
                            SourceName 
                     FROM tt_VisionPlus1_7 A
                            JOIN tt_AssetClass_7 b   ON a.FinalAssetClassAlt_Key = b.AssetClassAlt_key
                      WHERE  A.BankAssetClass = 1
                               AND A.FinalAssetClassAlt_Key > 1
                     UNION 
         ...
         GROUP BY SourceAlt_Key,SourceName );
   --------------MetaGrid----------------------------------------------------------------------------------------------------------------
   IF utils.object_id('TempDB..tt_MetaGrid_68') IS NOT NULL THEN
   ...
   EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MetaGrid_68 ';
   END IF;
   DELETE FROM tt_MetaGrid_68;
   UTILS.IDENTITY_RESET('tt_MetaGrid_68');

   INSERT INTO tt_MetaGrid_68 ( 
   	SELECT A.RefCustomerID ,
           A.RefCustomerID CustomerId  ,
           A.UCIF_ID ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
   ...
   --select * from tt_temp1_26            
   MERGE INTO A 
   USING (SELECT A.ROWID row_id, b.CNT
   FROM A ,StatusReport_Only_RERF a
          JOIN tt_temp1_26 b   ON a.SourceAlt_Key = b.SourceAlt_Key ) src
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_TRACKER_ONLY_RERF" TO "ADF_CDR_RBL_STGDB";
