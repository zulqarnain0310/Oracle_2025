--------------------------------------------------------
--  DDL for Procedure SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" 
(
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF utils.object_id('TempDB..tt_Finacle_9') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Finacle_9 ';
         END IF;
         DELETE FROM tt_Finacle_9;
         UTILS.IDENTITY_RESET('tt_Finacle_9');

         INSERT INTO tt_Finacle_9 ( 
            ...
            SELECT * 
              FROM ( SELECT 'FinacleAssetClassification' TableName  ,
                     ...
                     SELECT 'FinacleAssetClassification' TableName  ,
                            A.CustomerID || '|' || A.UCIF_ID || '|' || E.AssetClassShortNameEnum || '|' || E.AssetClassName || '|' || v_Date || '|' || NVL(UTILS.CONVERT_TO_VARCHAR2(NVL(A.UpgradeDate, v_date),10,p_style=>105), ' ') DataUtility  
                     FROM ReverseFeedData A
                            JOIN DIMSOURCEDB B   ON A.SourceAlt_Key = B.SourceAlt_Key
                            AND B.EffectiveFromTimeKey <= v_TimeKey
                            AND B.EffectiveToTimeKey >= v_TimeKey
                     ...
                     UNION 
                     SELECT 'FinacleAssetClassification' TableName  ,
                            A.RefCustomerID || '|' || A.UCIF_ID || '|' || E.AssetClassShortNameEnum || '|' || E.AssetClassName || '|' || v_Date || '|' || NVL(UTILS.CONVERT_TO_VARCHAR2(A.SysNPA_Dt,10,p_style=>105), ' ') DataUtility  
      ...
      BEGIN
         IF utils.object_id('TempDB..tt_Ganaseva_5') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Ganaseva_5 ';
         END IF;
         DELETE FROM tt_Ganaseva_5;
         UTILS.IDENTITY_RESET('tt_Ganaseva_5');

         INSERT INTO tt_Ganaseva_5 ( 
      ...
      BEGIN
         --------------ECBF
         --Select * from (
         IF utils.object_id('TempDB..tt_ACCOUNTCAL_DPD_9') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ACCOUNTCAL_DPD_9 ';
         END IF;
         DELETE FROM tt_ACCOUNTCAL_DPD_9;
         UTILS.IDENTITY_RESET('tt_ACCOUNTCAL_DPD_9');

         INSERT INTO tt_ACCOUNTCAL_DPD_9 ( 
         	SELECT CustomerEntityID ,
                 MAX(DPD_Max)  DPD_Max  
         	  FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         	  GROUP BY CustomerEntityID );
         IF utils.object_id('TempDB..tt_ECBF_11') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ECBF_11 ';
         END IF;
         DELETE FROM tt_ECBF_11;
         UTILS.IDENTITY_RESET('tt_ECBF_11');

         INSERT INTO tt_ECBF_11 ( 
         	SELECT RefCustomerID ,
         	  ...
         	  FROM ( SELECT A.RefCustomerID ,
                   ...
                   FROM PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                          ...
                          AND Z.EffectiveFromTimeKey <= v_Timekey
                          AND Z.EffectiveToTimeKey >= v_Timekey
                          LEFT JOIN tt_ACCOUNTCAL_DPD_9 AD   ON AD.CustomerEntityID = Z.CustomerEntityID
         ...
         OPEN  v_cursor FOR
            SELECT * 
             ...
             WHERE  B.SourceName = 'ECBF'
                      ...
                      AND A.EffectiveFromTimeKey <= v_TimeKey
                      AND A.EffectiveToTimeKey >= v_TimeKey
              GROUP BY A.ProductName,A.CustomerName,A.CustomerID,E.AssetClassGroup,E.SrcSysClassCode,A.DPD,UTILS.CONVERT_TO_VARCHAR2(A.NPADate,10,p_style=>105),UTILS.CONVERT_TO_VARCHAR2(A.DateofData,10,p_style=>105),CustomerID,UCIF_ID,SrcSysClassName,DateofData ) A ) T
                     UNION 
                            ...
                            FROM ( SELECT D.ProductName ProductType  ,
                                   ...
                                   FROM PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                                    ...
                                    WHERE  B.SourceName = 'ECBF'
                                             ...
                                             AND A.EffectiveFromTimeKey <= v_TimeKey
                                             AND A.EffectiveToTimeKey >= v_TimeKey
                                     GROUP BY D.ProductName,a.CustomerName,A.RefCustomerID,E.AssetClassGroup,E.SrcSysClassCode,c.DPD_Max,UTILS.CONVERT_TO_VARCHAR2(A.SysNPA_Dt,10,p_style=>105),a.RefCustomerID,a.UCIF_ID,E.SrcSysClassName --,Convert(Varchar(10),@Date,105)
                                  ) A ) T
                     UNION 
                     ...
                     FROM ( SELECT ROW_NUMBER() OVER ( ORDER BY ClientCustId  ) SrNo  ,
                            ...
                            FROM ( SELECT A.ProductName ProductType  ,
                                   ...
                                   WHERE  B.SourceName = 'ECBF'
                                            ...
                                            AND A.EffectiveFromTimeKey <= v_TimeKey
                                            AND A.EffectiveToTimeKey >= v_TimeKey
                                     GROUP BY A.ProductName,A.CustomerName,A.CustomerID,E.AssetClassGroup,E.AssetClassShortName,A.DPD,UTILS.CONVERT_TO_VARCHAR2(A.UpgradeDate,10,p_style=>105),UTILS.CONVERT_TO_VARCHAR2(A.DateofData,10,p_style=>105),UCIF_ID,SrcSysClassName ) A ) T
                     UNION 
      ...
      BEGIN
         ...
         IF utils.object_id('TempDB..tt_Indus_9') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Indus_9 ';
         END IF;
         DELETE FROM tt_Indus_9;
         UTILS.IDENTITY_RESET('tt_Indus_9');

         INSERT INTO tt_Indus_9 ( 
         	SELECT A.RefCustomerID ,
                 A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
                 A.SysNPA_Dt FinalNpaDt  ,
                 B.SourceAssetClass ,
      ...
      BEGIN
         ...
         IF utils.object_id('TempDB..tt_MiFin_9') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MiFin_9 ';
         END IF;
         DELETE FROM tt_MiFin_9;
         UTILS.IDENTITY_RESET('tt_MiFin_9');

         INSERT INTO tt_MiFin_9 ( 
         	SELECT A.RefCustomerID ,
                 A.SysAssetClassAlt_Key ,
                 A.SysNPA_Dt ,
                 B.SourceAssetClass ,
      ...
      BEGIN
         ...
         -- --And A.AssetSubClass<>'STD'
         -- AND A.EffectiveFromTimekey<=@TimeKey AND A.EffectiveToTimeKey>=@TimeKey
         IF utils.object_id('TempDB..tt_MetaGrid_11') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MetaGrid_11 ';
         END IF;
         DELETE FROM tt_MetaGrid_11;
         UTILS.IDENTITY_RESET('tt_MetaGrid_11');

         INSERT INTO tt_MetaGrid_11 ( 
         	SELECT A.RefCustomerID ,
      ...
      BEGIN
         IF utils.object_id('TempDB..tt_ACCOUNTCAL_TAF_9') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ACCOUNTCAL_TAF_9 ';
         END IF;
         DELETE FROM tt_ACCOUNTCAL_TAF_9;
         UTILS.IDENTITY_RESET('tt_ACCOUNTCAL_TAF_9');

         INSERT INTO tt_ACCOUNTCAL_TAF_9 ( 
         	SELECT CustomerEntityID ,
                 MAX(DPD_Max)  DPD_Max  
         	  FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         	  GROUP BY CustomerEntityID );
         IF utils.object_id('TempDB..tt_EIFS_9') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_EIFS_9 ';
         END IF;
         DELETE FROM tt_EIFS_9;
         UTILS.IDENTITY_RESET('tt_EIFS_9');

         INSERT INTO tt_EIFS_9 ( 
         	SELECT RefCustomerID ,
                 FinalAssetClassAlt_Key ,
                 FinalNpaDt ,
                 SourceAssetClass ,
         	  ...
         	  FROM ( SELECT A.RefCustomerID ,
                   ...
                   FROM PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                          ...
                          AND Z.EffectiveFromTimeKey <= v_Timekey
                          AND Z.EffectiveToTimeKey >= v_Timekey
                          LEFT JOIN tt_ACCOUNTCAL_TAF_9 AD   ON AD.CustomerEntityID = Z.CustomerEntityID
                          JOIN DimProduct DP   ON DP.ProductAlt_Key = Z.ProductAlt_Key
                          AND DP.EffectiveFromTimeKey <= v_Timekey
                          AND DP.EffectiveToTimeKey >= v_Timekey
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTASSETCLASSIFICATIONSOURCETYPEDATAFORUTILITY_24012023" TO "ADF_CDR_RBL_STGDB";
