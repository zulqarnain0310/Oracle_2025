--------------------------------------------------------
--  DDL for Procedure RPT_032_13102022
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."RPT_032_13102022" 
(
...
BEGIN
   ...
   -----------------================================================
   IF ( utils.object_id('TEMPDB..tt_DimAcBuSegment_25') IS NOT NULL ) THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DimAcBuSegment_25 ';
   END IF;
   DELETE FROM tt_DimAcBuSegment_25;
   UTILS.IDENTITY_RESET('tt_DimAcBuSegment_25');

   INSERT INTO tt_DimAcBuSegment_25 SELECT DENSE_RANK() OVER ( PARTITION BY AcBuRevisedSegmentCode ORDER BY AcBuSegmentCode  ) RN  ,
                                           AcBuSegmentCode ,
   ...
   ---------------------==========================================
   IF ( utils.object_id('TEMPDB..tt_DimAcBuSegment_251') IS NOT NULL ) THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DimAcBuSegment1_4 ';
   END IF;
   DELETE FROM tt_DimAcBuSegment1_4;
   UTILS.IDENTITY_RESET('tt_DimAcBuSegment1_4');

   INSERT INTO tt_DimAcBuSegment1_4 SELECT DENSE_RANK() OVER ( PARTITION BY AcBuRevisedSegmentCode ORDER BY AcBuSegmentCode  ) RN  ,
                                           AcBuSegmentCode ,
                                           AcBuRevisedSegmentCode ,
                                           AcBuSegmentDescription 
        FROM DimAcBuSegment 
   ...
   IF utils.object_id('tempdb..tt_DPD_23') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DPD_23 ';
   END IF;
   DELETE FROM tt_DPD_23;
   UTILS.IDENTITY_RESET('tt_DPD_23');

   INSERT INTO tt_DPD_23 ( 
   	SELECT * 
   ...
   EXECUTE IMMEDIATE ' ALTER TABLE tt_DPD_23 
   ...
   /* CALCULATE MAX DPD */
   IF utils.object_id('TEMPDB..tt_TEMPTABLE_122') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_122 ';
   END IF;
   DELETE FROM tt_TEMPTABLE_122;
   UTILS.IDENTITY_RESET('tt_TEMPTABLE_122');

   INSERT INTO tt_TEMPTABLE_122 ( 
   ...
   IF utils.object_id('tempdb..tt_DPD_231') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DPD1_4 ';
   END IF;
   DELETE FROM tt_DPD1_4;
   UTILS.IDENTITY_RESET('tt_DPD1_4');

   INSERT INTO tt_DPD1_4 ( 
   	SELECT * 
   	  FROM PRO_RBL_MISDB_PROD.AccountCal_Hist 
   	 WHERE  EffectiveFromTimeKey <= v_ToTimeKey
              AND EffectiveToTimeKey >= v_ToTimeKey
   ...
   EXECUTE IMMEDIATE ' ALTER TABLE tt_DPD1_4 
      ADD ( [DPD_IntService NUMBER(10,0) , DPD_NoCredit NUMBER(10,0) , DPD_Overdrawn NUMBER(10,0) , DPD_Overdue NUMBER(10,0) , DPD_Renewal NUMBER(10,0) , DPD_StockStmt NUMBER(10,0) , DPD_MAX NUMBER(10,0) ] ) ';
   /*---------- CALCULATED ALL DPD---------------------------------------------------------*/
   MERGE INTO A 
   USING (SELECT A.ROWID row_id, (CASE 
   ...
   WHEN MATCHED THEN UPDATE SET A.DPD_NoCredit = 0;
   /* CALCULATE MAX DPD */
   IF utils.object_id('TEMPDB..tt_TEMPTABLE_1221') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE1_6 ';
   END IF;
   DELETE FROM tt_TEMPTABLE1_6;
   UTILS.IDENTITY_RESET('tt_TEMPTABLE1_6');

   INSERT INTO tt_TEMPTABLE1_6 ( 
   	SELECT A.CustomerAcID ,
           CASE 
                WHEN NVL(A.DPD_IntService, 0) >= NVL(A.RefPeriodIntService, 0) THEN A.DPD_IntService
           ELSE 0
   	...
   	SELECT UCIF_ID ,
   	  ...
   	  FROM ( SELECT Cust.UCIF_ID ,
             ...
             FROM PRO_RBL_MISDB_PROD.CustomerCal_Hist CUST
                    ...
                    JOIN tt_DPD_23 DPD   ON ACCOUNT.CustomerAcID = DPD.CustomerAcID
                    ...
                    LEFT JOIN tt_DimAcBuSegment_25 DS   ON ACCOUNT.ActSegmentCode = DS.AcBuRevisedSegmentCode
                    AND DS.RN = 1
             ...
             FROM PRO_RBL_MISDB_PROD.CustomerCal_Hist CUST
                    ...
                    JOIN tt_DPD1_4 DPD   ON ACCOUNT.CustomerAcID = DPD.CustomerAcID
                    AND ACCOUNT.RefCustomerID = DPD.RefCustomerID
                    AND ACCOUNT.BranchCode = DPD.BranchCode
                    LEFT JOIN AdvAcBasicDetail ACBD   ON ACCOUNT.CustomerAcID = ACBD.CustomerAcID
                    ...
                    LEFT JOIN tt_DimAcBuSegment1_4 DS   ON ACCOUNT.ActSegmentCode = DS.AcBuRevisedSegmentCode
                    AND DS.RN = 1
              WHERE  DB.BranchCode IN ( SELECT * 
                                        FROM TABLE(SPLIT(v_BranchCode, ','))  )
      ...
      SELECT PivotTable.UCIF_ID,
        ...
        FROM ( 
           SELECT D.UCIF_ID ,
                  , 
                   aggregate
             FROM tt_DATA_12 ( SELECT PivotTable.CustomerACID,
               MAX( DECODE( , , PivotTable.aggregate, NULL) ) "Pivot Column"                                
        FROM ( 
           SELECT CustomerACID ,
                  , 
       ...
       GROUP BY PivotTable.CustomerACID ) Pvttt_DATA_12 ( SELECT PivotTable.CustomerACID,
               MAX( DECODE( , , PivotTable.aggregate, NULL) ) "Pivot Column"                                
        FROM ( 
           SELECT CustomerACID ,
                  , 
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_032_13102022" TO "ADF_CDR_RBL_STGDB";
