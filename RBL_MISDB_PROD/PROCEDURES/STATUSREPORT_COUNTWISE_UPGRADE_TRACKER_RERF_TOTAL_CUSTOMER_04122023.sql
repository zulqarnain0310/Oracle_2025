--------------------------------------------------------
--  DDL for Procedure STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" 
AS
...
BEGIN
   ...
   IF utils.object_id('tempdb..tt_temp1_71') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_temp1_71 ';
   END IF;
   DELETE FROM tt_temp1_71;
   --------------Finacle      

   --INSERT INTO tt_temp1_71      
   ...
   -- group by a.SourceAlt_Key,a.SourceSystemName      
    /*  
       IF OBJECT_ID('TempDB..tt_Finacle_76') Is Not Null      
      Drop Table tt_Finacle_76      
      ...
      Y.EffectiveFromTimeKey<=@TimeKey and Y.EffectiveToTimeKey>=@TimeKey)      
       ...
       Delete        a  
      from          tt_Finacle_76 a  
      inner join    ReverseFeedData b  
      on            a.CustomerAcID=b.AccountID  
      where         DateofData=@Date  
      ---------------------      
      */
   IF utils.object_id('TempDB..tt_Finacle_76') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Finacle_76 ';
   END IF;
   DELETE FROM tt_Finacle_76;
   UTILS.IDENTITY_RESET('tt_Finacle_76');

   INSERT INTO tt_Finacle_76 ( 
   ...
   -- group by a.SourceAlt_Key,a.SourceSystemName      
   /*     
    IF OBJECT_ID('TempDB..tt_Ganaseva_68') Is Not Null      
      Drop Table tt_Ganaseva_68      
      ...
      */
   ----------------      
   IF utils.object_id('TempDB..tt_Ganaseva_68') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Ganaseva_68 ';
   END IF;
   DELETE FROM tt_Ganaseva_68;
   UTILS.IDENTITY_RESET('tt_Ganaseva_68');

   INSERT INTO tt_Ganaseva_68 ( 
   	SELECT A.RefCustomerID CustomerID  ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
           A.SysNPA_Dt FinalNpaDt  ,
           Z.UpgDate ,
   ...
   -- group by a.SourceAlt_Key,a.SourceSystemName      
   /*   
    IF OBJECT_ID('TempDB..tt_VisionPlus_38') Is Not Null        
   Drop Table tt_VisionPlus_38        

   Select A.CustomerAcID,A.FinalAssetClassAlt_Key,A.FinalNpaDt,B.SourceAssetClass,B.SourceNpaDate,        
   DA.AssetClassAlt_Key BankAssetClass ,ds.SourceAlt_Key,ds.SourceName      
     INto tt_VisionPlus_38        
    from Pro.ACCOUNTCAL A        
   Inner Join dbo.AdvAcBalanceDetail B ON A.AccountEntityID=B.AccountEntityID        
   ANd B.EffectiveFromTimeKey<=@Timekey ANd B.EffectiveToTimeKey>=@Timekey        
   Inner Join DIMSOURCEDB DS ON DS.SourceAlt_Key=A.SourceAlt_Key        
   ...
   ANd Not exists (Select 1 from ReverseFeedDataInsertSync R where A.CustomerAcID=R.CustomerAcID      
      ...
      Y.EffectiveFromTimeKey<=@TimeKey and Y.EffectiveToTimeKey>=@TimeKey)    
         ...
         Delete        a  
      from          tt_VisionPlus_38 a  
      inner join    ReverseFeedData b  
      on            a.CustomerAcID=b.AccountID  
      where         DateofData=@Date  
      */
   IF utils.object_id('TempDB..tt_VisionPlus_38') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_VisionPlus_38 ';
   END IF;
   DELETE FROM tt_VisionPlus_38;
   UTILS.IDENTITY_RESET('tt_VisionPlus_38');

   INSERT INTO tt_VisionPlus_38 ( 
   	SELECT A.RefCustomerID CustomerID  ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
           A.SysNPA_Dt FinalNpaDt  ,
           (CASE 
   ...
   -- group by a.SourceAlt_Key,a.SourceSystemName      
   /*      
    IF OBJECT_ID('TempDB..tt_MiFin_76') Is Not Null      
      Drop Table tt_MiFin_76      
      ...
      */
   ----------------     
   IF utils.object_id('TempDB..tt_MiFin_76') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MiFin_76 ';
   END IF;
   DELETE FROM tt_MiFin_76;
   UTILS.IDENTITY_RESET('tt_MiFin_76');

   INSERT INTO tt_MiFin_76 ( 
   	SELECT A.RefCustomerID CustomerID  ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
           A.SysNPA_Dt FinalNpaDt  ,
           Z.UpgDate ,
   ...
   --       group by a.SourceAlt_Key,a.SourceSystemName      
   /*   
     IF OBJECT_ID('TempDB..tt_Indus_76') Is Not Null      
      Drop Table tt_Indus_76      
    ...
    */
   ----------------   
   IF utils.object_id('TempDB..tt_Indus_76') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Indus_76 ';
   END IF;
   DELETE FROM tt_Indus_76;
   UTILS.IDENTITY_RESET('tt_Indus_76');

   INSERT INTO tt_Indus_76 ( 
   	SELECT A.RefCustomerID CustomerID  ,
           A.SysAssetClassAlt_Key FinalAssetClassAlt_Key  ,
           A.SysNPA_Dt FinalNpaDt  ,
           Z.UpgDate ,
   ...
   INSERT INTO tt_temp1_71
     ( SELECT SourceAlt_Key ,
              SourceName ,
              COUNT(*)  CNT  
       FROM ( SELECT CustomerID Loan_Account_Number  ,
   ...
   --)T      
   /*      
   IF OBJECT_ID('TempDB..tt_ECBF_78') Is Not Null      
     Drop Table tt_ECBF_78      
      ...
      where         DateofData=@Date  
     */
   IF utils.object_id('TempDB..tt_ACCOUNTCAL_DPD_31') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ACCOUNTCAL_DPD_31 ';
   END IF;
   DELETE FROM tt_ACCOUNTCAL_DPD_31;
   UTILS.IDENTITY_RESET('tt_ACCOUNTCAL_DPD_31');

   INSERT INTO tt_ACCOUNTCAL_DPD_31 ( 
   	SELECT CustomerEntityID ,
           MAX(DPD_Max)  DPD_Max  
   	  FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL 
   	  GROUP BY CustomerEntityID );
   IF utils.object_id('TempDB..tt_ECBF_78') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ECBF_78 ';
   END IF;
   DELETE FROM tt_ECBF_78;
   UTILS.IDENTITY_RESET('tt_ECBF_78');

   INSERT INTO tt_ECBF_78 ( 
   	SELECT CustomerID ,
   	  ...
   	  FROM ( SELECT A.RefCustomerID CustomerID  ,
             ...
             FROM PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                    ...
                    AND DA.EffectiveFromTimeKey <= v_Timekey
                    AND DA.EffectiveToTimeKey >= v_Timekey
                    LEFT JOIN tt_ACCOUNTCAL_DPD_31 AD   ON AD.CustomerEntityID = A.CustomerEntityID
              WHERE  DS.SourceName = 'ECBF'
                       AND NOT EXISTS ( SELECT 1 
                                        FROM ReverseFeedDataInsertSync_Customer R
                                         WHERE  A.RefCustomerID = R.CustomerID
   ...
   INSERT INTO tt_temp1_71
     ...
     SourceName 
                            ...
                            WHERE  B.SourceName = 'ECBF'
                                     ...
                                     AND A.EffectiveFromTimeKey <= v_TimeKey
                                     AND A.EffectiveToTimeKey >= v_TimeKey
                              GROUP BY A.ProductName,A.CustomerName,A.CustomerID,E.AssetClassGroup,E.AssetClassShortName,A.DPD,UTILS.CONVERT_TO_VARCHAR2(A.UpgradeDate,10,p_style=>105),UTILS.CONVERT_TO_VARCHAR2(A.DateofData,10,p_style=>105),a.SourceAlt_Key,SourceName ) A ) T
              UNION 
   ...
   --  group by a.SourceAlt_Key,a.SourceSystemName      
   /*    
     IF OBJECT_ID('TempDB..tt_MetaGrid_78') Is Not Null      
      Drop Table tt_MetaGrid_78      
    ...
    */
   ----------------   
   IF utils.object_id('TempDB..tt_MetaGrid_78') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MetaGrid_78 ';
   END IF;
   DELETE FROM tt_MetaGrid_78;
   UTILS.IDENTITY_RESET('tt_MetaGrid_78');

   INSERT INTO tt_MetaGrid_78 ( 
   ...
   --select * from tt_temp1_71      
   --select *       
   MERGE INTO A 
   USING (SELECT A.ROWID row_id, b.CNT
   FROM A ,StatusReport_RERF_Customer a
          JOIN tt_temp1_71 b   ON a.SourceAlt_Key = b.SourceAlt_Key ) src
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."STATUSREPORT_COUNTWISE_UPGRADE_TRACKER_RERF_TOTAL_CUSTOMER_04122023" TO "ADF_CDR_RBL_STGDB";
