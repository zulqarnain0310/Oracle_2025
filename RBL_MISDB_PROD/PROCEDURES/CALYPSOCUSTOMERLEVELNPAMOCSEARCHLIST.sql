...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         DELETE FROM GTT_CUST_PREMOC_INVESTMENT;
         UTILS.IDENTITY_RESET('GTT_CUST_PREMOC_INVESTMENT');

         INSERT INTO GTT_CUST_PREMOC_INVESTMENT ( 
         	SELECT * 
         ...
         DELETE FROM GTT_CUST_POST_INVESTMENT;
         UTILS.IDENTITY_RESET('GTT_CUST_POST_INVESTMENT');

         INSERT INTO GTT_CUST_POST_INVESTMENT ( 
         	SELECT * 
         ...
         MERGE INTO GTT_CUST_POST_INVESTMENT A
         USING (SELECT A.ROWID row_id, B.SourceName
         FROM GTT_CUST_POST_INVESTMENT A
         ...
         --Select 'GTT_CUST_POST_INVESTMENT',MOCReason_1, * from GTT_CUST_POST_INVESTMENT
         MERGE INTO GTT_CUST_POST_INVESTMENT A
         ...
         OPEN  v_cursor FOR
            SELECT A.CustomerID CustomerId  ,
                   ...
                   ELSE DB1.SourceName
                      END) SourceName  
              FROM GTT_CUST_PREMOC_INVESTMENT A
                     LEFT JOIN GTT_CUST_POST_INVESTMENT B   ON A.CustomerID = b.CustomerID
         ...
         IF utils.object_id('tempdb..GTT_MOCAuthorisation_INVESTMENT') IS NOT NULL THEN

         BEGIN
            EXECUTE IMMEDIATE ' TRUNCATE TABLE GTT_MOCAuthorisation_INVESTMENT ';

         END;
         END IF;
         DELETE FROM GTT_MOCAuthorisation_INVESTMENT;
         UTILS.IDENTITY_RESET('GTT_MOCAuthorisation_INVESTMENT');

         INSERT INTO GTT_MOCAuthorisation_INVESTMENT ( ENTITY_KEY,	UCIFID,	CUSTOMERID,	CUSTOMERNAME,	CUSTOMERENTITYID,	ASSETCLASS,	ASSETCLASSALT_KEY,	NPADATE,	SECURITYVALUE,	ADDITIONALPROVISION,	FRAUDACCOUNTFLAGALT_KEY,	FRAUDDATE,	MOCTYPEALT_KEY,	MOCTYPE,	MOCREASON,	MOCDATE,	MOCBY,	DATEOFDATE,	LEVEL1APPROVEDBY,	LEVEL2APPROVEDBY,	EFFECTIVEFROMTIMEKEY,	EFFECTIVETOTIMEKEY,	AUTHORISATIONSTATUS,	CREATEDBY,	DATECREATED,	MODIFIEDBY,	DATEMODIFIED,	APPROVEDBY,	DATEAPPROVED,	MOCSOURCEALTKEY,	SCREENFLAG,	APPROVEDBYFIRSTLEVEL,	DATEAPPROVEDFIRSTLEVEL,	SRNO,	UPLOADID,	MOCSOURCE,	MODIFYBY,	CHANGEFIELD,	MOCTYPE_FLAG,	SMASUBASSETCLASSVALUE,	SMADATE,	BOOKVALUE,ErrorMessage ,ErrorinColumn ,Srnooferroneousrows  )
         	SELECT ENTITY_KEY,	UCIFID,	CUSTOMERID,	CUSTOMERNAME,	CUSTOMERENTITYID,	ASSETCLASS,	ASSETCLASSALT_KEY,	NPADATE,	SECURITYVALUE,	ADDITIONALPROVISION,	FRAUDACCOUNTFLAGALT_KEY,	FRAUDDATE,	MOCTYPEALT_KEY,	MOCTYPE,	MOCREASON,	MOCDATE,	MOCBY,	DATEOFDATE,	LEVEL1APPROVEDBY,	LEVEL2APPROVEDBY,	EFFECTIVEFROMTIMEKEY,	EFFECTIVETOTIMEKEY,	AUTHORISATIONSTATUS,	CREATEDBY,	DATECREATED,	MODIFIEDBY,	DATEMODIFIED,	APPROVEDBY,	DATEAPPROVED,	MOCSOURCEALTKEY,	SCREENFLAG,	APPROVEDBYFIRSTLEVEL,	DATEAPPROVEDFIRSTLEVEL,	SRNO,	UPLOADID,	MOCSOURCE,	MODIFYBY,	CHANGEFIELD,	MOCTYPE_FLAG,	SMASUBASSETCLASSVALUE,	SMADATE,	BOOKVALUE,
                    ...
                    AND A.Entity_Key IN ( SELECT MAX(Entity_Key)  
                                          FROM CalypsoCustomerLevelMOC_Mod 
                                           WHERE  EffectiveFromTimeKey <= v_Timekey
                                            ...
                                            GROUP BY UCIFID )
          ;
         --Select ' GTT_MOCAuthorisation_INVESTMENT',* from  GTT_MOCAuthorisation_INVESTMENT
         --where abc=1
         UPDATE GTT_MOCAuthorisation_INVESTMENT V
         ...
         ;
         MERGE INTO GTT_MOCAuthorisation_INVESTMENT Z
         ...
         FROM CalypsoCustomerLevelMOC_Mod V
                ...
                JOIN GTT_MOCAuthorisation_INVESTMENT Z   ON X.UcifId = Z.UCIFID 
         ...
         FROM CalypsoCustomerLevelMOC_Mod V
                ...
                JOIN GTT_MOCAuthorisation_INVESTMENT Z   ON X.UCIC_ID = Z.UCIFID 
          WHERE X.AuthorisationStatus IN ( 'NP','MP','DP','1A' )
         ...
         BEGIN
            SELECT 1 INTO v_temp
             ...
             WHERE EXISTS ( SELECT 1 
                            FROM GTT_MOCAuthorisation_INVESTMENT 
