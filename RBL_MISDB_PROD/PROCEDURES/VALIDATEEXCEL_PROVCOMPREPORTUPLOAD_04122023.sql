--------------------------------------------------------
--  DDL for Procedure VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" 
(
  iv_MenuID IN NUMBER DEFAULT 27769 ,
  v_UserLoginId IN VARCHAR2 DEFAULT u'C33228' ,
  v_rptDate IN VARCHAR2 DEFAULT u'31/12/2022' ,
  v_FileName IN VARCHAR2 DEFAULT u'ProvisionComputationUpload.xlsx' 
)
AS
...
--DECLARE
--	 @MenuID		INT				= 27769
--	,@rptDate		VARCHAR(10)		= '19/06/2023'
--	,@UserLoginId	VARCHAR(20)		= 'C33228'
--	,@FileName		VARCHAR(MAX)	= N'ProvisionComputationUpload.xlsx'
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT MenuId 
              ...
              INTO v_MenuId
              FROM SysCRisMacMenu 
             WHERE  MenuCaption = 'Provision Computation Report';
            DBMS_OUTPUT.PUT_LINE(v_MenuId);

         END;
         END IF;
         IF ( v_MenuID = 27769 ) THEN
          DECLARE
            v_temp NUMBER(1, 0) := 0;
            ...
            /* --------------------------------------------------------------------------------------------------------------------------------------------------------------
            			...
            			UPDATE tt_UploadProvCompReport_2 SET    
            					ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN  'CustomerId & Account_Id is Availble and Valid but UCIC ID is not available for the Uploaded date'+' '+CONVERT(VARCHAR(10),@rptDate,103)+'. Please check the values and upload again'       
            					ELSE ErrorMessage+','+SPACE(1)+'CustomerId & Account_Id is Availble and Valid but UCIC ID is not available for the Uploaded date'+' '+CONVERT(VARCHAR(10),@rptDate,103)+'. Please check the values and upload again'     END  
            			...
            			FROM tt_UploadProvCompReport_2 V    
            			...
            			UPDATE tt_UploadProvCompReport_2 SET    
            					ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN  'Account_Id, CustomerId & UCIC ID is not available for the Uploaded date'+' '+CONVERT(VARCHAR(10),@rptDate,103)+'. Please check the values and upload again'       
            					ELSE ErrorMessage+','+SPACE(1)+'Account_Id, CustomerId & UCIC ID is not available for the Uploaded date'+' '+CONVERT(VARCHAR(10),@rptDate,103)+'. Please check the values and upload again'     END  
            					,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'CUSTOMER ID' ELSE ErrorinColumn +','+SPACE(1)+  'CUSTOMER ID' END
            					,Srnooferroneousrows=V.Srnooferroneousrows  
            			FROM tt_UploadProvCompReport_2 V    
            			WHERE (ISNULL(V.UCIC_Id,'')<>'' AND ISNULL(V.CIF_Id,'')<>''  AND ISNULL(V.ACCOUNT_Id,'')<>'') 
            			...
            			UPDATE tt_UploadProvCompReport_2 SET    
            					ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN  'Account_Id is Availble and Valid but CustomerId & UCIC ID is not available for the Uploaded date'+' '+CONVERT(VARCHAR(10),@rptDate,103)+'. Please check the values and upload again'       
            					ELSE ErrorMessage+','+SPACE(1)+'Account_Id is Availble and Valid but CustomerId & UCIC ID is not available for the Uploaded date'+' '+CONVERT(VARCHAR(10),@rptDate,103)+'. Please check the values and upload again'     END  
            					,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'CUSTOMER ID' ELSE ErrorinColumn +','+SPACE(1)+  'CUSTOMER ID' END
            					,Srnooferroneousrows=V.Srnooferroneousrows  
            			FROM tt_UploadProvCompReport_2 V    
            			WHERE (ISNULL(V.UCIC_Id,'')<>'' AND ISNULL(V.CIF_Id,'')<>''  AND ISNULL(V.ACCOUNT_Id,'')<>'') 
            ...
            v_UCICID_CNT NUMBER(10,0) := ( SELECT COUNT(UCIC_Id)  
              FROM tt_UploadProvCompReport_2  );
         ...
         BEGIN
            ...
            END;
            END IF;
            IF utils.object_id('TEMPDB..tt_UploadProvCompReport_2') IS NOT NULL THEN

            BEGIN
               EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UploadProvCompReport_2 ';
            ...
            BEGIN
               ...
               DELETE FROM tt_UploadProvCompReport_2;
               UTILS.IDENTITY_RESET('tt_UploadProvCompReport_2');

               INSERT INTO tt_UploadProvCompReport_2 ( 
               	SELECT * ,
               	  ...
               	  FROM ProvisionComputationCSVUpload  );--WHERE UserLoginId=@UserLoginId
            ...
            BEGIN
               UPDATE tt_UploadProvCompReport_2 V
               ...
               BEGIN
                  DELETE ProvisionComputationCSVUpload
                  ;
                  DBMS_OUTPUT.PUT_LINE(1);
                  DBMS_OUTPUT.PUT_LINE('ROWS DELETED FROM DBO.ProvisionComputationCSVUpload' || UTILS.CONVERT_TO_VARCHAR2(SQL%ROWCOUNT,100));
               ...
               BEGIN
                  DELETE tt_UploadProvCompReport_2
                  ;
                  DBMS_OUTPUT.PUT_LINE(1);
                  DBMS_OUTPUT.PUT_LINE('ROWS DELETED FROM tt_UploadProvCompReport_2' || UTILS.CONVERT_TO_VARCHAR2(SQL%ROWCOUNT,100));
            ...
            END IF;
            --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            UPDATE tt_UploadProvCompReport_2 
            ...
            UPDATE tt_UploadProvCompReport_2 V
             ...
             WHERE  NVL(SR_No, ' ') <> ' '
              ...
              AND NVL(ACCOUNT_Id, ' ') = ' '
              AND NVL(UserLoginId, ' ') <> ' ';
            --IF EXISTS(SELECT 1 FROM tt_UploadProvCompReport_2 WHERE ISNULL(ErrorMessage,'')<>'')  
            --BEGIN  
            --	PRINT 'NO DATA'  
            --	GOTO ERRORDATA;  
            --END
            ...
            --SELECT DISTINCT UserLoginId INTO #DistinctUserId FROM tt_UploadProvCompReport_2
            ...
            UPDATE tt_UploadProvCompReport_2 V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'UCIC ID is not Availble for the Uploaded date' || ' ' || UTILS.CONVERT_TO_VARCHAR2(v_rptDate,10,p_style=>103) || '. Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'UCIC ID is not Availble for the Uploaded date' || ' ' || UTILS.CONVERT_TO_VARCHAR2(v_rptDate,10,p_style=>103) || '. Please check the values and upload again'
                      END,
                   ErrorinColumn = CASE 
            ...
            ;
            IF utils.object_id('TEMPDB..tt_DUB_UCIF_6') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_UCIF_6 ';
            END IF;
            --SELECT * FROM tt_UploadProvCompReport_2
            DELETE FROM tt_DUB_UCIF_6;
            UTILS.IDENTITY_RESET('tt_DUB_UCIF_6');

            INSERT INTO tt_DUB_UCIF_6 SELECT * 
                 FROM ( SELECT * ,
                               ROW_NUMBER() OVER ( PARTITION BY UCIC_ID ORDER BY UCIC_ID  ) rw  
                        FROM tt_UploadProvCompReport_2  ) X
                WHERE  rw > 1;
            MERGE INTO V 
            USING (SELECT V.ROWID row_id, CASE 
            WHEN NVL(V.ErrorMessage, ' ') = ' ' THEN 'Duplicate UCIC ID found in the Uploaded file. Please check & remove the duplicate UCIC ID and upload the file again'
            ELSE V.ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Duplicate UCIC ID found in the Uploaded file. Please check & remove the duplicate UCIC ID and upload the file again'
               END AS pos_2, CASE 
            WHEN NVL(V.ErrorinColumn, ' ') = ' ' THEN 'UCIC ID'
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'UCIC ID'
               END AS pos_3, V.SR_No
            FROM V ,tt_UploadProvCompReport_2 V
                   JOIN tt_DUB_UCIF_6 D   ON D.UCIC_Id = V.UCIC_Id ) src
            ...
            UPDATE tt_UploadProvCompReport_2 V
               SET ErrorMessage = CASE 
                                       WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Invalid UCIC ID. Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Invalid UCIC ID. Please check the values and upload again'
                      END,
                   ErrorinColumn = CASE 
                                        WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'UCIC ID'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'UCIC ID'
              ...
              AND V.CIF_Id NOT IN ( SELECT CustomerId 
                                    FROM CustomerBasicDetail 
                                     WHERE  ( EffectiveFromTimeKey <= v_Timekey
                                              AND EffectiveToTimeKey >= v_Timekey ) )
            ;
            IF utils.object_id('TEMPDB..tt_DUB_CIF_6') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_CIF_6 ';
            END IF;
            DELETE FROM tt_DUB_CIF_6;
            UTILS.IDENTITY_RESET('tt_DUB_CIF_6');

            INSERT INTO tt_DUB_CIF_6 SELECT * 
                 FROM ( SELECT * ,
            ...
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'CUSTOMER ID'
               END AS pos_3, V.SR_No
            FROM V ,tt_UploadProvCompReport_2 V
                   JOIN tt_DUB_CIF_6 D   ON D.CIF_ID = V.CIF_ID ) src
            ON ( V.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET ErrorMessage = pos_2,
                                         ErrorinColumn = pos_3,
                                         Srnooferroneousrows = src.SR_No;
              ...
              AND V.ACCOUNT_Id NOT IN ( SELECT CustomerACID 
                                        FROM AdvAcBasicDetail 
                                         WHERE  ( EffectiveFromTimeKey <= v_Timekey
                                                  AND EffectiveToTimeKey >= v_Timekey ) )
            ;
            IF utils.object_id('TEMPDB..tt_DUB_ACCOUNTID_6') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_ACCOUNTID_6 ';
            END IF;
            DELETE FROM tt_DUB_ACCOUNTID_6;
            UTILS.IDENTITY_RESET('tt_DUB_ACCOUNTID_6');

            INSERT INTO tt_DUB_ACCOUNTID_6 SELECT * 
                 FROM ( SELECT * ,
            ...
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'ACCOUNT ID'
               END AS pos_3, V.SR_No
            FROM V ,tt_UploadProvCompReport_2 V
                   JOIN tt_DUB_ACCOUNTID_6 D   ON D.ACCOUNT_Id = V.ACCOUNT_Id ) src
            ON ( V.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET ErrorMessage = pos_2,
                                         ErrorinColumn = pos_3,
                                         Srnooferroneousrows = src.SR_No;
         ...
         BEGIN
            ...
            --IF EXISTS(SELECT 1 FROM ProvisionComputationCSVUpload)      --WHERE UserLoginId=@UserLoginId  
            --BEGIN  
            --	DELETE FROM ProvisionComputationCSVUpload  
            --	--WHERE UserLoginId=@UserLoginId  
            --	PRINT 1  
            --	PRINT 'ROWS DELETED FROM DBO.ProvisionComputationCSVUpload'+CAST(@@ROWCOUNT AS VARCHAR(100))
            --END  
            OPEN  v_cursor FOR
               SELECT -10 
                 FROM DUAL  ;
         ...
         --	IF EXISTS(SELECT 1 FROM tt_UploadProvCompReport_2)
         --	BEGIN
         --		DELETE FROM tt_UploadProvCompReport_2
         --		PRINT 1
         --		PRINT 'ROWS DELETED FROM tt_UploadProvCompReport_2'+CAST(@@ROWCOUNT AS VARCHAR(100))
         --	END
         --	SELECT 1
         --	SELECT 1
         ...
         --END
         ------------------------------------------------------------------------------------------------------------------  
         IF utils.object_id('TEMPDB..tt_Temp_Success_3') IS NOT NULL THEN
          --GROUP BY FileNames  
         ...
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Temp_Success_3 ';
         END IF;
         DELETE FROM tt_Temp_Success_3;
         UTILS.IDENTITY_RESET('tt_Temp_Success_3');

         INSERT INTO tt_Temp_Success_3 ( 
         	SELECT COUNT(*)  COUNT  ,
         	 ...
         	 WHERE  ( NVL(ColumnName, ' ') = ' '
                    ...
                    AND Flag = 'SUCCESS' );
         EXECUTE IMMEDIATE ' TRUNCATE TABLE IRAC_Success_ProvComp_UploadCount ';
         INSERT INTO IRAC_Success_ProvComp_UploadCount
           ( COUNT, FILENAME, UserLoginID )
           ( SELECT COUNT ,
                    FileName_HC ,
                    UserLoginId 
             FROM tt_Temp_Success_3  );
         EXECUTE IMMEDIATE ' TRUNCATE TABLE SAG_ProvComp_SuccessDetails ';
         ...
         BEGIN
            ...
            INSERT INTO SAG_ProvComp_SuccessDetails
              ( UCIC, UserLoginID )
              ( SELECT UCIF_ID ,
                       v_UserLoginId 
                FROM RBL_MISDB_PROD.MasterUploadData_SAGACLProvDPD 
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD_04122023" TO "ADF_CDR_RBL_STGDB";
