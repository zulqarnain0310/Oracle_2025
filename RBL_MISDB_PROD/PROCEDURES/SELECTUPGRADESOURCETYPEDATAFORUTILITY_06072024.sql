--------------------------------------------------------
--  DDL for Procedure SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" 
...
BEGIN
   ...
   IF utils.object_id('TempDB..tt_ReverseFeedDataInsertSyn_12') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ReverseFeedDataInsertSyn_12 ';
   END IF;
   DELETE FROM tt_ReverseFeedDataInsertSyn_12;
   UTILS.IDENTITY_RESET('tt_ReverseFeedDataInsertSyn_12');

   INSERT INTO tt_ReverseFeedDataInsertSyn_12 ( 
   	SELECT * 
   ...
   BEGIN
      --------------Finacle
      ----------Added on 04/04/2022
      IF utils.object_id('TempDB..tt_Finacle_40') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Finacle_40 ';
      END IF;
      DELETE FROM tt_Finacle_40;
      UTILS.IDENTITY_RESET('tt_Finacle_40');

      INSERT INTO tt_Finacle_40 ( 
      	SELECT A.CustomerAcID ,
      	  ...
      	  FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_ReverseFeedDataInsertSyn_12 RC   ON A.CustomerAcID = RC.CustomerAcID
   ...
   /*
     IF (@SourceType ='Ganaseva')
   	...
   	And B.EffectiveFromTimekey<=@TimeKey AND B.EffectiveToTimeKey>=@TimeKey
   	 ...
   	 Select 'GanasevaUpgrade' AS TableName, CustomerAcID +'|'+'0'+'|'+Convert(Varchar(10),isnull(UpgDate,@Date),103)+'|'+'19718'+'|'+'19718' as DataUtility from #Ganaseva A
   ...
   BEGIN
      --------------MiFin
      ----------Added on 04/04/2022
      IF utils.object_id('TempDB..tt_MiFin_40') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MiFin_40 ';
      END IF;
      DELETE FROM tt_MiFin_40;
      UTILS.IDENTITY_RESET('tt_MiFin_40');

      INSERT INTO tt_MiFin_40 ( 
      	SELECT A.CustomerAcID ,
              A.FinalAssetClassAlt_Key ,
              A.FinalNpaDt ,
              A.UpgDate ,
   ...
   BEGIN
      --------------Indus
      ----------Added on 04/04/2022
      IF utils.object_id('TempDB..tt_Indus_40') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Indus_40 ';
      END IF;
      DELETE FROM tt_Indus_40;
      UTILS.IDENTITY_RESET('tt_Indus_40');

      INSERT INTO tt_Indus_40 ( 
      	SELECT A.CustomerAcID ,
              A.FinalAssetClassAlt_Key ,
              A.FinalNpaDt ,
              A.UpgDate ,
   ...
   BEGIN
      --------------ECBF
      ----------------Added on 04/04/2022
      IF utils.object_id('TempDB..tt_ECBF_42') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ECBF_42 ';
      END IF;
      DELETE FROM tt_ECBF_42;
      UTILS.IDENTITY_RESET('tt_ECBF_42');

      INSERT INTO tt_ECBF_42 ( 
      	SELECT CustomerAcID ,
      	  ...
      	  FROM ( SELECT A.CustomerAcID ,
                       ...
                       DA.AssetClassGroup SystemClassification  ,
                       DA.AssetClassShortName SystemSubClassification  ,
                       A.DPD_Max DPD  ,
                       'STD' UserClassification  ,
                       'DPD0' UserSubClassification  ,
                       UTILS.CONVERT_TO_VARCHAR2(A.FinalNpaDt,10,p_style=>105) NpaDate  ,
                       v_Date CurrentDate  

                --Added by Prashant--08042024--only change in select columns--sysClass-SysSubClass--UserClass--UserSubClass--	 
                FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                       JOIN tt_ReverseFeedDataInsertSyn_12 RC   ON A.CustomerAcID = RC.CustomerAcID
                       JOIN RBL_MISDB_PROD.AdvAcBalanceDetail B   ON A.AccountEntityID = B.AccountEntityID
                       AND B.EffectiveFromTimeKey <= v_Timekey
         ...
         SELECT SrNo ,
           ...
           FROM ( SELECT ROW_NUMBER() OVER ( ORDER BY ClientCustId  ) SrNo  ,
                  ...
                  FROM ( 
                         ...
                         --Added by Prashant--08042024--change in select columns--sysClass-SysSubClass--UserClass--UserSubClass--

                         --Added AdvAcBalanceDetail and DimAssetClassMapping in below from query
                         FROM ReverseFeedData A
                                JOIN DIMSOURCEDB B   ON A.SourceAlt_Key = B.SourceAlt_key
                                AND B.EffectiveFromTimekey <= v_TimeKey
                                AND B.EffectiveToTimeKey >= v_TimeKey
                                ...
                                AND E.EffectiveFromTimekey <= v_TimeKey
                                AND E.EffectiveToTimeKey >= v_TimeKey
                                JOIN RBL_MISDB_PROD.AdvAcBalanceDetail AB   ON A.AccountID = AB.RefSystemAcId
                                AND AB.EffectiveFromTimeKey <= v_Timekey
                                AND AB.EffectiveToTimeKey >= v_Timekey
                                JOIN DimAssetClassMapping DA   ON DA.SrcSysClassCode = AB.SourceAssetClass
                                AND A.SourceAlt_Key = DA.SourceAlt_Key
                                AND DA.EffectiveFromTimeKey <= v_Timekey
                                AND DA.EffectiveToTimeKey >= v_Timekey
                         ...
                         WHERE  B.SourceName = 'ECBF'
                                  ...
                                  AND A.EffectiveFromTimekey <= v_TimeKey
                                  AND A.EffectiveToTimeKey >= v_TimeKey
                           GROUP BY A.ProductName,A.CustomerName,A.CustomerID,E.AssetClassGroup,E.SrcSysClassCode,A.DPD,UTILS.CONVERT_TO_VARCHAR2(A.UpgradeDate,10,p_style=>105),UTILS.CONVERT_TO_VARCHAR2(A.DateofData,10,p_style=>105),DA.AssetClassGroup,DA.AssetClassShortName ) A ) T
         UNION 
   ...
   BEGIN
      --------------MetaGrid
      ----------Added on 04/04/2022
      IF utils.object_id('TempDB..tt_MetaGrid_42') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MetaGrid_42 ';
      END IF;
      DELETE FROM tt_MetaGrid_42;
      UTILS.IDENTITY_RESET('tt_MetaGrid_42');

      INSERT INTO tt_MetaGrid_42 ( 
      	SELECT A.CustomerAcID ,
   ...
   END;
   END IF;
   --===================================EIFS ADDEDS BY MANDEEP SINGH 27-09-2022--------------------------------------------------------------------
   ...
   BEGIN
      IF utils.object_id('TempDB..tt_EIFS_25') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_EIFS_25 ';
      END IF;
      DELETE FROM tt_EIFS_25;
      UTILS.IDENTITY_RESET('tt_EIFS_25');

      INSERT INTO tt_EIFS_25 ( 
      	SELECT CustomerAcID ,
              FinalAssetClassAlt_Key ,
              FinalNpaDt ,
              SourceAssetClass ,
      	  ...
      	  FROM ( 
                ...
                --DA.AssetClassAlt_Key BankAssetClass,DP.ProductName ProductType,C.CustomerName ClientName,A.RefCustomerID as ClientCustId,E.AssetClassGroup as SystemAssetClassification,  

                --Case When E.SrcSysClassCode='SS' then 'SBSTD' When E.SrcSysClassCode='D1' then 'DBT01'  When E.SrcSysClassCode='D2' then 'DBT02'   

                --    When E.SrcSysClassCode='D3' then 'DBT03' When E.SrcSysClassCode='L1' then 'LOSS' Else E.SrcSysClassCode End as SystemSubClassification  

                --,A.DPD_Max as UserDPD,'NPA' as UserClassification,'SBSTD' as UserSubClassification,  
                SELECT A.CustomerAcID ,
                       A.FinalAssetClassAlt_Key ,
                       A.FinalNpaDt ,
                       B.SourceAssetClass ,
                       ...
                       C.CustomerName ClientName  ,
                       A.RefCustomerID ClientCustId  ,
                       DA.AssetClassGroup SystemAssetClassification  ,
                       DA.AssetClassShortName SystemSubClassification  ,
                       A.DPD_Max UserDPD  ,
                       'STD' UserClassification  ,
                       'DPD0' UserSubClassification  ,
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."SELECTUPGRADESOURCETYPEDATAFORUTILITY_06072024" TO "ADF_CDR_RBL_STGDB";
