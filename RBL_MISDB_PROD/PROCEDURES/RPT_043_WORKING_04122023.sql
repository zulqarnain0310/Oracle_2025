--------------------------------------------------------
--  DDL for Procedure RPT_043_WORKING_04122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" 
--USE [RBL_MISDB]
 --GO
...
BEGIN
   ...
   BEGIN
      ...
      IF utils.object_id('TEMPDB..tt_accountcal_hist_2') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_accountcal_hist_2 ';
      END IF;
      ...
      --Drop table if exists   tt_DPD_34   
      IF utils.object_id('TEMPDB..tt_DPD_34') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DPD_34 ';
      END IF;
      IF utils.object_id('TEMPDB..tt_A_38') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_A_38 ';
      END IF;
      DELETE FROM tt_DPD_34;
      UTILS.IDENTITY_RESET('tt_DPD_34');

      INSERT INTO tt_DPD_34 ( 
      ...
      EXECUTE IMMEDIATE ' ALTER TABLE tt_DPD_34 
      ...
      IF tt_DPD_34_UCIF_ID  --SQLDEV: NOT RECOGNIZED
      DELETE FROM tt_DPD_UCIF_ID_5;
      UTILS.IDENTITY_RESET('tt_DPD_UCIF_ID_5');

      INSERT INTO tt_DPD_UCIF_ID_5 ( 
      	SELECT UCIF_ID ,
      ...
      FROM A ,tt_DPD_34 A
             JOIN tt_DPD_UCIF_ID_5 B   ON A.UCIF_ID = B.UCIF_ID ) src
      ON ( A.ROWID = src.row_id )
      WHEN MATCHED THEN UPDATE SET A.DPD_UCIF_ID = src.DPD_UCIF_ID;
      -----------------------------------------------------------------
      DELETE FROM tt_A_38;
      UTILS.IDENTITY_RESET('tt_A_38');
      ...
      INSERT INTO tt_A_38 ( 
      	SELECT 'a' CurrentProcessingDate  ,
      	  ...
      	  FROM tt_accountcal_hist_2 A
                ...
                JOIN tt_DPD_34 DPD   ON DPD.AccountEntityID = A.AccountEntityID
   ...
   IF utils.object_id('tEMPDB..tt_B_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_B_2 ';
   END IF;
   --SELECT * FROM tt_A_38
   ...
   IF utils.object_id('Tempdb..tt_B_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_B_2 ';
   END IF;
   IF utils.object_id('Tempdb..tt_BALANCE_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BALANCE_2 ';
   END IF;
   --Declare @BAL decimal(30,2)
   --set @Bal=(
   DELETE FROM tt_BALANCE_2;
   UTILS.IDENTITY_RESET('tt_BALANCE_2');

   INSERT INTO tt_BALANCE_2 ( 
   	SELECT SUM(Balance)  BALAN  ,
           UCIF_ID 
   	  FROM tt_A_38 
   	  GROUP BY UCIF_ID );
   DELETE FROM tt_B_2;
   UTILS.IDENTITY_RESET('tt_B_2');
   ...
   EXECUTE IMMEDIATE ' ALTER TABLE tt_B_2 
   ...
   IF utils.object_id('tempdb..tt_DP_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DP_2 ';
   END IF;
   DELETE FROM tt_DP_2;
   UTILS.IDENTITY_RESET('tt_DP_2');
   ...
   IF utils.object_id('tempdb..tt_F_DPD_2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_F_DPD_2 ';
   END IF;
   DELETE FROM tt_F_DPD_2;
   UTILS.IDENTITY_RESET('tt_F_DPD_2');

   INSERT INTO tt_F_DPD_2 ( 
   	SELECT MAX(DPD)  DPD  ,
           UCIF_ID 
   	  FROM tt_DP_2 
   	  GROUP BY UCIF_ID );--where UCIF_ID='RBL001858036'
   --select * from tt_F_DPD_2 where UCIF_ID='RBL001858036'
   MERGE INTO B 
   USING (SELECT B.ROWID row_id, A.DPD
   FROM B ,tt_B_2 B
          JOIN tt_DP_2 A   ON A.UCIF_ID = B.UCIF_ID 
   ...
   --SET DL_DPD=A.DPD 
   --FROM tt_B_2 B
   --INNER JOIN tt_DP_2 A  ON A.UCIF_ID=B.UCIF_ID
   ...
   FROM A ,tt_BALANCE_2 B
          JOIN tt_B_2 A   ON A.UCIF_ID = B.UCIF_ID ) src
   ON ( A.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET A.Balance = src.Balan;
   MERGE INTO B 
   USING (SELECT B.ROWID row_id, A.DPD
   FROM B ,tt_B_2 B
          JOIN tt_F_DPD_2 A   ON A.UCIF_ID = B.UCIF_ID ) src
   ON ( B.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET B.SMA_DPD = src.DPD;
   ----select MAX(DPD) from tt_DP_2 where UCIF_ID='RBL001858036'
   MERGE INTO b 
   ...
   OPEN  v_cursor FOR
      SELECT CurrentProcessingDate ,
        ...
        --,MovementToStatus
        FROM tt_B_2 --where  UCIF_ID='RBL001858036'
               ;
      DBMS_SQL.RETURN_RESULT(v_cursor);--select * from tt_B_2 where UCIF_ID='RBL001858036'
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."RPT_043_WORKING_04122023" TO "ADF_CDR_RBL_STGDB";
