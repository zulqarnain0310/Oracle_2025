--------------------------------------------------------
--  DDL for Procedure TWOREPORT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."TWOREPORT" 
AS
...
BEGIN
   ...
   IF tt_ExceptionFinalStatusType_12  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_ExceptionFinalStatusType_12;
   UTILS.IDENTITY_RESET('tt_ExceptionFinalStatusType_12');

   INSERT INTO tt_ExceptionFinalStatusType_12 SELECT DISTINCT A.* ,
   ...
   DELETE tt_ExceptionFinalStatusType_12

    WHERE  RANK > 1;
   -----------------------------------------------------------------------------------------------------
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_OpeningBalance_12  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_OpeningBalance_12;
   UTILS.IDENTITY_RESET('tt_OpeningBalance_12');

   INSERT INTO tt_OpeningBalance_12 SELECT A.ACID ,
                                           B.PrincOutStd ,
                                           ROW_NUMBER() OVER ( PARTITION BY A.ACID ORDER BY A.ACID, B.EffectiveToTimeKey DESC  ) RANK  
        FROM tt_ExceptionFinalStatusType_12 A
       ...
       WHERE  ( ( a.EffectiveFromTimeKey <= v_LastQtrTimekey
                ...
                AND statustype = 'TWO';
   UPDATE tt_OpeningBalance_12
      SET PrincOutStd = 0
    WHERE  PrincOutStd < 0;
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_A_41ccountCal  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_Accountcal_12;
   UTILS.IDENTITY_RESET('tt_Accountcal_12');

   INSERT INTO tt_Accountcal_12 ( 
   	SELECT * 
   	  FROM ( SELECT UCIF_ID ,
                    RefCustomerID ,
                    CustomerAcID ,
   ...
   UPDATE tt_ExceptionFinalStatusType_12
   ...
   --------------------------------------------------------------------------------------------------------
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_DimCustomer_12  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_DimCustomer_12;
   UTILS.IDENTITY_RESET('tt_DimCustomer_12');

   INSERT INTO tt_DimCustomer_12 ( 
             ...
             JOIN ( SELECT DISTINCT CustomerID ,
                    ...
                    FROM CustomerBasicDetail 
                     WHERE  CustomerID IN ( SELECT DISTINCT CustomerId 
                                            FROM tt_ExceptionFinalStatusType_12  )
   ...
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_A_41  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_A_41;
   UTILS.IDENTITY_RESET('tt_A_41');
   	...
   	SELECT DISTINCT CustomerAcID ,
   	 ...
   	 WHERE  EffectiveFromTimeKey <= v_Timekey
              ...
              AND CustomerAcID IN ( SELECT b.CustomerAcID 
                                    FROM tt_ExceptionFinalStatusType_12 Z
                                           ...
                                           LEFT JOIN ( SELECT AcID CustomerACID  ,
                                                       ...
                                                       FROM tt_ExceptionFinalStatusType_12  ) Y   ON Z.ACID = Y.CustomerACID
   ...
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_DPD_53  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_DPD_53;
   UTILS.IDENTITY_RESET('tt_DPD_53');

   INSERT INTO tt_DPD_53 ( 
   	SELECT CustomerACID ,
           AccountEntityid ,
   ...
   EXECUTE IMMEDIATE ' ALTER TABLE tt_DPD_53 
   ...
   ----/* CALCULATE MAX DPD */
   IF utils.object_id('TEMPDB..tt_TEMPTABLE_147') IS NOT NULL THEN
    --drop table if exists tt_TWOReport_15_Final
   --select * into tt_TWOReport_15_Final from tt_TWOReport_15 
   EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_147 ';
   END IF;
   DELETE FROM tt_TEMPTABLE_147;
   UTILS.IDENTITY_RESET('tt_TEMPTABLE_147');

   INSERT INTO tt_TEMPTABLE_147 ( 
   ...
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_TWOReport_15  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_TWOReport_15;
   UTILS.IDENTITY_RESET('tt_TWOReport_15');

   INSERT INTO tt_TWOReport_15 ( 
   	SELECT UTILS.CONVERT_TO_NVARCHAR2(v_Date,30,p_style=>105) Report_date  ,
   	  ...
   	  FROM tt_ExceptionFinalStatusType_12 Z
             ...
             AND B.EffectiveFromTimeKey <= v_Timekey
             AND B.EffectiveToTimeKey >= v_Timekey
             LEFT JOIN tt_Accountcal_12 AC   ON Z.ACID = AC.CustomerACID
             LEFT JOIN PRO_RBL_MISDB_PROD.CustomerCal_Hist A   ON A.CustomerEntityID = b.CustomerEntityID
             AND A.EffectiveFromTimeKey <= v_Timekey
             AND A.EffectiveToTimeKey >= v_Timekey
             LEFT JOIN tt_DimCustomer_12 xyz   ON Z.CustomerID = xyz.CustomerId
             ...
             LEFT JOIN ( SELECT ACID CustomerAcID  ,
                                ...
                                StatusDate WriteOffDt  
                         FROM tt_ExceptionFinalStatusType_12  ) Y   ON Z.ACID = Y.CustomerAcID
             LEFT JOIN tt_DPD_53 DPD   ON DPD.AccountEntityID = b.AccountEntityID
   ...
   FROM A ,tt_TWOReport_15 A
          JOIN tt_Accountcal_12 B   ON A.Account_No_ = B.CustomerACID ) src
   ON ( A.ROWID = src.row_id )
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."TWOREPORT" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."TWOREPORT" TO "ADF_CDR_RBL_STGDB";
