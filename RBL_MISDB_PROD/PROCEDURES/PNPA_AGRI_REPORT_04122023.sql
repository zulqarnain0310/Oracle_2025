--------------------------------------------------------
--  DDL for Procedure PNPA_AGRI_REPORT_04122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" 
AS
...
BEGIN
   ...
   END IF;
   IF utils.object_id('TEMPDB..tt_CUSTOMERCAL_50') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CUSTOMERCAL_50 ';
   END IF;
   ...
   DELETE FROM tt_CUSTOMERCAL_50;
   UTILS.IDENTITY_RESET('tt_CUSTOMERCAL_50');

   INSERT INTO tt_CUSTOMERCAL_50 ( 
   	SELECT * 
   ...
   FROM A ,tt_CUSTOMERCAL_50 A ) src
   ON ( A.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET A.FlgPNPA = 'N',
                                PNPA_Dt = NULL,
                                PNPA_Class_Key = NULL;
   ...
   ELSE 'N'
      END) AS FlgPNPA
   FROM A ,tt_ACCOUNTCAL_49 A
          JOIN tt_CUSTOMERCAL_50 B   ON A.RefCustomerID = B.RefCustomerID 
   ...
   DELETE FROM tt_PNPA_REASON_DATE_7;
   --and isnull(REFPeriodMax,0) >=180
   --select FlgPNPA,BALANCE,* from tt_ACCOUNTCAL_49 where AccountEntityID=16724163
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_PNPA_REASON_DATE_7  --SQLDEV: NOT RECOGNIZED
   INSERT INTO tt_PNPA_REASON_DATE_7
   ...
   FROM B ,tt_ACCOUNTCAL_49 A
          JOIN tt_CUSTOMERCAL_50 B   ON A.CustomerEntityID = B.CustomerEntityID 
    WHERE A.FlgPNPA = 'Y'
     AND ( B.FlgProcessing = 'N' )) src
   ON ( B.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET B.FlgPNPA = 'Y';
   IF utils.object_id('TEMPDB..tt_TEMPTABLEPNPA_13') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLEPNPA_13 ';
   END IF;
   DELETE FROM tt_TEMPTABLEPNPA_13;
   UTILS.IDENTITY_RESET('tt_TEMPTABLEPNPA_13');

   INSERT INTO tt_TEMPTABLEPNPA_13 ( 
   ...
   FROM A ,tt_CUSTOMERCAL_50 A
          JOIN tt_TEMPTABLEPNPA_13 B   ON A.CustomerEntityID = B.CustomerEntityID ) src
   ON ( A.ROWID = src.row_id )
   ...
   FROM A ,tt_ACCOUNTCAL_49 A
          JOIN tt_ACCOUNTCAL_49 B   ON A.CustomerEntityID = B.CustomerEntityID
          AND A.FlgPNPA = 'Y'
          AND A.FlgPNPA = 'Y'
   ...
   USING (SELECT B.ROWID row_id, 'Y'
   FROM B ,tt_ACCOUNTCAL_49 A
          JOIN tt_CUSTOMERCAL_50 B   ON A.UcifEntityID = B.UcifEntityID 
    WHERE A.FlgPNPA = 'Y'
     AND ( B.FlgProcessing = 'N' )) src
   ON ( B.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET B.FlgPNPA = 'Y';
   IF utils.object_id('TEMPDB..tt_CTE_CUSTOMERWISEBALANCEP_13') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CTE_CUSTOMERWISEBALANCEP_13 ';
   END IF;
   DELETE FROM tt_CTE_CUSTOMERWISEBALANCEP_13;
   UTILS.IDENTITY_RESET('tt_CTE_CUSTOMERWISEBALANCEP_13');

   INSERT INTO tt_CTE_CUSTOMERWISEBALANCEP_13 ( 
   ...
   USING (SELECT A.ROWID row_id, B.PNPA_DATE, 'Y', b.PNPA_Class_Key
   FROM A ,tt_CUSTOMERCAL_50 A
          JOIN tt_CTE_CUSTOMERWISEBALANCEP_13 B   ON A.UcifEntityID = B.UcifEntityID ) src
   ON ( A.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET A.PNPA_DT = src.PNPA_DATE,
                                A.FlgPNPA = 'Y',
                                a.PNPA_Class_Key = src.PNPA_Class_Key;
   ...
   USING (SELECT A.ROWID row_id, b.PNPA_DATE, 'Y', b.PNPA_Class_Key
   FROM A ,tt_ACCOUNTCAL_49 a
          JOIN tt_CTE_CUSTOMERWISEBALANCEP_13 b   ON a.UcifEntityID = b.UcifEntityID
          AND ( NVL(A.Asset_Norm, 'NORMAL') <> 'ALWYS_STD' ) ) src
   ON ( A.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET a.PNPA_DATE = src.PNPA_DATE,
                                a.FlgPNPA = 'Y',
   ...
   FROM A ,tt_ACCOUNTCAL_49 a
          JOIN tt_ACCOUNTCAL_49 b   ON a.UcifEntityID = b.UcifEntityID 
    WHERE b.FlgPNPA = 'Y'
     AND A.FlgPNPA = 'Y'
     AND A.PNPA_Reason IS NULL
     AND B.PNPA_Reason IS NOT NULL
   ...
   IF ( utils.object_id('TEMPDB..tt_UCIF_3') IS NOT NULL ) THEN
    --select distinct UCIF_ID,UcifEntityID into tt_UCIF_3 from tt_ACCOUNTCAL_49
   --where	FlgPNPA ='Y'  and ProductCode in ('CLIBA',	'HLIBA',	'PLIBA',	'VLIBA',	'HLCTC',	'PLCTC',	'VLCTC',	'RFLPL',	
   --										  'PLSPL',	'ELHRP',	'PLODS',	'PLSOD')
   EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UCIF_3 ';
   END IF;
   DELETE FROM tt_UCIF_3;
   UTILS.IDENTITY_RESET('tt_UCIF_3');

   INSERT INTO tt_UCIF_3 ( 
   	SELECT DISTINCT UCIF_ID ,
                    UcifEntityID 
   	  FROM tt_ACCOUNTCAL_49 
   	 WHERE  FlgPNPA = 'Y' );
   ...
   INSERT INTO RBL_MISDB_PROD.AGRI_PNPA_REPORT_AUTOMATE
     ( SELECT UTILS.CONVERT_TO_NVARCHAR2(SYSDATE,30,p_style=>105) Generation_Date  ,
       ...
       FROM tt_CUSTOMERCAL_50 A
              JOIN tt_ACCOUNTCAL_49 B   ON A.UcifEntityID = B.UcifEntityID
              JOIN tt_UCIF_3 AB   ON b.UcifEntityID = ab.UcifEntityID
   ...
   OPEN  v_cursor FOR
      ...
      DBMS_SQL.RETURN_RESULT(v_cursor);--select * from tt_ACCOUNTCAL_49
   --where FlgPNPA='Y'
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PNPA_AGRI_REPORT_04122023" TO "ADF_CDR_RBL_STGDB";
