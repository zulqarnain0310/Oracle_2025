--------------------------------------------------------
--  DDL for Procedure VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" /*--------------------------------------------------------------------------
         ...
         BEGIN
            ...
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
                   ...
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'SlNo'
                      END,
                   V.Srnooferroneousrows = V.SrNo
             WHERE  REGEXP_LIKE(NVL(SrNo, ' '), '%[,!@#$%^&*()_-+=/]%- \ / _');
            --
            SELECT COUNT(1)  
            ...
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
                   ...
                   V.Srnooferroneousrows = V.SrNo
             WHERE  NVL(RestructureFacility, ' ') <> ' '
              AND RestructureFacility NOT IN ( 'Restructured','Additional Finance','FITL','Other' )
            ;
            ...
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
                   ...
                   V.Srnooferroneousrows = V.SrNo
             WHERE  NVL(TypeofRestructuring, ' ') <> ' '
              AND NVL(TypeofRestructuring, ' ') NOT IN ( 'IRAC Circular','Prudential Framework (June 7,2020 circular)','Resolution Framework (COVID-OTR)','MSME_Old circular','MSME_COVID circular','MSME_COVID_RF 2.0','COVID_OTR_RF 2.0','Under DCCO - PUI','Natural Calamity','BIFR','Other' )
            ;
            ---------------------------------------------------------------------------------------------------------------------
            ...
            ;
            IF utils.object_id('TEMPDB..tt_DUB2_39') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB2_39 ';
            END IF;
            DELETE FROM tt_DUB2_39;
            UTILS.IDENTITY_RESET('tt_DUB2_39');

            INSERT INTO tt_DUB2_39 SELECT * 
                 FROM ( SELECT * ,
            ...
            FROM V ,RestructureAssets V
                   JOIN tt_DUB2_39 D   ON D.AccountID = V.AccountID ) src
            ...
            UPDATE RestructureAssets
            	SET  
                    ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'DisbursementDate Can not be Blank or Null or Zero. Please enter the DisbursementDate and upload again'     
            						ELSE ErrorMessage+','+SPACE(1)+ 'DisbursementDate Can not be Blank or Null. Please enter the DisbursementDate and upload again'      END
            		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'DisbursementDate' ELSE   ErrorinColumn +','+SPACE(1)+'DisbursementDate' END      
            		,Srnooferroneousrows=V.SrNo

             FROM RestructureAssets V  
             WHERE ISNULL(DisbursementDate,'')='' OR len(isnull(DisbursementDate,''))=0
            ...
            UPDATE RestructureAssets V
             ...
             WHERE  NVL(ReferenceDate, ' ') <> ' '
              AND DisbursementDate NOT LIKE '__-__-____'
            ...
            UPDATE RestructureAssets
            	SET  
                    ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'ReferenceDate Can not be Blank or Null or Zero . Please enter the ReferenceDate and upload again'     
            						ELSE ErrorMessage+','+SPACE(1)+ 'ReferenceDate Can not be Blank or Null or Zero. Please enter the ReferenceDate and upload again'      END
             ...
             WHERE ISNULL(ReferenceDate,'')='' OR len(isnull(ReferenceDate,''))=0
             */
            -------
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
             ...
             WHERE  NVL(ReferenceDate, ' ') <> ' '
              AND ReferenceDate NOT LIKE '__-__-____'
            ...
            UPDATE RestructureAssets
            	SET  
                    ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'InvocationDate Can not be Blank or Null or Zero. Please enter the InvocationDate and upload again'     
            						ELSE ErrorMessage+','+SPACE(1)+ 'InvocationDate Can not be Blank or Null or Zero. Please enter the InvocationDate and upload again'      END
            		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'InvocationDate' ELSE   ErrorinColumn +','+SPACE(1)+'InvocationDate' END      
            		,Srnooferroneousrows=V.SrNo


             FROM RestructureAssets V  
             WHERE ISNULL(InvocationDate,'')='' OR len(isnull(InvocationDate,''))=0
             */
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
             ...
             WHERE  NVL(InvocationDate, ' ') <> ' '
              AND InvocationDate NOT LIKE '__-__-____'
            ...
            UPDATE RestructureAssets
            	SET  
                    ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'Date of Conversion in to Equity Can not be Blank or Null or Zero . Please enter the DateofConversionintoEquity and upload again'     
            						ELSE ErrorMessage+','+SPACE(1)+ 'Date of Conversion in to Equity Can not be Blank. Please enter the DateofConversionintoEquity and upload again'      END
             ...
             FROM RestructureAssets V  
             WHERE ISNULL(DateofConversionintoEquity,'')='' OR len(isnull(DateofConversionintoEquity,''))=0
             */
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
             ...
             WHERE  NVL(DateofConversionintoEquity, ' ') <> ' '
              AND DateofConversionintoEquity NOT LIKE '__-__-____'
            ...
            UPDATE RestructureAssets
            	SET  
                    ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'PrinRpymntStartDate Can not be Blank or Null or Zero . Please enter the PrinRpymntStartDate and upload again'     
            						ELSE ErrorMessage+','+SPACE(1)+ 'PrinRpymntStartDate Can not be Blank or Null or Zero. Please enter the PrinRpymntStartDate and upload again'      END
            		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'PrinRpymntStartDate' ELSE   ErrorinColumn +','+SPACE(1)+'PrinRpymntStartDate' END      
            		,Srnooferroneousrows=V.SrNo


             FROM RestructureAssets V  
             WHERE ISNULL(PrinRpymntStartDate,'')='' OR len(isnull(PrinRpymntStartDate,''))=0
             */
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
             ...
             WHERE  NVL(PrinRpymntStartDate, ' ') <> ' '
              AND PrinRpymntStartDate NOT LIKE '__-__-____'
            ...
            UPDATE RestructureAssets
            	SET  
                    ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'InttRpymntStartDate Can not be Blank or Null or Zero . Please enter the InttRpymntStartDate and upload again'     
            						ELSE ErrorMessage+','+SPACE(1)+ 'InttRpymntStartDate Can not be Blank  or Null or Zero . Please enter the InttRpymntStartDate and upload again'      END
            		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'InttRpymntStartDate' ELSE   ErrorinColumn +','+SPACE(1)+'InttRpymntStartDate' END      
            		,Srnooferroneousrows=V.SrNo


             FROM RestructureAssets V  
             WHERE ISNULL(InttRpymntStartDate,'')='' OR len(isnull(InttRpymntStartDate,''))=0
             */
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
             ...
             WHERE  NVL(InttRpymntStartDate, ' ') <> ' '
              AND InttRpymntStartDate NOT LIKE '__-__-____'
            ...
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
                                         WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'AssetClassatRstrctr Can not be Blank . Please enter the AssetClassatRstrctr and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'AssetClassatRstrctr Can not be Blank. Please enter the AssetClassatRstrctr and upload again'
                      END,
                   V.ErrorinColumn = CASE 
            ...
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
                   ...
                   V.Srnooferroneousrows = V.SrNo
             WHERE  NVL(AssetClassatRstrctr, ' ') <> ' '
              AND NVL(AssetClassatRstrctr, ' ') NOT IN ( 'STANDARD','SUB-STANDARD','DOUBTFUL I','DOUBTFUL II','DOUBTFUL III','LOS' )
            ;
            ...
            UPDATE RestructureAssets
            	SET  
                    ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'NPADate Can not be Blank or Null or Zero. Please enter the NPADate and upload again'     
            						ELSE ErrorMessage+','+SPACE(1)+ 'NPADate Can not be Blank or Null or Zero. Please enter the NPADate and upload again'      END
            		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'NPADate' ELSE   ErrorinColumn +','+SPACE(1)+'NPADate' END      
            		,Srnooferroneousrows=V.SrNo

             FROM RestructureAssets V  
             WHERE ISNULL(NPADate,'')='' OR len(isnull(NPADate,''))=0
             */
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
                   ...
                   V.Srnooferroneousrows = V.SrNo
             WHERE  NVL(InttRpymntStartDate, ' ') <> ' '
              AND NPADate NOT LIKE '__-__-____'
            ...
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
                   ...
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'DateofRestructuring'
                      END,
                   V.Srnooferroneousrows = V.SrNo
             WHERE  NVL(V.DateofRestructuring, ' ') <> ' '
              AND DateofRestructuring NOT LIKE '__-__-____'
            ...
            BEGIN
               UPDATE RestructureAssets V
                  SET V.ErrorMessage = CASE 
                      ...
                      V.Srnooferroneousrows = V.SrNo
                WHERE  NVL(V.DateofRestructuring, ' ') <> ' '
                 AND ( SUBSTR(DateofRestructuring, 0, 2) NOT BETWEEN 1 AND 31 );
               UPDATE RestructureAssets V
                  SET V.ErrorMessage = CASE 
                      ...
                      V.Srnooferroneousrows = V.SrNo
                WHERE  NVL(V.DateofRestructuring, ' ') <> ' '
                 AND ( SUBSTR(SUBSTR(DateofRestructuring, 0, 5), -2, 2) NOT BETWEEN 1 AND 12 );
            ...
            UPDATE RestructureAssets
            	SET  
                    ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'Date of Ist Defaulton CRILIC can not be blank or Null or Zero. Please check and Upload again  '     
            						ELSE ErrorMessage+','+SPACE(1)+ 'Date of Ist Defaulton CRILIC can not be blank or Zero or Null. Please check and Upload again '      END
             ...
             FROM RestructureAssets V  
              WHERE ISNULL(DateofIstDefaultonCRILIC,'')='' OR len(isnull(DateofIstDefaultonCRILIC,''))=0
            ...
            UPDATE RestructureAssets V
             ...
             WHERE  NVL(DateofIstDefaultonCRILIC, ' ') <> ' '
              AND DateofIstDefaultonCRILIC NOT LIKE '__-__-____'
              AND utils.isdate(DateofIstDefaultonCRILIC) <> 1;
            BEGIN
               SELECT 1 INTO v_temp
                 FROM DUAL
                WHERE EXISTS ( SELECT 1 
                               FROM RestructureAssets 
                                WHERE  DateofIstDefaultonCRILIC LIKE '__-__-____'
                                         AND utils.isdate(DateofIstDefaultonCRILIC) = 1 );
            EXCEPTION
               WHEN OTHERS THEN
                  NULL;
            ...
            BEGIN
               UPDATE RestructureAssets V
                  SET V.ErrorMessage = CASE 
                                            WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Invalid date format. Please enter the date in format ‘dd-mm-yyyy’  Day part in DateofIstDefaultonCRILIC not between in 1 to 31'
                      ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Invalid date format. Please enter the date in format ‘dd-mm-yyyy’  Day part in DateofIstDefaultonCRILIC not between in 1 to 31'
                         END,
                      V.ErrorinColumn = CASE 
                                             WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'DateofIstDefaultonCRILIC'
                      ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'DateofIstDefaultonCRILIC'
               ...
               UPDATE RestructureAssets V
                  SET V.ErrorMessage = CASE 
                                            WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Invalid date format. Please enter the date in format ‘dd-mm-yyyy’ Month part in DateofIstDefaultonCRILIC will be in 1 to 12'
                      ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'Invalid date format. Please enter the date in format ‘dd-mm-yyyy’ Month part in DateofIstDefaultonCRILIC will be in 1 to 12'
                         END,
                      V.ErrorinColumn = CASE 
                                             WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'DateofIstDefaultonCRILIC'
                      ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'DateofIstDefaultonCRILIC'
            ...
            */
            ...
            UPDATE RestructureAssets
            	SET  
                    ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'DateofSigningICA Can not be Blank or Zero or Null . Please enter the DateofSigningICA and upload again'     
            						ELSE ErrorMessage+','+SPACE(1)+ 'DateofSigningICA Can not be Blank or Zero or Null. Please enter the DateofSigningICA and upload again'      END
            		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'DateofSigningICA' ELSE   ErrorinColumn +','+SPACE(1)+'DateofSigningICA' END      
            		,Srnooferroneousrows=V.SrNo

             FROM RestructureAssets V  
             WHERE  ISNULL(DateofSigningICA,'')='' OR len(isnull(DateofSigningICA,''))=0
             */
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
                                         WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'Invalid date format. Please enter the date in format ‘DD-MM-YYYY’'
             ...
             WHERE  NVL(DateofSigningICA, ' ') <> ' '
              AND DateofSigningICA NOT LIKE '__-__-____'
            ...
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
                                         WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'OS as on date of Rstrctr cannot be blank . Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'OS as on date of Rstrctr cannot be blank . Please check the values and upload again'
                      END,
                   V.ErrorinColumn = CASE 
                                          WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'OS as on date of Rstrctr'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'OS as on date'
                      END,
                   V.Srnooferroneousrows = V.SrNo
                   --STUFF((SELECT ','+SRNO 
                    --FROM RestructureAssets A
            ...
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
                                         WHEN NVL(ErrorMessage, ' ') = ' ' THEN 'POS as on date of Rstrctr cannot be blank . Please check the values and upload again'
                   ELSE ErrorMessage || ',' || LPAD(' ', 1, ' ') || 'POS as on date of Rstrctr cannot be blank . Please check the values and upload again'
                      END,
                   V.ErrorinColumn = CASE 
                                          WHEN NVL(ErrorinColumn, ' ') = ' ' THEN 'POS as on date of Rstrctr'
                   ELSE ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'POS as on date'
                      END,
                   V.Srnooferroneousrows = V.SrNo
                   --STUFF((SELECT ','+SRNO 
                    --FROM RestructureAssets A
             ...
             WHERE  NVL(POSasonDateofRstrctr, ' ') = ' ';
            UPDATE RestructureAssets V
               SET V.ErrorMessage = CASE 
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_DATAUPLOAD_RESTRUCTUREASSETSUPLOAD" TO "ADF_CDR_RBL_STGDB";
