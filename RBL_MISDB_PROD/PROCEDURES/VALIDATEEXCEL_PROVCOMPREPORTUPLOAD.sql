--------------------------------------------------------
--  DDL for Procedure VALIDATEEXCEL_PROVCOMPREPORTUPLOAD
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" 
(
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF ( v_MenuID = 27769 ) THEN
          DECLARE
            ...
            /* --------------------------------------------------------------------------------------------------------------------------------------------------------------
            			...
            			UPDATE tt_UploadProvCompReport SET    
            			...
            			FROM tt_UploadProvCompReport V    
         ...
         BEGIN
            ...
            END;
            END IF;
            IF utils.object_id('TEMPDB..tt_UploadProvCompReport') IS NOT NULL THEN

            BEGIN
               EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UploadProvCompReport ';
            ...
            BEGIN
               ...
               DELETE FROM tt_UploadProvCompReport;
               UTILS.IDENTITY_RESET('tt_UploadProvCompReport');

               INSERT INTO tt_UploadProvCompReport ( 
               	SELECT * ,
            ...
            BEGIN
               UPDATE tt_UploadProvCompReport V
               ...
               BEGIN
                  DELETE tt_UploadProvCompReport
                  ;
                  DBMS_OUTPUT.PUT_LINE(1);
                  DBMS_OUTPUT.PUT_LINE('ROWS DELETED FROM tt_UploadProvCompReport' || UTILS.CONVERT_TO_VARCHAR2(SQL%ROWCOUNT,100));
            ...
            END IF;
            --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            UPDATE tt_UploadProvCompReport 
            ...
            UPDATE tt_UploadProvCompReport V
             ...
             WHERE  NVL(SR_No, ' ') <> ' '
              ...
              AND NVL(ACCOUNT_Id, ' ') = ' '
              AND NVL(UserLoginId, ' ') <> ' ';
            --IF EXISTS(SELECT 1 FROM tt_UploadProvCompReport WHERE ISNULL(ErrorMessage,'')<>'')  
            --BEGIN  
            --	PRINT 'NO DATA'  
            --	GOTO ERRORDATA;  
            --END
            ...
            --SELECT DISTINCT UserLoginId INTO #DistinctUserId FROM tt_UploadProvCompReport
            ...
            ;
            IF utils.object_id('TEMPDB..tt_DUB_UCIF_5') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_UCIF_5 ';
            END IF;
            --SELECT * FROM tt_UploadProvCompReport
            DELETE FROM tt_DUB_UCIF_5;
            UTILS.IDENTITY_RESET('tt_DUB_UCIF_5');

            INSERT INTO tt_DUB_UCIF_5 SELECT * 
                 FROM ( SELECT * ,
                               ROW_NUMBER() OVER ( PARTITION BY UCIC_ID ORDER BY UCIC_ID  ) rw  
                        FROM tt_UploadProvCompReport  ) X
                WHERE  rw > 1;
            MERGE INTO V 
            ...
            FROM V ,tt_UploadProvCompReport V
                   JOIN tt_DUB_UCIF_5 D   ON D.UCIC_Id = V.UCIC_Id ) src
              ...
              AND V.CIF_Id NOT IN ( SELECT CustomerId 
                                    FROM CustomerBasicDetail 
                                     WHERE  ( EffectiveFromTimeKey <= v_Timekey
                                              AND EffectiveToTimeKey >= v_Timekey ) )
            ;
            IF utils.object_id('TEMPDB..tt_DUB_CIF_5') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_CIF_5 ';
            END IF;
            DELETE FROM tt_DUB_CIF_5;
            UTILS.IDENTITY_RESET('tt_DUB_CIF_5');

            INSERT INTO tt_DUB_CIF_5 SELECT * 
                 FROM ( SELECT * ,
            ...
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'CUSTOMER ID'
               END AS pos_3, V.SR_No
            FROM V ,tt_UploadProvCompReport V
                   JOIN tt_DUB_CIF_5 D   ON D.CIF_ID = V.CIF_ID ) src
            ON ( V.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET ErrorMessage = pos_2,
                                         ErrorinColumn = pos_3,
                                         Srnooferroneousrows = src.SR_No;
              ...
              AND V.ACCOUNT_Id NOT IN ( SELECT CustomerACID 
                                        FROM AdvAcBasicDetail 
                                         WHERE  ( EffectiveFromTimeKey <= v_Timekey
                                                  AND EffectiveToTimeKey >= v_Timekey ) )
            ;
            IF utils.object_id('TEMPDB..tt_DUB_ACCOUNTID_5') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_ACCOUNTID_5 ';
            END IF;
            DELETE FROM tt_DUB_ACCOUNTID_5;
            UTILS.IDENTITY_RESET('tt_DUB_ACCOUNTID_5');

            INSERT INTO tt_DUB_ACCOUNTID_5 SELECT * 
                 FROM ( SELECT * ,
            ...
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'ACCOUNT ID'
               END AS pos_3, V.SR_No
            FROM V ,tt_UploadProvCompReport V
                   JOIN tt_DUB_ACCOUNTID_5 D   ON D.ACCOUNT_Id = V.ACCOUNT_Id ) src
            ON ( V.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET ErrorMessage = pos_2,
                                         ErrorinColumn = pos_3,
                                         Srnooferroneousrows = src.SR_No;
         ...
         --	IF EXISTS(SELECT 1 FROM tt_UploadProvCompReport)
         --	BEGIN
         --		DELETE FROM tt_UploadProvCompReport
         --		PRINT 1
         --		PRINT 'ROWS DELETED FROM tt_UploadProvCompReport'+CAST(@@ROWCOUNT AS VARCHAR(100))
         --	END
         --	SELECT 1
         --	SELECT 1
         ...
         --END
         ------------------------------------------------------------------------------------------------------------------  
         IF utils.object_id('TEMPDB..tt_Temp_Success_2') IS NOT NULL THEN
          --GROUP BY FileNames  
         ...
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Temp_Success_2 ';
         END IF;
         DELETE FROM tt_Temp_Success_2;
         UTILS.IDENTITY_RESET('tt_Temp_Success_2');

         INSERT INTO tt_Temp_Success_2 ( 
         	SELECT COUNT(*)  COUNT  ,
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_PROVCOMPREPORTUPLOAD" TO "ADF_CDR_RBL_STGDB";
