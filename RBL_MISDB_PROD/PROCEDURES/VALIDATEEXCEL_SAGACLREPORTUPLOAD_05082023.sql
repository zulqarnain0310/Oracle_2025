--------------------------------------------------------
--  DDL for Procedure VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" 
(
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF ( v_MenuID = 24754 ) THEN
          DECLARE
            ...
            /*  --------------------------------------------------------------------------------------------------------------------------------------------------------------  
               ...
               UPDATE tt_UploadSAGACLReport_4 SET      
               ...
               FROM tt_UploadSAGACLReport_4 V      
         ...
         BEGIN
            ...
            END;
            END IF;
            IF utils.object_id('TEMPDB..tt_UploadSAGACLReport_4') IS NOT NULL THEN

            BEGIN
               EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UploadSAGACLReport_4 ';

            END;
            END IF;
            --drop table IF exists tt_UploadSAGACLReport_4  
            BEGIN
               SELECT 1 INTO v_temp
                 FROM DUAL
            ...
            BEGIN
               ...
               DELETE FROM tt_UploadSAGACLReport_4;
               UTILS.IDENTITY_RESET('tt_UploadSAGACLReport_4');

               INSERT INTO tt_UploadSAGACLReport_4 ( 
               	SELECT * ,
            ...
            END;
            END IF;
            UPDATE tt_UploadSAGACLReport_4 
             ...
             WHERE  NVL(SR_No, ' ') = ' '
              ...
              AND NVL(ACCOUNT_Id, ' ') = ' '
              AND NVL(UserLoginId, ' ') = ' ';
            UPDATE tt_UploadSAGACLReport_4 V
            ...
            UPDATE tt_UploadSAGACLReport_4 V
             ...
             WHERE  NVL(SR_No, ' ') <> ' '
              ...
              AND NVL(ACCOUNT_Id, ' ') = ' '
              AND NVL(UserLoginId, ' ') <> ' ';
            --IF EXISTS(SELECT 1 FROM tt_UploadSAGACLReport_4 WHERE ISNULL(ErrorMessage,'')<>'')    
            --BEGIN    
            -- PRINT 'NO DATA'    
            -- GOTO ERRORDATA;    
            --END  
            ...
            --SELECT DISTINCT UserLoginId INTO #DistinctUserId FROM tt_UploadSAGACLReport_4  
            ...
            ;
            IF utils.object_id('TEMPDB..tt_DUB_UCIF_11') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_UCIF_11 ';
            END IF;
            --SELECT * FROM tt_UploadSAGACLReport_4  
            DELETE FROM tt_DUB_UCIF_11;
            UTILS.IDENTITY_RESET('tt_DUB_UCIF_11');

            INSERT INTO tt_DUB_UCIF_11 SELECT * 
                 FROM ( SELECT * ,
                               ROW_NUMBER() OVER ( PARTITION BY UCIC_ID ORDER BY UCIC_ID  ) rw  
                        FROM tt_UploadSAGACLReport_4  ) X
                WHERE  rw > 1;
            MERGE INTO V 
            ...
            FROM V ,tt_UploadSAGACLReport_4 V
                   JOIN tt_DUB_UCIF_11 D   ON D.UCIC_Id = V.UCIC_Id ) src
              ...
              AND V.CIF_Id NOT IN ( SELECT CustomerId 
                                    FROM CustomerBasicDetail 
                                     WHERE  ( EffectiveFromTimeKey <= v_Timekey
                                              AND EffectiveToTimeKey >= v_Timekey ) )
            ;
            IF utils.object_id('TEMPDB..tt_DUB_CIF_11') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_CIF_11 ';
            END IF;
            DELETE FROM tt_DUB_CIF_11;
            UTILS.IDENTITY_RESET('tt_DUB_CIF_11');

            INSERT INTO tt_DUB_CIF_11 SELECT * 
                 FROM ( SELECT * ,
            ...
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'CUSTOMER ID'
               END AS pos_3, V.SR_No
            FROM V ,tt_UploadSAGACLReport_4 V
                   JOIN tt_DUB_CIF_11 D   ON D.CIF_ID = V.CIF_ID ) src
            ON ( V.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET ErrorMessage = pos_2,
                                         ErrorinColumn = pos_3,
                                         Srnooferroneousrows = src.SR_No;
              ...
              AND V.ACCOUNT_Id NOT IN ( SELECT CustomerACID 
                                        FROM AdvAcBasicDetail 
                                         WHERE  ( EffectiveFromTimeKey <= v_Timekey
                                                  AND EffectiveToTimeKey >= v_Timekey ) )
            ;
            IF utils.object_id('TEMPDB..tt_DUB_ACCOUNTID_11') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DUB_ACCOUNTID_11 ';
            END IF;
            DELETE FROM tt_DUB_ACCOUNTID_11;
            UTILS.IDENTITY_RESET('tt_DUB_ACCOUNTID_11');

            INSERT INTO tt_DUB_ACCOUNTID_11 SELECT * 
                 FROM ( SELECT * ,
            ...
            ELSE V.ErrorinColumn || ',' || LPAD(' ', 1, ' ') || 'ACCOUNT ID'
               END AS pos_3, V.SR_No
            FROM V ,tt_UploadSAGACLReport_4 V
                   JOIN tt_DUB_ACCOUNTID_11 D   ON D.ACCOUNT_Id = V.ACCOUNT_Id ) src
            ON ( V.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET ErrorMessage = pos_2,
                                         ErrorinColumn = pos_3,
                                         Srnooferroneousrows = src.SR_No;
            ...
            BEGIN
               ...
               BEGIN
                  DELETE tt_UploadSAGACLReport_4
                  ;
                  DBMS_OUTPUT.PUT_LINE(1);
                  DBMS_OUTPUT.PUT_LINE('ROWS DELETED FROM tt_UploadSAGACLReport_4' || UTILS.CONVERT_TO_VARCHAR2(SQL%ROWCOUNT,100));
            ...
            BEGIN
               ...
               OPEN  v_cursor FOR
                  SELECT 5 
                    FROM DUAL  ;
                  DBMS_SQL.RETURN_RESULT(v_cursor);
               EXECUTE IMMEDIATE ' TRUNCATE TABLE IRAC_Total_ACL_UploadCount ';
               INSERT INTO IRAC_Total_ACL_UploadCount
                 ...
                 ( SELECT COUNT(*)  COUNT  ,
                   ...
                   FROM RBL_MISDB_PROD.MasterUploadData_SAGACLProvDPD 
                    WHERE  FileNames = v_FileName
                             AND Flag = 'SUCCESS'
                     GROUP BY FileNames );
               IF utils.object_id('TEMPDB..tt_InvalidRecords') IS NOT NULL THEN
                EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_InvalidRecords ';
               END IF;
               DELETE FROM tt_InvalidRecords;
               INSERT INTO tt_InvalidRecords
                 ( SR_NO, Ucic_Id, Customer_Id, Account_Id )
                 ( SELECT SR_NO ,
                          Ucif_Id ,
                          CustomerId ,
                          CustomerACID 
               ...
               ( SELECT A.ROWID
                 FROM DPDCSVUpload A
                        LEFT JOIN tt_InvalidRecords B   ON NVL(A.SR_No, ' ') = NVL(B.SR_NO, ' ')
                        OR NVL(A.UCIC_Id, ' ') = NVL(B.Ucic_Id, ' ')
                        OR NVL(A.CIF_Id, ' ') = NVL(B.Customer_Id, ' ')
                        OR NVL(A.ACCOUNT_Id, ' ') = NVL(B.Account_Id, ' '),
                      A );
         ...
         -- IF EXISTS(SELECT 1 FROM tt_UploadSAGACLReport_4)  
         -- BEGIN  
         --  DELETE FROM tt_UploadSAGACLReport_4  
         --  PRINT 1  
         --  PRINT 'ROWS DELETED FROM tt_UploadSAGACLReport_4'+CAST(@@ROWCOUNT AS VARCHAR(100))  
         -- END  
         -- SELECT 1  
         -- SELECT 1  
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."VALIDATEEXCEL_SAGACLREPORTUPLOAD_05082023" TO "ADF_CDR_RBL_STGDB";
