--------------------------------------------------------
--  DDL for Function PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" 
(
...
BEGIN
   ...
   --)
   --------------------------------------------------------------------------------------------------------
   IF ( utils.object_id('TEMPDB..tt_ADVACRESTRUCTUREDETAIL_13') IS NOT NULL ) THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ADVACRESTRUCTUREDETAIL_13 ';
   END IF;
   DELETE FROM tt_ADVACRESTRUCTUREDETAIL_13;
   UTILS.IDENTITY_RESET('tt_ADVACRESTRUCTUREDETAIL_13');

   INSERT INTO tt_ADVACRESTRUCTUREDETAIL_13 SELECT * 
        FROM ( SELECT * ,
   ...
   ------------------------------------------------------------------------
   ---------------------------------------------------------------------------------------------
   IF ( utils.object_id('TEMPDB..tt_PrevQtrDataAccountWise_11') IS NOT NULL ) THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PrevQtrDataAccountWise_11 ';
   END IF;
   DELETE FROM tt_PrevQtrDataAccountWise_11;
   UTILS.IDENTITY_RESET('tt_PrevQtrDataAccountWise_11');

   INSERT INTO tt_PrevQtrDataAccountWise_11 ( 
   ...
   DELETE FROM tt_AccountEntityID;
   --and B.FinalAssetClassAlt_Key>1 
   -------------------------------------------------------------------------------------------------
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_AccountEntityID  --SQLDEV: NOT RECOGNIZED
   -----------------------------------------------------------------------------------------------------
   BEGIN
   ...
   BEGIN
      IF utils.object_id('TEMPDB..tt_TEMP11') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMP11 ';
      END IF;
      --insert into  tt_AccountEntityID
      ...
      INSERT INTO tt_AccountEntityID
        ( SELECT DISTINCT AccountEntityId 
          FROM AdvAcBasicDetail a
                 JOIN CustomerBasicDetail b   ON a.CustomerEntityId = b.CustomerEntityId
                 JOIN tt_TEMP11 c   ON b.UcifEntityID = c.UcifEntityID
           WHERE  a.EffectiveFromTimeKey <= v_Timekey
                    AND a.EffectiveToTimeKey >= v_Timekey
                    AND b.EffectiveFromTimeKey <= v_Timekey
                    AND b.EffectiveToTimeKey >= v_Timekey );
   ...
   BEGIN
      IF utils.object_id('TEMPDB..tt_TEMP12') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMP12 ';
      END IF;
      --insert into  tt_AccountEntityID
      --Select       distinct top 1000000  AccountEntityId  
      --from         AdvAcBasicDetail a
      ...
      --where        EffectiveFromTimeKey <= @Timekey and EffectiveToTimeKey>=@Timekey
      DELETE FROM tt_TEMP12;
      UTILS.IDENTITY_RESET('tt_TEMP12');

      INSERT INTO tt_TEMP12 ( 
      ...
      INSERT INTO tt_AccountEntityID
        ( SELECT DISTINCT AccountEntityId 
          FROM AdvAcBasicDetail a
                 JOIN CustomerBasicDetail b   ON a.CustomerEntityId = b.CustomerEntityId
                 JOIN tt_TEMP12 c   ON b.UcifEntityID = c.UcifEntityID
           WHERE  a.EffectiveFromTimeKey <= v_Timekey
                    AND a.EffectiveToTimeKey >= v_Timekey
                    AND b.EffectiveFromTimeKey <= v_Timekey
                    AND b.EffectiveToTimeKey >= v_Timekey );
   ...
   --DECLARE @Timekey int =   
   --(26376)  
   IF ( utils.object_id('TEMPDB..tt_PrevQtrData_19') IS NOT NULL ) THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PrevQtrData_19 ';
   END IF;
   DELETE FROM tt_PrevQtrData_19;
   UTILS.IDENTITY_RESET('tt_PrevQtrData_19');

   INSERT INTO tt_PrevQtrData_19 ( 
   	SELECT B.ACCOUNTENTITYID ,
   	  ...
   	  FROM PRO_RBL_MISDB_PROD.AccountCal_Hist B
             JOIN tt_AccountEntityID c   ON B.AccountEntityID = c.AccountEntityID
   	 WHERE  EffectiveFromTimeKey <= v_LastQtrDateKey
              AND EffectiveToTimeKey >= v_LastQtrDateKey );
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_DPD2  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_DPD2;
   UTILS.IDENTITY_RESET('tt_DPD2');
   ...
   EXECUTE IMMEDIATE ' ALTER TABLE tt_DPD2 
   ...
   --UPDATE A SET DPD_NoCredit=0 FROM tt_DPD2 A   
   /* CALCULATE MAX DPD */
   IF utils.object_id('TEMPDB..tt_TEMPTABLE_103') IS NOT NULL THEN
    --Truncate table  ProvisionComputationCSVUpload
   EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_103 ';
   END IF;
   DELETE FROM tt_TEMPTABLE_103;
   UTILS.IDENTITY_RESET('tt_TEMPTABLE_103');

   INSERT INTO tt_TEMPTABLE_103 ( 
     ...
     SELECT UTILS.CONVERT_TO_NVARCHAR2(v_Date,30,p_style=>105) Report_Date  ,
       ...
       FROM PRO_RBL_MISDB_PROD.CustomerCal_Hist A
            ...
            --AND ISNULL(B.WriteOffAmount,0)=0  
              ...
              LEFT JOIN tt_PrevQtrDataAccountWise_11 Y   ON A.CustomerEntityID = Y.CustomerEntityID
              LEFT JOIN tt_DPD2 DPD   ON B.AccountEntityID = DPD.AccountEntityID
              LEFT JOIN tt_PrevQtrData_19 prev   ON B.AccountEntityID = PREV.ACCOUNTENTITYID
       ...
       AND B.ACID = A.ACID
              ...
              LEFT JOIN tt_ADVACRESTRUCTUREDETAIL_13 RES   ON RES.AccountEntityId = B.AccountEntityID
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."PROVISIONCOMPUTATIONREPORT_FRONTEND_REPORT" TO "ADF_CDR_RBL_STGDB";
