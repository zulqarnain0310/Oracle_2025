--------------------------------------------------------
--  DDL for Function RESTRUCTUREASSETS_UPLOADDATAINUP_NEW_K
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "RBL_MISDB_PROD"."RESTRUCTUREASSETS_UPLOADDATAINUP_NEW_K" 
(
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  FROM A ,CurDat_RBL_MISDB_PROD.AdvAcRestructureDetail A
                         ...
                         AND B.EffectiveToTimeKey >= v_Timekey ---------Newly added on 13/06/2024 by Kapil

                         AND B.AuthorisationStatus = 'A' ---------Newly added on 13/06/2024 by Kapil

                   WHERE B.UploadID = v_UniqueUploadID
                    AND B.Action = 'Y'
                    AND A.AuthorisationStatus = 'A') src
                  ON ( A.ROWID = src.row_id )
                  WHEN MATCHED THEN UPDATE SET A.EffectiveToTimeKey = src.EffectiveToTimeKey;---------Newly added on 13/06/2024 by Kapil
                    ...
                    ( SELECT DISTINCT AccountID ,
                      ...
                      FROM RestructureAsset_Upload_Mod A
                       ...
                       WHERE  A.UploadID = v_UniqueUploadID
                                AND A.EffectiveToTimeKey >= v_Timekey
                                AND a.Action = 'Y' );
                  --VVVVV----------------Newly added on 13/06/2024 by Kapil
                  MERGE INTO B 
                  USING (SELECT B.ROWID row_id, v_Timekey - 1 AS pos_2, CASE 
                  WHEN v_AUTHMODE = 'Y' THEN 'A'
                  ELSE NULL
                     END AS pos_3
                  FROM B ,RestructureAsset_Upload_Mod A
                         JOIN CurDat_RBL_MISDB_PROD.AdvAcRestructureDetail B   ON A.AccountID = B.RefSystemAcId
                         AND B.EffectiveFromTimeKey <= v_timekey
                         AND B.EffectiveToTimeKey >= v_Timekey 
                   WHERE A.UploadID = v_UniqueUploadID
                    AND A.Action = 'N') src
                  ON ( B.ROWID = src.row_id )
                  WHEN MATCHED THEN UPDATE SET B.EffectiveToTimeKey = pos_2,
                                               B.AuthorisationStatus = pos_3;
                  --^^^^^----------------Newly added on 13/06/2024 by Kapil
                  UPDATE ExcelUploadHistory
                     SET AuthorisationStatus = 'A',
                         ApprovedBy = v_UserLoginID,
                         DateApproved = SYSDATE
