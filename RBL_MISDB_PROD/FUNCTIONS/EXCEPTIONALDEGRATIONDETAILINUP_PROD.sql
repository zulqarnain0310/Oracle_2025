--------------------------------------------------------
--  DDL for Function EXCEPTIONALDEGRATIONDETAILINUP_PROD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" 
-- =============================================
 -- Author:				<Amar>
                              ...
                              ELSE
                                 ...
                                 BEGIN
                                    ...
                                    BEGIN
                                       ...
                                       BEGIN
                                          IF utils.object_id('TempDB..tt_Flags_23') IS NOT NULL THEN
                                           EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Flags_23 ';
                                          END IF;
                                          DELETE FROM tt_Flags_23;
                                          UTILS.IDENTITY_RESET('tt_Flags_23');

                                          INSERT INTO tt_Flags_23 ( 
                                          	SELECT A.AccountID ,
                                          ...
                                          FROM A ,RBL_MISDB_PROD.AdvAcOtherDetail A
                                                 JOIN tt_Flags_23 B   ON A.RefSystemAcId = B.AccountID 
                                       ...
                                       BEGIN
                                          ...
                                          ON ( B.ROWID = src.row_id )
                                          WHEN MATCHED THEN UPDATE SET B.EffectiveToTimeKey = src.EffectiveToTimeKey;
                                          IF utils.object_id('TempDB..tt_Flags_231') IS NOT NULL THEN
                                           EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Flags1_23 ';
                                          END IF;
                                          DELETE FROM tt_Flags1_23;
                                          UTILS.IDENTITY_RESET('tt_Flags1_23');

                                          INSERT INTO tt_Flags1_23 ( 
                                          	SELECT A.AccountID ,
                                                  B.SplCatShortNameEnum 
                                          	  FROM ExceptionalDegrationDetail_Mod A
                                                    JOIN ( SELECT B.ParameterAlt_Key ,
                                          	 ...
                                          	 WHERE  A.EffectiveToTimeKey = 49999
                                                     ...
                                                     AND A.Entity_Key = ( SELECT MAX(Entity_Key)  
                                                                          FROM ExceptionalDegrationDetail_Mod 
                                                                           WHERE  AccountID = v_AccountID
                                                                                    AND FlagAlt_Key = v_FlagAlt_Key ) );
                                          IF utils.object_id('TempDB..tt_Temp_77') IS NOT NULL THEN
                                           EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Temp_77 ';
                                          END IF;
                                          DELETE FROM tt_Temp_77;
                                          UTILS.IDENTITY_RESET('tt_Temp_77');

                                          INSERT INTO tt_Temp_77 ( 
                                          	SELECT A.AccountentityID ,
                                                  A.SplFlag 
                                          	  FROM CurDat_RBL_MISDB_PROD.AdvAcOtherDetail A
                                                    JOIN tt_Flags1_23 B   ON A.RefSystemAcId = B.AccountID
                                          	 WHERE  A.EffectiveToTimeKey = 49999 );
                                          --Select * from tt_Temp_77
                                          IF utils.object_id('TEMPDB..tt_SplitValue_25') IS NOT NULL THEN
                                           EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_SplitValue_25 ';
                                          END IF;
                                          DELETE FROM tt_SplitValue_25;
                                          UTILS.IDENTITY_RESET('tt_SplitValue_25');

                                          INSERT INTO tt_SplitValue_25 ( 
                                          ...
                                          --Select * from tt_SplitValue_25 
                                          DELETE tt_SplitValue_25

                                           WHERE  Businesscolvalues1 IN ( SELECT DISTINCT SplCatShortNameEnum 
                                                                          FROM tt_Flags1_23  )
                                          ;
                                          IF utils.object_id('TEMPDB..tt_NEWTRANCHE_68') IS NOT NULL THEN
                                           EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_NEWTRANCHE_68 ';
                                          END IF;
                                          DELETE FROM tt_NEWTRANCHE_68;
                                          UTILS.IDENTITY_RESET('tt_NEWTRANCHE_68');

                                          INSERT INTO tt_NEWTRANCHE_68 SELECT * 
                                               FROM ( SELECT ss.AccountEntityID ,
                                                             utils.stuff(( SELECT ',' || US.BUSINESSCOLVALUES1 
                                                                           FROM tt_SplitValue_25 US
                                                                            WHERE  US.AccountentityID = ss.AccountEntityID ), 1, 1, ' ') REPORTIDSLIST  
                                                      FROM tt_Temp_77 SS
                                                        GROUP BY ss.AccountEntityID ) B
                                               ORDER BY 1;
                                          --Select * from tt_NEWTRANCHE_68
                                          --SELECT * 
                                          MERGE INTO A 
                                          USING (SELECT A.ROWID row_id, B.REPORTIDSLIST
                                          FROM A ,RBL_MISDB_PROD.AdvAcOtherDetail A
                                                 JOIN tt_NEWTRANCHE_68 B   ON A.AccountentityID = B.AccountentityID 
                                           WHERE A.EFFECTIVEFROMTIMEKEY <= v_TimeKey
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP_PROD" TO "ADF_CDR_RBL_STGDB";
