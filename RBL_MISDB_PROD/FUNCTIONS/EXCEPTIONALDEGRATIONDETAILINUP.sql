--------------------------------------------------------
--  DDL for Function EXCEPTIONALDEGRATIONDETAILINUP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" 
(
  --@AccountID			INT				= 0	
         ...
         BEGIN
                              ...
                              ELSE
                                 ...
                                 BEGIN
                                    ...
                                    BEGIN
                                       ...
                                       BEGIN
                                          IF utils.object_id('TempDB..tt_Flags_15') IS NOT NULL THEN
                                           EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Flags_15 ';
                                          END IF;
                                          DELETE FROM tt_Flags_15;
                                          UTILS.IDENTITY_RESET('tt_Flags_15');

                                          INSERT INTO tt_Flags_15 ( 
                                          	SELECT A.AccountID ,
                                          ...
                                          FROM A ,RBL_MISDB_PROD.AdvAcOtherDetail A
                                                 JOIN tt_Flags_15 B   ON A.RefSystemAcId = B.AccountID 
                                       ...
                                       BEGIN
                                          ...
                                          IF utils.object_id('TempDB..tt_Flags_151') IS NOT NULL THEN
                                           EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Flags1_15 ';
                                          END IF;
                                          DELETE FROM tt_Flags1_15;
                                          UTILS.IDENTITY_RESET('tt_Flags1_15');

                                          INSERT INTO tt_Flags1_15 ( 
                                          	SELECT A.AccountID ,
                                                  B.SplCatShortNameEnum 
                                          	  FROM ExceptionalDegrationDetail_Mod A
                                                    JOIN ( SELECT B.ParameterAlt_Key ,
                                          	 ...
                                          	 WHERE  A.EffectiveToTimeKey = 49999
                                                     ...
                                                     AND A.Entity_Key = ( SELECT MAX(Entity_Key)  
                                                                          FROM ExceptionalDegrationDetail_Mod 
                                                                           WHERE  AccountID = v_AccountID
                                                                                    AND FlagAlt_Key = v_FlagAlt_Key ) );
                                          IF utils.object_id('TempDB..tt_Temp_69') IS NOT NULL THEN
                                           EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Temp_69 ';
                                          END IF;
                                          DELETE FROM tt_Temp_69;
                                          UTILS.IDENTITY_RESET('tt_Temp_69');

                                          INSERT INTO tt_Temp_69 ( 
                                          	SELECT A.AccountentityID ,
                                                  A.SplFlag 
                                          	  FROM CurDat_RBL_MISDB_PROD.AdvAcOtherDetail A
                                                    JOIN tt_Flags1_15 B   ON A.RefSystemAcId = B.AccountID
                                          	 WHERE  A.EffectiveToTimeKey = 49999 );
                                          --Select * from tt_Temp_69
                                          IF utils.object_id('TEMPDB..tt_SplitValue_17') IS NOT NULL THEN
                                           EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_SplitValue_17 ';
                                          END IF;
                                          DELETE FROM tt_SplitValue_17;
                                          UTILS.IDENTITY_RESET('tt_SplitValue_17');

                                          INSERT INTO tt_SplitValue_17 ( 
                                          ...
                                          --Select * from tt_SplitValue_17 
                                          DELETE tt_SplitValue_17

                                           WHERE  Businesscolvalues1 IN ( SELECT DISTINCT SplCatShortNameEnum 
                                                                          FROM tt_Flags1_15  )
                                          ;
                                          IF utils.object_id('TEMPDB..tt_NEWTRANCHE_60') IS NOT NULL THEN
                                           EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_NEWTRANCHE_60 ';
                                          END IF;
                                          DELETE FROM tt_NEWTRANCHE_60;
                                          UTILS.IDENTITY_RESET('tt_NEWTRANCHE_60');

                                          INSERT INTO tt_NEWTRANCHE_60 SELECT * 
                                               FROM ( SELECT ss.AccountEntityID ,
                                                             utils.stuff(( SELECT ',' || US.BUSINESSCOLVALUES1 
                                                                           FROM tt_SplitValue_17 US
                                                                            WHERE  US.AccountentityID = ss.AccountEntityID ), 1, 1, ' ') REPORTIDSLIST  
                                                      FROM tt_Temp_69 SS
                                                        GROUP BY ss.AccountEntityID ) B
                                               ORDER BY 1;
                                          --Select * from tt_NEWTRANCHE_60
                                          --SELECT * 
                                          MERGE INTO A 
                                          USING (SELECT A.ROWID row_id, B.REPORTIDSLIST
                                          FROM A ,RBL_MISDB_PROD.AdvAcOtherDetail A
                                                 JOIN tt_NEWTRANCHE_60 B   ON A.AccountentityID = B.AccountentityID 
                                           WHERE A.EFFECTIVEFROMTIMEKEY <= v_TimeKey
            ...
            BEGIN
               ...
               SELECT utils.stuff(( SELECT DISTINCT ',' || ChangeFields 
                 ...
                 INTO v_Parameter3
                 FROM DUAL ;
               IF utils.object_id('tt_AA_15') IS NOT NULL THEN
                EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AA_15 ';
               END IF;
               DELETE FROM tt_AA_15;
               UTILS.IDENTITY_RESET('tt_AA_15');

               INSERT INTO tt_AA_15 ( 
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."EXCEPTIONALDEGRATIONDETAILINUP" TO "ADF_CDR_RBL_STGDB";
