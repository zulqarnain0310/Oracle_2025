--------------------------------------------------------
--  DDL for Function METADYNAMICSCREENINSERUPDATE_04122023
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" 
-- ====================================================================================================================
 -- Author:			<Amar>
...
(
  ...
  v_TabID IN NUMBER,
  iv_Remark IN VARCHAR2 DEFAULT NULL ,
...
AS
   ...
   v_Remark VARCHAR2(200) := iv_Remark;
...
BEGIN
   ...
   DECLARE
      ...
      v_ClosureReasonAlt_Key NUMBER(5,0);
      v_ClosureDt VARCHAR2(200);

   BEGIN
      DBMS_OUTPUT.PUT_LINE('MetaDynamicScreenInserUpdate');
      ...
      IF utils.object_id('Tempdb..tt_MasterInfo_6') IS NOT NULL THEN
       EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MasterInfo_6 ';
      END IF;
      DELETE FROM tt_MasterInfo_6;
      ...
      IF v_OperationFlag = 1
       ...
       THEN
       DECLARE
         v_temp TT_MASTERINFO_6%ROWTYPE;
      ...
      BEGIN
         ...
         cv_ins := OPEN cv_3 FOR
         v_SQL;
         LOOP
            FETCH cv_ins INTO v_temp;
            EXIT WHEN cv_ins%NOTFOUND;
            INSERT INTO tt_MasterInfo_6 VALUES v_temp;
         END LOOP;
         CLOSE cv_ins;
         SELECT EntityKey 

           INTO v_BaseColumnValue
           FROM tt_MasterInfo_6 ;
      ...
      END;
      END IF;
      IF v_MenuID @MenuID = 640 AND @OperationFlag IN ( 16 , 3 ) BEGIN DROP TABLE IF  --SQLDEV: NOT RECOGNIZED = 640
        AND v_OperationFlag IN ( 16,3 )
       THEN

      BEGIN
         IF tt_data_2  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_data_2;
         UTILS.IDENTITY_RESET('tt_data_2');

         INSERT INTO tt_data_2 ( 
         	SELECT SickEntityId,
         	MAX( DECODE( PivotTable.ControlName, PivotTable.ClosureReasonAlt_Key, PivotTable.aggregate, null ) ) "ClosureReasonAlt_Key",
         	MAX( DECODE( PivotTable.ControlName, PivotTable.ClosureDt, PivotTable.aggregate, null ) ) "ClosureDt"                                
         	  FROM ( 
         	     SELECT UTILS.CONVERT_TO_NUMBER(v_BaseColumnValue,10,0) SickEntityId,
                      ControlName, 
                      MAX(DataVal)  aggregate
         	       FROM tt_TmpData_8 
         	      GROUP BY UTILS.CONVERT_TO_NUMBER(v_BaseColumnValue,10,0),
         	               ControlName ) PivotTable 
         	 GROUP BY SickEntityId );
         SELECT ClosureReasonAlt_Key ,
                ClosureDt 

           INTO v_ClosureReasonAlt_Key,
                v_ClosureDt
           FROM tt_data_2 
          WHERE  NVL(ClosureReasonAlt_Key, ' ') <> ' '
                   AND NVL(ClosureDt, ' ') <> ' ';
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               DBMS_OUTPUT.PUT_LINE(' 1 ' || v_AuthMode);
            ...
            ELSE
                  ...
                  BEGIN
                     ...
                     BEGIN
                        SELECT TimeKey 
                          ...
                          INTO v_DateTimeKey
                          FROM SysDayMatrix 
                         WHERE  UTILS.CONVERT_TO_VARCHAR2(SYSDATE,200) = UTILS.CONVERT_TO_VARCHAR2(Date_,200);
                        v_SQL := 'UPDATE ' || v_TableWithSchema_Mod || ' SET AuthorisationStatus=''FM''' ;
                        v_SQL := v_SQL || ', ModifiedBy=' || '''' || v_Modifiedby || '''' || ', DateModified=' || '''' || UTILS.CONVERT_TO_VARCHAR2(v_DateModified,19) || +'''' ;
                        v_SQL := v_SQL || ' WHERE (EffectiveFromTimeKey<=' || UTILS.CONVERT_TO_VARCHAR2(v_DateTimeKey,5) || ' AND EffectiveToTimeKey>=' || UTILS.CONVERT_TO_VARCHAR2(v_DateTimeKey,5) || ')' ;
                        v_SQL := v_SQL || CASE 
                                               WHEN v_ParentColumn <> ' ' THEN ' AND ' || v_ParentColumn || '= ' || v_ParentColumnValue
                        ELSE ' '
                           END ;
               ...
               ELSE
                  ...
                  BEGIN
                     IF v_MenuID = 640 THEN

                     BEGIN
                        DBMS_OUTPUT.PUT_LINE('3  640');
                        MERGE INTO A 
                        USING (SELECT A.ROWID row_id, CASE 
                        WHEN NVL(v_ClosureReasonAlt_Key, 0) = 1 THEN EffectiveFromTimeKey - 1
                        WHEN NVL(v_ClosureReasonAlt_Key, 0) IN ( 2,3 )
                         THEN ( SELECT SysDayMatrix.TimeKey 
                                FROM SysDayMatrix 
                                 WHERE  SysDayMatrix.Date_ = v_ClosureDt )   END AS pos_2, v_ClosureReasonAlt_Key, v_ClosureDt
                        FROM A ,AdvCustSickWeakUnits A 
                         WHERE EffectiveFromTimeKey <= v_TimeKey
                          AND EffectiveToTimeKey >= v_TimeKey
                          AND SickEntityId = UTILS.CONVERT_TO_NUMBER(v_BaseColumnValue,10,0)) src
                        ON ( A.ROWID = src.row_id )
                        WHEN MATCHED THEN UPDATE SET EffectiveToTimeKey = pos_2,
                                                     ClosureReasonAlt_Key = v_ClosureReasonAlt_Key,
                                                     ClosureDt = v_ClosureDt;
                  ...
                  ELSE
                     ...
                     BEGIN
                        ...
                        v_SQL := v_SQL || ' ApprovedBy=' || '''' || v_ApprovedBy || '''' || ', DateApproved=' || '''' || UTILS.CONVERT_TO_VARCHAR2(v_DateApproved,19) || '''' || ',Remark=' || '''' || v_Remark || '''' || ', AuthorisationStatus=''R''' ;
                        v_SQL := v_SQL || ', EffectiveToTimeKey=' || UTILS.CONVERT_TO_VARCHAR2(v_EffectiveFromTimeKey - 1,5) ;
                        v_SQL := v_SQL || '  WHERE (EffectiveFromTimeKey<=' || UTILS.CONVERT_TO_VARCHAR2(v_TimeKey,5) || ' AND EffectiveToTimeKey>=' || UTILS.CONVERT_TO_VARCHAR2(v_TimeKey,5) || ')' ;
                        v_SQL := v_SQL || CASE 
                                               WHEN v_ParentColumn <> ' ' THEN ' AND ' || v_ParentColumn || '= ' || v_ParentColumnValue
                     ...
                     ELSE
                        ...
                        BEGIN
                           DBMS_OUTPUT.PUT_LINE('16' || v_AuthMode);
                           ...
                           BEGIN
                              ...
                              v_SQL := v_SQL || 'AND ' || v_BaseColumn || '=''' || UTILS.CONVERT_TO_VARCHAR2(v_BaseColumnValue,20) || '''' ;
                              v_SQL := v_SQL || ' AND AuthorisationStatus in(''NP'',''MP'',''DP'')' ;
                              DBMS_OUTPUT.PUT_LINE('querry ' || v_SQL);
                              EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_MasterInfo_6 ';
                              cv_ins := EXECUTE IMMEDIATE v_SQL;
                              LOOP
                                 FETCH cv_ins INTO v_temp;
                              ...
                              DBMS_OUTPUT.PUT_LINE(+'before DElStatus=DP' || ' ' || v_SQL);
                              EXECUTE IMMEDIATE v_SQL;
                           ...
                           BEGIN
                              DBMS_OUTPUT.PUT_LINE('DelStatus ' || v_DelStatus);
                              ...
                              BEGIN
                                 ...
                                 BEGIN
                                    ...
                                    DBMS_OUTPUT.PUT_LINE(+'xyz' || v_SQL);
                                    EXECUTE IMMEDIATE v_SQL;
                              ...
                              BEGIN
                                 DBMS_OUTPUT.PUT_LINE(v_IsAvailable || ' 123 IsAvailable');
                                 ...
                                 DBMS_OUTPUT.PUT_LINE(+'AUTHMODE ' || v_AUTHMODE);
                                 v_SQL := v_SQL || CASE 
                                                        WHEN v_AUTHMODE = 'Y' THEN ',''' || (CASE 
                                                                                                  WHEN NVL(v_AuthorisationStatus, ' ') = ' ' THEN 'NULL'
                                                        ELSE v_AuthorisationStatus
                                                           END) || ''''
                                 ELSE ',NULL'
                                    END ;
                                 ...
                                 DBMS_OUTPUT.PUT_LINE('Hamiiddd');
                                 DBMS_OUTPUT.PUT_LINE(v_SQL);
                                 DBMS_OUTPUT.PUT_LINE(v_SQL || '1');
                                 v_SQL := v_SQL || CASE 
                                 ...
                                 DBMS_OUTPUT.PUT_LINE('rrr  ' || v_SQL);
                                 EXECUTE IMMEDIATE v_SQL;
                                 v_SQL := 'UPDATE ' || v_TableWithSchema || ' SET AuthorisationStatus=null WHERE  (EffectiveFromTimeKey<=' || UTILS.CONVERT_TO_VARCHAR2(v_TimeKey,5) || ' AND EffectiveToTimeKey>=' || UTILS.CONVERT_TO_VARCHAR2(v_TimeKey,5) || ')  and ISNULL(AuthorisationStatus,'''')=''NULL''AND ' || v_BaseColumn || '=''' || UTILS.CONVERT_TO_VARCHAR2(v_BaseColumnValue,20) || '''' ;
                                 DBMS_OUTPUT.PUT_LINE('pqr' || v_SQL);
                                 EXECUTE IMMEDIATE v_SQL;
                              ...
                              BEGIN
                                 ...
                                 ELSE ' '
                                    END ;
                                 v_SQL := v_SQL || ' AND ' || v_BaseColumn || '=''' || UTILS.CONVERT_TO_VARCHAR2(v_BaseColumnValue,20) || '''' ;
                                 DBMS_OUTPUT.PUT_LINE('abc' || v_SQL);
                                 EXECUTE IMMEDIATE v_SQL;
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."METADYNAMICSCREENINSERUPDATE_04122023" TO "ADF_CDR_RBL_STGDB";
