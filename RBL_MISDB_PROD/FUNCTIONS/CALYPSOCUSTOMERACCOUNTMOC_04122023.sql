--------------------------------------------------------
--  DDL for Function CALYPSOCUSTOMERACCOUNTMOC_04122023
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" 
(
  iv_TimeKey IN NUMBER DEFAULT 26479 ,
...
BEGIN
   ...
   SELECT TIMEKEY 
     ...
     FROM SysDayMatrix 
    WHERE  TIMEKEY = v_TIMEKEY;
   DELETE FROM tt_CUST_AC_MOC_2;
   UTILS.IDENTITY_RESET('tt_CUST_AC_MOC_2');

   INSERT INTO tt_CUST_AC_MOC_2 ( 
   	SELECT A.CustomerEntityId ,
           A.AccountEntityid ,
           RefInvID CustomerACID  ,
           NVL(BB.MOCAssetClassification, CL.AssetClassShortNameEnum) MOCAssetClassification  ,
           NVL(BB.NPADATE, NPIDt) NPADATE  ,
           AmountofWriteOff ,
           NVL(A.Balance, C.BookValueINR) Balance  ,
           UnservInterestAmt ,
           A.AdditionalProvision ,
           AdditionalProvisionAmount 
   	  FROM DataUpload_RBL_MISDB_PROD.CalypsoMocAccountDataUpload A
             LEFT JOIN DataUpload_RBL_MISDB_PROD.CalypsoMocCustomerDataUpload BB   ON A.CustomerEntityID = BB.CustomerEntityID
             JOIN RBL_MISDB_PROD.InvestmentFinancialDetail C   ON C.InvEntityId = A.AccountEntityId
             AND ( C.EffectiveFromTimeKey <= v_MocTimeKey
             AND C.EffectiveToTimeKey >= v_MocTimeKey )
             JOIN DimAssetClass CL   ON CL.AssetClassAlt_Key = C.FinalAssetClassAlt_Key
   	...
   	SELECT A.CustomerEntityId ,
           ...
           NVL(BB.NPADATE, NPIDt) NPADATE  ,
           AmountofWriteOff ,
           NVL(A.Balance, C.MTMIncomeAmt) Balance  ,
           UnservInterestAmt ,
           A.AdditionalProvision ,
           AdditionalProvisionAmount 
   	  FROM DataUpload_RBL_MISDB_PROD.CalypsoMocAccountDataUpload A
             LEFT JOIN DataUpload_RBL_MISDB_PROD.CalypsoMocCustomerDataUpload BB   ON A.CustomerEntityID = BB.CustomerEntityID
             JOIN CurDat_RBL_MISDB_PROD.DerivativeDetail C   ON C.DerivativeRefNo = A.CustomerAcID
             AND ( C.EffectiveFromTimeKey <= v_MocTimeKey
             AND C.EffectiveToTimeKey >= v_MocTimeKey )
             JOIN DimAssetClass CL   ON CL.AssetClassAlt_Key = C.FinalAssetClassAlt_Key
             AND ( CL.EffectiveFromTimeKey <= v_MocTimeKey
   ...
   INSERT INTO tt_CUST_AC_MOC_2
     ( SELECT A.CustomerEntityId ,
              ...
              0 AdditionalProvisionAmount  
       FROM DataUpload_RBL_MISDB_PROD.CalypsoMocCustomerDataUpload A
              ...
              JOIN InvestmentFinancialDetail D   ON D.InvEntityId = B.InvEntityId
              AND ( D.EffectiveFromTimeKey <= v_MocTimeKey
              AND D.EffectiveToTimeKey >= v_MocTimeKey )
              LEFT JOIN tt_CUST_AC_MOC_2 E   ON E.AccountEntityid = B.InvEntityId
       ...
       SELECT A.CustomerEntityId ,
              ...
              0 AdditionalProvisionAmount  
       FROM DataUpload_RBL_MISDB_PROD.CalypsoMocCustomerDataUpload A
              JOIN CurDat_RBL_MISDB_PROD.DerivativeDetail B   ON A.CustomerID = B.CustomerID
              AND ( B.EffectiveFromTimeKey <= v_MocTimeKey
              AND B.EffectiveToTimeKey >= v_MocTimeKey )
              LEFT JOIN tt_CUST_AC_MOC_2 E   ON E.CustomerACID = B.DerivativeRefNo
        WHERE  E.AccountEntityid IS NULL );
   INSERT INTO tt_CUST_AC_MOC_2
     ( SELECT A.CustomerEntityId ,
              B.InvEntityId ,
       ...
       FROM ( SELECT CustomerEntityId 
              FROM DataUpload_RBL_MISDB_PROD.CalypsoMocAccountDataUpload 
                GROUP BY CustomerEntityId ) A
              JOIN InvestmentBasicDetail B   ON A.CustomerEntityID = B.IssuerEntityid
              AND ( B.EffectiveFromTimeKey <= v_MocTimeKey
              AND B.EffectiveToTimeKey >= v_MocTimeKey )
              JOIN RBL_MISDB_PROD.InvestmentFinancialDetail C   ON C.InvEntityId = B.InvEntityId
              AND ( C.EffectiveFromTimeKey <= v_MocTimeKey
              AND C.EffectiveToTimeKey >= v_MocTimeKey )
              JOIN DimAssetClass CL   ON CL.AssetClassAlt_Key = C.FinalAssetClassAlt_key
              AND ( CL.EffectiveFromTimeKey <= v_MocTimeKey
       ...
       FROM ( SELECT CustomerEntityId ,
              ...
              AND ( B.EffectiveFromTimeKey <= v_MocTimeKey
              AND B.EffectiveToTimeKey >= v_MocTimeKey )
              JOIN DimAssetClass CL   ON CL.AssetClassAlt_Key = B.FinalAssetClassAlt_key
              AND ( CL.EffectiveFromTimeKey <= v_MocTimeKey
              AND CL.EffectiveToTimeKey >= v_MocTimeKey )
              LEFT JOIN tt_CUST_AC_MOC_2 E   ON E.AccountEntityid = B.DerivativeEntityID
        WHERE  E.AccountEntityid IS NULL );
   BEGIN
      ...
      BEGIN
         ...
         IF tt_CustNpa_2  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_CustNpa_2;
         UTILS.IDENTITY_RESET('tt_CustNpa_2');

         INSERT INTO tt_CustNpa_2 ( 
         	SELECT A.CustomerEntityId ,
                 CustomerId ,
                 CASE 
         	  ...
         	  FROM DataUpload_RBL_MISDB_PROD.CalypsoMocCustomerDataUpload A
                   LEFT JOIN InvestmentBasicDetail B   ON A.CustomerEntityID = b.IssuerEntityId
                   AND ( b.EffectiveFromTimeKey <= v_MocTimeKey
                   AND b.EffectiveToTimeKey >= v_MocTimeKey )
                   LEFT JOIN InvestmentFinancialDetail C   ON C.InvEntityID = b.InvEntityID
                   AND ( C.EffectiveFromTimeKey <= v_MocTimeKey
                   AND C.EffectiveToTimeKey >= v_MocTimeKey )
         	...
         	SELECT A.CustomerEntityId ,
                 ...
                 ELSE ACL1.AssetClassAlt_Key
                    END PostMocAssetClassAlt_key  
         	  FROM DataUpload_RBL_MISDB_PROD.CalypsoMocCustomerDataUpload A
                   LEFT JOIN CurDat_RBL_MISDB_PROD.DerivativeDetail B   ON A.CustomerID = b.CustomerID
                   AND ( b.EffectiveFromTimeKey <= v_MocTimeKey
                   AND b.EffectiveToTimeKey >= v_MocTimeKey )
         ...
         IF tt_Npadt_2  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_Npadt_2;
         UTILS.IDENTITY_RESET('tt_Npadt_2');

         INSERT INTO tt_Npadt_2 ( 
         ...
         ELSE NULL
            END AS pos_3
         FROM N ,tt_Npadt_2 N
                JOIN tt_CustNpa_2 C   ON N.CustomerEntityId = C.CustomerEntityId
         ...
         FROM A ,tt_CustNpa_2 A
                JOIN tt_Npadt_2 B   ON A.CustomerEntityId = B.CustomerEntityId ) src
         ...
         IF utils.object_id('Tempdb..tt_TmpCustNPA_2') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TmpCustNPA_2 ';
         END IF;
         DELETE FROM tt_TmpCustNPA_2;
         UTILS.IDENTITY_RESET('tt_TmpCustNPA_2');

         INSERT INTO tt_TmpCustNPA_2 ( 
         	SELECT NPA.* 
         ...
         FROM NPA ,AdvCustNPADetail NPA
                JOIN tt_TmpCustNPA_2 T   ON NPA.CustomerEntityId = T.CustomerEntityId
                AND ( NPA.EffectiveFromTimeKey <= v_MocTimeKey
                AND NPA.EffectiveToTimeKey >= v_MocTimeKey ) 
         ...
         FROM NPA ,AdvCustNPADetail NPA
                JOIN tt_Npadt_2 SD   ON NPA.CustomerEntityId = SD.CustomerEntityId
         ...
         IF utils.object_id('Tempdb..tt_TmpAcBalance_2') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TmpAcBalance_2 ';
         END IF;
         DELETE FROM tt_TmpAcBalance_2;
         UTILS.IDENTITY_RESET('tt_TmpAcBalance_2');

         INSERT INTO tt_TmpAcBalance_2 ( 
         	SELECT AABD.* 
         	  FROM RBL_MISDB_PROD.AdvAcBalanceDetail AABD
                   ...
                   JOIN tt_CUST_AC_MOC_2 T   ON ( ABD.AccountEntityId = T.AccountEntityId ) );
         DBMS_OUTPUT.PUT_LINE('Expire data');
         MERGE INTO AABD 
         USING (SELECT AABD.ROWID row_id, v_MocTimeKey - 1 AS EffectiveToTimeKey
         FROM AABD ,RBL_MISDB_PROD.AdvAcBalanceDetail AABD
                JOIN tt_TmpAcBalance_2 T   ON AABD.AccountEntityId = T.AccountEntityId
                AND ( AABD.EffectiveFromTimeKey <= v_MocTimeKey
                AND AABD.EffectiveToTimeKey >= v_MocTimeKey ) 
         ...
         ELSE AABD.NPS_Balance
            END AS pos_11
         FROM AABD ,RBL_MISDB_PROD.AdvAcBalanceDetail AABD
                JOIN tt_CUST_AC_MOC_2 T   ON AABD.AccountEntityId = T.AccountEntityId
                AND ( AABD.EffectiveFromTimeKey <= v_MocTimeKey
                AND AABD.EffectiveToTimeKey >= v_MocTimeKey )
         ...
         , OverduePrincipal, Overdueinterest, AdvanceRecovery, NotionalInttAmt, PrincipalBalance )
           ( SELECT A.AccountEntityId ,
                    ...
                    A.NotionalInttAmt ,
                    A.PrincipalBalance 
             FROM tt_TmpAcBalance_2 A
                    LEFT JOIN tt_CUST_AC_MOC_2 SD   ON A.AccountEntityId = SD.AccountEntityId
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_TempCustBalance_New_2  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_TempCustBalance_New_2;
         UTILS.IDENTITY_RESET('tt_TempCustBalance_New_2');

         INSERT INTO tt_TempCustBalance_New_2 ( 
         	SELECT ABD.CustomerEntityId ,
                 ABD.BranchCode ,
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_AdvCustFinancialDetail_O_2  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_AdvCustFinancialDetail_O_2;
         UTILS.IDENTITY_RESET('tt_AdvCustFinancialDetail_O_2');

         INSERT INTO tt_AdvCustFinancialDetail_O_2 ( 
         	SELECT F.* 
         	  FROM AdvCustFinancialDetail F
                   JOIN tt_TempCustBalance_New_2 N   ON F.EffectiveFromTimeKey <= v_MocTimeKey
         ...
         FROM ACFD ,AdvCustFinancialDetail ACFD
                JOIN tt_TempCustBalance_New_2 T   ON ACFD.CustomerEntityId = T.CustomerEntityId
         ...
         INSERT INTO PreMoc_RBL_MISDB_PROD.AdvCustFinancialDetail
           ...
           ( SELECT T.CustomerEntityId ,
                    ...
                    SYSDATE MocDate  ,
                    T.MocTypeAlt_Key 
             FROM tt_AdvCustFinancialDetail_O_2 T
                    JOIN tt_TempCustBalance_New_2 N   ON T.EffectiveFromTimeKey <= v_MocTimeKey
                    AND T.EffectiveToTimeKey >= v_MocTimeKey
         ...
         INSERT INTO AdvCustFinancialDetail
           ...
           ( SELECT A.CustomerEntityId ,
                    ...
                    SYSDATE MocDate  ,
                    MocTypeAlt_Key 
             FROM tt_TempCustBalance_New_2 A
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_AdvAcFinancialDetail_ORG_2  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_AdvAcFinancialDetail_ORG_2;
         UTILS.IDENTITY_RESET('tt_AdvAcFinancialDetail_ORG_2');

         INSERT INTO tt_AdvAcFinancialDetail_ORG_2 ( 
         	SELECT F.* 
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_AdvAcFinancialDetail_New_2  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_AdvAcFinancialDetail_New_2;
         UTILS.IDENTITY_RESET('tt_AdvAcFinancialDetail_New_2');

         INSERT INTO tt_AdvAcFinancialDetail_New_2 ( 
         	SELECT O.AccountEntityId ,
                 NVL(S.PostMoc_NPAdt, '1900-01-01') NpaDt  
         	  FROM tt_AdvAcFinancialDetail_ORG_2 O
         ...
         , RefCustomerId, RefSystemAcId, AuthorisationStatus, EffectiveFromTimeKey, EffectiveToTimeKey, CreatedBy, DateCreated, ModifiedBy, DateModified, ApprovedBy, DateApproved, MocDate, MocTypeAlt_Key, CropDuration, Ac_ReviewAuthLevelAlt_Key )
           ( SELECT T.AccountEntityId ,
             ...
             FROM tt_AdvAcFinancialDetail_ORG_2 T
                    JOIN tt_AdvAcFinancialDetail_New_2 N   ON T.EffectiveFromTimeKey <= v_MocTimeKey
                    AND T.EffectiveToTimeKey >= v_MocTimeKey
         ...
         , RefCustomerId, RefSystemAcId, AuthorisationStatus, EffectiveFromTimeKey, EffectiveToTimeKey, CreatedBy, DateCreated, ModifiedBy, DateModified, ApprovedBy, DateApproved, MocDate, MocTypeAlt_Key, CropDuration, Ac_ReviewAuthLevelAlt_Key )
           ( SELECT A.AccountEntityId ,
             ...
             FROM tt_AdvAcFinancialDetail_New_2 A
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC_04122023" TO "ADF_CDR_RBL_STGDB";
