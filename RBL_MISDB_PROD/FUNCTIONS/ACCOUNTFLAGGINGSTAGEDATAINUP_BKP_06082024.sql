--------------------------------------------------------
--  DDL for Function ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" 
(
            ...
            BEGIN
                  ...
                  BEGIN
                     ...
                     , ACTION, UploadTypeParameterAlt_Key, AuthorisationStatus, EffectiveFromTimeKey, EffectiveToTimeKey, CreatedBy, DateCreated )
                       ( SELECT v_ExcelUploadId ,
                                ACID ,
                                --,case when Amount='' then NULL else Amount end as Amount
                                CASE 
                                     WHEN Amount = 0.1 THEN 0.1
                                     WHEN Amount = 0.2 THEN 0.2
                                     WHEN Amount = 0.3 THEN 0.3
                                     WHEN Amount = 0.4 THEN 0.4
                                     WHEN UTILS.CONVERT_TO_NUMBER(Amount,18) <= 0 THEN 0
                                     WHEN Amount = ' ' THEN NULL
                                ELSE Amount
                                   END Amount ,--added by Prashant 30052024----
               ...
               BEGIN
                  ...
                  , ACTION, UploadTypeParameterAlt_Key, AuthorisationStatus, EffectiveFromTimeKey, EffectiveToTimeKey, CreatedBy, DateCreated, ModifyBy, DateModified, ApprovedBy, DateApproved )
                    ( SELECT ACID ,
                             Amount ,
                             --,case when cast(Amount as decimal) <= 0 then 0 else Amount end as Amount
                             Date_ ,
                             ACTION ,
                             --,@UploadType
                  ...
                  INSERT INTO ExceptionFinalStatusType
                    ...
                    ( SELECT B.SourceAlt_Key ,
                             ...
                             Date_ ,
                             Amount ,
                             --,case when cast(Amount as decimal) <= 0 then 0 else Amount end as Amount--added by Prashant 30052024----
                  ...
                  IF utils.object_id('TempDB..tt_Flags_10') IS NOT NULL THEN
                   EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Flags_10 ';
                  END IF;
                  DELETE FROM tt_Flags_10;
                  UTILS.IDENTITY_RESET('tt_Flags_10');

                  INSERT INTO tt_Flags_10 ( 
                  	SELECT A.ACID ,
                  ...
                  FROM A ,RBL_MISDB_PROD.AdvAcOtherDetail A
                         JOIN tt_Flags_10 B   ON A.RefSystemAcId = B.ACID 
                  ...
                  END IF;
                  -----------------------------------------------------------------------END---------------------------------------------------------------------------------
                  IF utils.object_id('TempDB..tt_Flags_101') IS NOT NULL THEN
                   EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Flags1_10 ';
                  END IF;
                  DELETE FROM tt_Flags1_10;
                  UTILS.IDENTITY_RESET('tt_Flags1_10');

                  INSERT INTO tt_Flags1_10 ( 
                  	SELECT A.ACID ,
                          B.SplCatShortNameEnum 
                  	  FROM AccountFlaggingDetails_Mod A
                            JOIN ( SELECT B.ParameterAlt_Key ,
                  	 ...
                  	 WHERE  A.UploadId = v_UniqueUploadID
                             AND A.ACTION = 'N' );
                  IF utils.object_id('TempDB..tt_Temp_16') IS NOT NULL THEN
                   EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Temp_16 ';
                  END IF;
                  DELETE FROM tt_Temp_16;
                  UTILS.IDENTITY_RESET('tt_Temp_16');

                  INSERT INTO tt_Temp_16 ( 
                  	SELECT A.AccountentityID ,
                          A.SplFlag 
                  	  FROM RBL_MISDB_PROD.AdvAcOtherDetail A
                            JOIN tt_Flags1_10 B   ON A.RefSystemAcId = B.ACID

                  	--where A.EffectiveToTimeKey=49999 
                  	WHERE  A.EFFECTIVEFROMTIMEKEY <= v_TimeKey
                            AND A.EFFECTIVETOTIMEKEY >= v_TimeKey );
                  --Select * from tt_Temp_16
                  IF utils.object_id('TEMPDB..tt_SplitValue_10') IS NOT NULL THEN
                   EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_SplitValue_10 ';
                  END IF;
                  DELETE FROM tt_SplitValue_10;
                  UTILS.IDENTITY_RESET('tt_SplitValue_10');

                  INSERT INTO tt_SplitValue_10 ( 
                  ...
                  --Select * from tt_SplitValue_10 
                  DELETE tt_SplitValue_10

                   WHERE  Businesscolvalues1 IN ( SELECT DISTINCT SplCatShortNameEnum 
                                                  FROM tt_Flags1_10  )
                  ;
                  IF utils.object_id('TEMPDB..tt_NEWTRANCHE_26') IS NOT NULL THEN
                   EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_NEWTRANCHE_26 ';
                  END IF;
                  DELETE FROM tt_NEWTRANCHE_26;
                  UTILS.IDENTITY_RESET('tt_NEWTRANCHE_26');

                  INSERT INTO tt_NEWTRANCHE_26 SELECT * 
                       FROM ( SELECT ss.AccountEntityID ,
                                     utils.stuff(( SELECT ',' || US.BUSINESSCOLVALUES1 
                                                   FROM tt_SplitValue_10 US
                                                    WHERE  US.AccountentityID = ss.AccountEntityID ), 1, 1, ' ') REPORTIDSLIST  
                              FROM tt_Temp_16 SS
                                GROUP BY ss.AccountEntityID ) B
                       ORDER BY 1;
                  --Select * from tt_NEWTRANCHE_26
                  --SELECT * 
                  MERGE INTO A 
                  USING (SELECT A.ROWID row_id, B.REPORTIDSLIST
                  FROM A ,RBL_MISDB_PROD.AdvAcOtherDetail A
                         JOIN tt_NEWTRANCHE_26 B   ON A.AccountentityID = B.AccountentityID 
                   WHERE A.EFFECTIVEFROMTIMEKEY <= v_TimeKey
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."ACCOUNTFLAGGINGSTAGEDATAINUP_BKP_06082024" TO "ADF_CDR_RBL_STGDB";
