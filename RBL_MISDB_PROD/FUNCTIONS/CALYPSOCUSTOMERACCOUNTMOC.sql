--------------------------------------------------------
--  DDL for Function CALYPSOCUSTOMERACCOUNTMOC
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" 
(
...
BEGIN
   ...
   SELECT TIMEKEY 
     ...
     FROM SysDayMatrix 
    WHERE  TIMEKEY = v_TIMEKEY;
   DELETE FROM tt_CUST_AC_MOC;
   UTILS.IDENTITY_RESET('tt_CUST_AC_MOC');

   INSERT INTO tt_CUST_AC_MOC ( 
   ...
   INSERT INTO tt_CUST_AC_MOC
     ( SELECT A.CustomerEntityId ,
       ...
       FROM DataUpload_RBL_MISDB_PROD.CalypsoMocCustomerDataUpload A
              ...
              LEFT JOIN tt_CUST_AC_MOC E   ON E.AccountEntityid = B.InvEntityId
       ...
       FROM DataUpload_RBL_MISDB_PROD.CalypsoMocCustomerDataUpload A
              ...
              AND ( B.EffectiveFromTimeKey <= v_MocTimeKey
              AND B.EffectiveToTimeKey >= v_MocTimeKey )
              LEFT JOIN tt_CUST_AC_MOC E   ON E.CustomerACID = B.DerivativeRefNo
        WHERE  E.AccountEntityid IS NULL );
   INSERT INTO tt_CUST_AC_MOC
     ( SELECT A.CustomerEntityId ,
              B.InvEntityId ,
       ...
       FROM ( SELECT CustomerEntityId ,
              ...
              AND ( CL.EffectiveFromTimeKey <= v_MocTimeKey
              AND CL.EffectiveToTimeKey >= v_MocTimeKey )
              LEFT JOIN tt_CUST_AC_MOC E   ON E.AccountEntityid = B.DerivativeEntityID
        WHERE  E.AccountEntityid IS NULL );
   BEGIN
      ...
      BEGIN
         ...
         IF tt_CustNpa  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_CustNpa;
         UTILS.IDENTITY_RESET('tt_CustNpa');

         INSERT INTO tt_CustNpa ( 
         	SELECT A.CustomerEntityId ,
                 CustomerId ,
                 CASE 
         ...
         IF tt_Npadt  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_Npadt;
         UTILS.IDENTITY_RESET('tt_Npadt');

         INSERT INTO tt_Npadt ( 
         ...
         ELSE NULL
            END AS pos_3
         FROM N ,tt_Npadt N
                JOIN tt_CustNpa C   ON N.CustomerEntityId = C.CustomerEntityId
         ...
         FROM A ,tt_CustNpa A
                JOIN tt_Npadt B   ON A.CustomerEntityId = B.CustomerEntityId ) src
         ...
         IF utils.object_id('Tempdb..tt_TmpCustNPA') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TmpCustNPA ';
         END IF;
         DELETE FROM tt_TmpCustNPA;
         UTILS.IDENTITY_RESET('tt_TmpCustNPA');

         INSERT INTO tt_TmpCustNPA ( 
         	SELECT NPA.* 
         ...
         FROM NPA ,AdvCustNPADetail NPA
                JOIN tt_TmpCustNPA T   ON NPA.CustomerEntityId = T.CustomerEntityId
                AND ( NPA.EffectiveFromTimeKey <= v_MocTimeKey
                AND NPA.EffectiveToTimeKey >= v_MocTimeKey ) 
         ...
         FROM NPA ,AdvCustNPADetail NPA
                JOIN tt_Npadt SD   ON NPA.CustomerEntityId = SD.CustomerEntityId
         ...
         IF utils.object_id('Tempdb..tt_TmpAcBalance') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TmpAcBalance ';
         END IF;
         DELETE FROM tt_TmpAcBalance;
         UTILS.IDENTITY_RESET('tt_TmpAcBalance');

         INSERT INTO tt_TmpAcBalance ( 
         	SELECT AABD.* 
         	  FROM RBL_MISDB_PROD.AdvAcBalanceDetail AABD
                   ...
                   JOIN tt_CUST_AC_MOC T   ON ( ABD.AccountEntityId = T.AccountEntityId ) );
         DBMS_OUTPUT.PUT_LINE('Expire data');
         MERGE INTO AABD 
         USING (SELECT AABD.ROWID row_id, v_MocTimeKey - 1 AS EffectiveToTimeKey
         FROM AABD ,RBL_MISDB_PROD.AdvAcBalanceDetail AABD
                JOIN tt_TmpAcBalance T   ON AABD.AccountEntityId = T.AccountEntityId
                AND ( AABD.EffectiveFromTimeKey <= v_MocTimeKey
                AND AABD.EffectiveToTimeKey >= v_MocTimeKey ) 
         ...
         ELSE AABD.NPS_Balance
            END AS pos_11
         FROM AABD ,RBL_MISDB_PROD.AdvAcBalanceDetail AABD
                JOIN tt_CUST_AC_MOC T   ON AABD.AccountEntityId = T.AccountEntityId
                AND ( AABD.EffectiveFromTimeKey <= v_MocTimeKey
                AND AABD.EffectiveToTimeKey >= v_MocTimeKey )
         ...
         , OverduePrincipal, Overdueinterest, AdvanceRecovery, NotionalInttAmt, PrincipalBalance )
           ( SELECT A.AccountEntityId ,
                    ...
                    A.NotionalInttAmt ,
                    A.PrincipalBalance 
             FROM tt_TmpAcBalance A
                    LEFT JOIN tt_CUST_AC_MOC SD   ON A.AccountEntityId = SD.AccountEntityId
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_TempCustBalance_New  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_TempCustBalance_New;
         UTILS.IDENTITY_RESET('tt_TempCustBalance_New');

         INSERT INTO tt_TempCustBalance_New ( 
         	SELECT ABD.CustomerEntityId ,
                 ABD.BranchCode ,
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_AdvCustFinancialDetail_O  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_AdvCustFinancialDetail_O;
         UTILS.IDENTITY_RESET('tt_AdvCustFinancialDetail_O');

         INSERT INTO tt_AdvCustFinancialDetail_O ( 
         	SELECT F.* 
         	  FROM AdvCustFinancialDetail F
                   JOIN tt_TempCustBalance_New N   ON F.EffectiveFromTimeKey <= v_MocTimeKey
         ...
         FROM ACFD ,AdvCustFinancialDetail ACFD
                JOIN tt_TempCustBalance_New T   ON ACFD.CustomerEntityId = T.CustomerEntityId
         ...
         INSERT INTO PreMoc_RBL_MISDB_PROD.AdvCustFinancialDetail
           ...
           ( SELECT T.CustomerEntityId ,
                    ...
                    SYSDATE MocDate  ,
                    T.MocTypeAlt_Key 
             FROM tt_AdvCustFinancialDetail_O T
                    JOIN tt_TempCustBalance_New N   ON T.EffectiveFromTimeKey <= v_MocTimeKey
                    AND T.EffectiveToTimeKey >= v_MocTimeKey
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_AdvAcFinancialDetail_ORG  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_AdvAcFinancialDetail_ORG;
         UTILS.IDENTITY_RESET('tt_AdvAcFinancialDetail_ORG');

         INSERT INTO tt_AdvAcFinancialDetail_ORG ( 
         	SELECT F.* 
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_AdvAcFinancialDetail_New  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_AdvAcFinancialDetail_New;
         UTILS.IDENTITY_RESET('tt_AdvAcFinancialDetail_New');

         INSERT INTO tt_AdvAcFinancialDetail_New ( 
         	SELECT O.AccountEntityId ,
                 NVL(S.PostMoc_NPAdt, '1900-01-01') NpaDt  
         	  FROM tt_AdvAcFinancialDetail_ORG O
         ...
         , RefCustomerId, RefSystemAcId, AuthorisationStatus, EffectiveFromTimeKey, EffectiveToTimeKey, CreatedBy, DateCreated, ModifiedBy, DateModified, ApprovedBy, DateApproved, MocDate, MocTypeAlt_Key, CropDuration, Ac_ReviewAuthLevelAlt_Key )
           ( SELECT T.AccountEntityId ,
             ...
             FROM tt_AdvAcFinancialDetail_ORG T
                    JOIN tt_AdvAcFinancialDetail_New N   ON T.EffectiveFromTimeKey <= v_MocTimeKey
                    AND T.EffectiveToTimeKey >= v_MocTimeKey
         ...
         , RefCustomerId, RefSystemAcId, AuthorisationStatus, EffectiveFromTimeKey, EffectiveToTimeKey, CreatedBy, DateCreated, ModifiedBy, DateModified, ApprovedBy, DateApproved, MocDate, MocTypeAlt_Key, CropDuration, Ac_ReviewAuthLevelAlt_Key )
           ( SELECT A.AccountEntityId ,
             ...
             FROM tt_AdvAcFinancialDetail_New A
...
/
  ...
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "MAIN_PRO";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "MAIN_PRO";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "RBL_TEMPDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "RBL_MISDB_PROD"."CALYPSOCUSTOMERACCOUNTMOC" TO "ADF_CDR_RBL_STGDB";
